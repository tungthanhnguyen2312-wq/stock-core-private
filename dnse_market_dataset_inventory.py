...
def dataset_inventory() -> list[dict[str, Any]]:
    ...
    entries = [
        ...
        _entry("trades_history", "trades_history", READY_FOR_RAW_INGEST,
               instrument_types=["ST/EQUITY for the current dynamic stock universe"],
               request_behavior="one Vietnam-local session; DESC; optional boardId; nextPageToken pagination to observed null terminal",
               time_dimensions="provider trade time plus requested session date", existing_support="bulk_ingest_dnse_intraday_history_raw.py",
               note="Raw retention is ready; quantity, volume, board, and ordering semantics remain unqualified."),
