...
def build(product:Mapping[str,Any], context:Mapping[str,Any], scenarios:Mapping[str,Any])->dict[str,Any]:
 ctx={x['ticker']:x for x in context['records']}; sc={x['ticker']:x for x in scenarios['scenarios']}; rows=[]
 for r in sorted(product['stock_research'],key=lambda x:x['ticker']):
  t=r['ticker']; f=r['ai_ready_brief']['facts']; a=r['research_summary']['fundamental_authority']; c=ctx[t]; s=sc.get(t)
  ...
  lenses={
   ...
   'CATALYST_RESEARCH':_v('CATALYST_RESEARCH','UNAVAILABLE','RESEARCH_SHADOW',['NO_EVIDENCE_BACKED_CATALYST'],{'scenario_present':bool(s)}),
 ...
 artifact={'schema_version':'1.0.0','contract_version':'strategy_research_eligibility/v1','registry':REGISTRY,'research_session':product['daily_market_research']['session'],'cohort_scope':'EMPIRICAL_ACTIVE_SHADOW_ONLY','records':rows,'coverage':{'records':len(rows),'per_lens_status_counts':{k:dict(v) for k,v in counts.items()},'top_reason_counts':dict(reasons),'at_least_one_usable':sum(any(v['eligibility'] in ('ELIGIBLE','ELIGIBLE_LOWER_AUTHORITY','PARTIAL') for v in r['lenses'].values()) for r in rows),'multiple_complementary_lenses':sum(sum(v['eligibility'] in ('ELIGIBLE','ELIGIBLE_LOWER_AUTHORITY') for v in r['lenses'].values())>=2 for r in rows),'fully_blocked':sum(not any(v['eligibility'] in ('ELIGIBLE','ELIGIBLE_LOWER_AUTHORITY','PARTIAL') for v in r['lenses'].values()) for r in rows)},'authority_boundary':{'not_a_signal_or_recommendation':True,'ranking':'NOT_EMITTED','portfolio_execution':'NOT_EMITTED'},'verdict':'STRATEGY_RESEARCH_ELIGIBILITY_V1_READY'}; artifact['artifact_sha256']=_hash(artifact);artifact['artifact_identity']='strategy_research_eligibility:'+artifact['artifact_sha256'];return artifact
