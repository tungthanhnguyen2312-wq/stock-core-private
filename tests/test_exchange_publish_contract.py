...
# Source files (the publisher and its entrypoints) live here; the published frontend and
# its generated data files are runtime state and live in the runtime root.
ROOT = Path(__file__).resolve().parents[1]


def _snapshot_rows():
    with require_runtime_path("screen_snapshot.csv").open("r", encoding="utf-8-sig", newline="") as handle:
        return list(csv.DictReader(handle))


def _fallback_rows():
    source = require_runtime_path("data", "screener_data.js").read_text(encoding="utf-8")
...
class ExchangePublishContractTests(unittest.TestCase):
    ...
    def test_shared_frontend_exchange_display_contract(self):
        formatter = require_runtime_path("assets", "js", "value-format.js").read_text(encoding="utf-8")
        self.assertIn('normalized === "HSX" ? "HOSE"', formatter)
        self.assertIn("displayExchange(row.exchange)", require_runtime_path("app.js").read_text(encoding="utf-8"))
        self.assertIn("displayExchange(r.exchange)", require_runtime_path("assets", "js", "company-panel.js").read_text(encoding="utf-8"))

    def test_dashboard_screener_is_full_width_without_internal_datatable_scroll(self):
        app = require_runtime_path("app.js").read_text(encoding="utf-8")
        dashboard = require_runtime_path("dashboard.html").read_text(encoding="utf-8")
    ...
    def test_build_manifest_public_schema_has_no_absolute_paths(self):
        manifest = json.loads(require_runtime_path("data", "build_info.json").read_text(encoding="utf-8"))
