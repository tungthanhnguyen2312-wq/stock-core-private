...
class BasisTests(unittest.TestCase):
 ...
 def test_provider_separation_and_unknown_volume(self):
  with tempfile.TemporaryDirectory() as d:
   r=qualify(*self.fixture(d));self.assertFalse(r["providers"]["VCI"]["price_basis_verified"]);self.assertEqual(r["providers"]["VCI"]["price_basis"],"unknown");self.assertIn("provider_series_retrospectively_rewritten",r["providers"]["VCI"]["limitations"]);self.assertEqual(r["providers"]["KBS"]["price_basis"],"unknown");self.assertEqual(r["providers"]["VCI"]["volume_basis"],"unknown")
