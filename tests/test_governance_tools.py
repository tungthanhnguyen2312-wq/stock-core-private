...
class GovernanceToolsTests(unittest.TestCase):
 def test_handoff_reads_canonical_state_and_contract(self):
  result = build_handoff(); self.assertIn("P0", result["active_phase"]); self.assertEqual(result["price_basis_status"], "INCONCLUSIVE"); self.assertIn("newly rotated, unexposed EODHD credential", result["active_milestone"])
