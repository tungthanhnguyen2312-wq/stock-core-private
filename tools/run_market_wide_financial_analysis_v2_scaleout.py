...
def main() -> int:
    ...
    p.add_argument("--qualified-flow-artifact", type=Path, help="Required for qualified standalone TTM/growth integration.")
    ...
    qualified_flow = json.loads(a.qualified_flow_artifact.read_text(encoding="utf-8")) if a.qualified_flow_artifact else None
    artifact = scaleout.build_scaleout(semantic_rows=rows, feature_records=records, feature_store_artifact=store, period_semantics_identity=semantic["artifact_identity"], requested_at=a.requested_at, legacy_records=legacy, classification_diagnostics_identity=classification_identity, qualified_flow_artifact=qualified_flow)
