...
def _bind_current_product_json(
    source: Path, target: Path, *, session: str, unavailable_reason: str, mismatch_reason: str,
) -> dict[str, Any]:
    """Copy a single-file current-product JSON artifact into the Dashboard web root only when
    its OWN ``as_of_session`` field matches the release session -- the same session-gated
    pattern already used for the cockpit projection above. A stale or absent source never
    leaves (or leaves behind) a stale file masquerading as current: on any non-exact outcome
    the existing web-root file, if any, is removed rather than left as an implicit stale
    carry-forward."""
    if not source.is_file():
        _remove_if_present(target)
        return {"status": "UNAVAILABLE", "source_session": None, "freshness": "EXACT_SESSION",
                "reason_codes": [unavailable_reason]}
    payload = _load_json_object(source, unavailable_reason.rsplit("_UNAVAILABLE", 1)[0])
    payload_session = payload.get("as_of_session") or payload.get("session")
    if payload_session != session:
        _remove_if_present(target)
        return {"status": "UNAVAILABLE", "source_session": payload_session, "freshness": "EXACT_SESSION",
                "reason_codes": [mismatch_reason]}
    _atomic_copy(source, target)
    return {"status": "CURRENT", "source_session": session, "freshness": "EXACT_SESSION",
            "artifact_identity": payload.get("artifact_identity"), "reason_codes": []}


def _bind_current_product_json_js_pair(
    json_source: Path, js_source: Path, json_target: Path, js_target: Path,
    *, session: str, unavailable_reason: str, mismatch_reason: str,
) -> dict[str, Any]:
    """Same session-gated binding as ``_bind_current_product_json``, but for a JSON+JS pair
    that must represent one logical projection: both files are copied from the exact same
    Producer-generated bytes for this operation (never independently sourced), so the pair can
    never diverge -- a stale JS fallback can never silently survive next to a current JSON, or
    vice versa. Missing/stale removes both target files, never just one."""
    if not json_source.is_file() or not js_source.is_file():
        _remove_if_present(json_target)
        _remove_if_present(js_target)
        return {"status": "UNAVAILABLE", "source_session": None, "freshness": "EXACT_SESSION",
                "reason_codes": [unavailable_reason]}
    payload = _load_json_object(json_source, unavailable_reason.rsplit("_UNAVAILABLE", 1)[0])
    payload_session = payload.get("as_of_session") or payload.get("session")
    if payload_session != session:
        _remove_if_present(json_target)
        _remove_if_present(js_target)
        return {"status": "UNAVAILABLE", "source_session": payload_session, "freshness": "EXACT_SESSION",
                "reason_codes": [mismatch_reason]}
    _atomic_copy(json_source, json_target)
    _atomic_copy(js_source, js_target)
    return {"status": "CURRENT", "source_session": session, "freshness": "EXACT_SESSION",
            "artifact_identity": payload.get("artifact_identity"), "reason_codes": []}
...
def publish_dashboard_release(
    ...
    *,
    ...
    # Investment Decision Workspace and Screener Master Projection: both come only from this
    # exact Daily Research Session Operation's current-product materialization step
    # (canonical_current_product_projections.py), never from a stale carry-forward web-root
    # file. Each is bound by its OWN payload's as_of_session -- never by a nearby artifact's
    # session (the defect this replaces: dashboard_release_publisher.py used to declare
    # `screening=CURRENT` unconditionally, with no reference to screener_master_projection at
    # all, while that file sat frozen at a stale session -- see
    # docs/dashboard_current_session_surface_coherence_20260912.md in the Dashboard repository).
    domains["investment_workspace"] = _bind_current_product_json(
        operation_dir / "investment_decision_workspace_projection.json",
        web_root / "data/investment_decision_workspace.json",
        session=session, unavailable_reason="EXACT_INVESTMENT_WORKSPACE_PROJECTION_UNAVAILABLE",
        mismatch_reason="INVESTMENT_WORKSPACE_SESSION_MISMATCH",
    )
    domains["screener_master"] = _bind_current_product_json_js_pair(
        operation_dir / "screener_master_projection.json",
        operation_dir / "screener_master_projection.js",
        web_root / "data/screener_master_projection.json",
        web_root / "data/screener_master_projection.js",
        session=session, unavailable_reason="EXACT_SCREENER_MASTER_PROJECTION_UNAVAILABLE",
        mismatch_reason="SCREENER_MASTER_SESSION_MISMATCH",
    )
