...
- Active phase: development runs on two pillars — A, market-wide canonical financial normalization (layers 1–4 complete and active); B, official corporate-action evidence, dated shares timeline, price adjustment factors, and provider share authority (measured and fail-closed as of P1J.1; **1 of 1,683 tickers has a qualified current share count** after B1.1, and the official ledger holds 1 qualified executed event). P0 market-data basis remains open and is the binding constraint: the **generic** price and volume basis are still `unknown`/`unverified`, so every current-market capability stays blocked — a **`vci.`-namespaced shadow** basis is qualified (P0-V/P0-W, below) and unlocks nothing generic. VCI is now recorded as **not raw-as-traded eligible**, which closed the previously-open P2a historical valuation path. P1 exact-session integrity, P1B/P1C/P1F/P1G/P1H/P1I/P1J/P1J.1 completed.
- Active milestone: P0-W VCI price-contract reconciliation + one-session intraday pagination — **PAGINATION_INCOMPLETE** (Part A resolved, Part B bounded). The Phase 3A verdict labelling 1,923,111 VCI rows `raw_as_quoted_no_adjustment_applied` was a **hard-coded module constant**, never derived or verified; it recorded that *this repository* applies no adjustment and was read as a claim about the provider. It is superseded with provenance by `provider_price_basis_registry:VCI@1.0.0` (`price_basis = empirically_event_adjusted`, `historical_mutability = retrospectively_rewritten`, `raw_as_traded_eligible = false`). Both production market-price citations (HPG 19,830 and VCB 60,560 at 2024-12-31 — neither on the HOSE tick lattice) now reject at `semantic_evidence_bridge` with `provider_series_retrospectively_rewritten`, which reopens **P2a historical point-in-time valuation as BLOCKED**. Providers with no established verdict are deliberately left ungated. Pagination: the intraday cursor is **strictly exclusive** and serves **only the current session**; VCB's morning segment reconciled to the share (1,873,500 enumerated + 3,500 measured-unenumerable = 1,877,000 = daily `v`), but a second holding ≥100 trades cannot be enumerated at one-second cursor resolution, so enumeration is `incomplete_cursor_failure` and `market_scope` stays `unknown`. See `operations-review/vci-contract-reconciliation-20260804/`.
