...
1. **Current-share source-authority decision** — P3-F5 completed as a read-only review. `VCI.overview.issue_share` remains `ISSUED_SHARES` and `AUTHORITY_NOT_PROMOTED_PENDING_OWNER_DECISION`; acquire independent provider semantic/effective-date evidence before any bounded provider-proxy policy decision. No market-cap or valuation activation follows from the review.
