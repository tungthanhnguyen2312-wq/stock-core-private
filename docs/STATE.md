...
- Active milestone: P0-X VCI volume market-composition closeout — **PARTIALLY_QUALIFIED**. One bounded probe of the price board (`POST /api/price/symbols/getList`, an endpoint `meta_sync.py` and `blacklist_sync.py` already call in production) returned 96 fields across 3 groups. **Opening-auction inclusion is qualified**: the board's `matchVolumeATO` 42,700 / `matchPriceATO` 60,900 / `firstTimeMatchPrice` 02:15:00Z agree exactly with the retained VCB tape's first trade of the session, which is the accumulator's first entry — so ATO volume sits inside daily `v`. Everything else stays `unknown`, and **`negotiated_inclusion` is closed as `unavailable_from_observed_vci_surfaces`** — not one of the 96 fields separates put-through from matched trading, and no first-party VCI definition exists anywhere. `accumulatedVolumeG1` equals `accumulatedVolume` exactly and was **refused** as evidence: an equality with no definition is arithmetic, not semantics. `market_scope = partially_qualified`, `liquidity_actionable = false`, `further_vci_pagination_authorized = false`. Evidence audit: 154 artifacts, 0 unreferenced (4 repaired), 0 secrets (2 prose false positives), 0 removed. See `operations-review/vci-volume-composition-20260804/`.
