...
- Active phase: development runs on two pillars — A, market-wide canonical financial normalization (layers 1–4 complete and active); B, official corporate-action evidence, dated shares timeline, price adjustment factors, and provider share authority (measured and fail-closed as of P1J.1; **1 of 1,683 tickers has a qualified current share count** after B1.1, and the official ledger holds 1 qualified executed event). P0 market-data basis remains open and is the binding constraint: the **generic** price and volume basis are still `unknown`/`unverified`, so every current-market capability stays blocked — a **`vci.`-namespaced shadow** basis is now qualified (P0-V, below) and unlocks nothing generic. P1 exact-session integrity, P1B/P1C/P1F/P1G/P1H/P1I/P1J/P1J.1 completed.
- Active milestone: P0-V Direct VN broker price/volume basis qualification pilot (VCI) — **PARTIAL**. `vci.raw_{open,high,low,close}` is qualified `split_and_dividend_adjusted`: the returned prices sit off the HOSE tick lattice, the off-lattice prefix ends **exactly** at a qualified ex-date for three tickers (VCB 2026-07-23 cash, HPG 2026-05-25 share issue, VNM 2026-06-26 cash), and a pre-ex-date DB snapshot proves 13/13 VCB closes were rewritten retroactively while a no-event control re-request came back byte-identical. Source documentation is **absent**, so the verdict is provider-namespaced and shadow-only. Volume field identity and unit (**shares**) are qualified from the provider's own accumulator arithmetic; `volume_market_scope` stays `unknown` — the intraday endpoint caps at 100 rows and the reconciliation could not complete. TCBS was **not implemented**: `vnstock` 4.0.4 ships no TCBS quote explorer and no bars endpoint is observable in this environment. Production and actionability gates unchanged and closed. See `operations-review/vci-direct-basis-pilot-20260804/`.
