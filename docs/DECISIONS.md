...
5. **Period correction and primary sequence.** The retained provider corpus is described as `retained_period_depth`, never annual history unless duration is actually evidenced. Where the historical 523-name artifact applies, approximately 449 proxy-ready corporate tickers and 17,276 retained provider facts remain available research context with duration semantics bounded by their contracts. The one executable next gate is `MARKET_WIDE_STRUCTURED_FINANCIAL_PERIOD_SEMANTICS_V1`, following completed operational-proxy and quarterly-feature foundations. Its strategic successors are `MARKET_WIDE_FUNDAMENTAL_FEATURE_STORE_V1`, `CURRENT_VALUATION_RESEARCH_PROXY_SCALEOUT_V1`, `MARKET_FUNDAMENTAL_OPPORTUNITY_INTEGRATION_V1`, and `PORTFOLIO_RISK_AND_CAPITAL_POLICY_RESEARCH_V1`; they are dependency-linked backlog, not simultaneous work. Existing RAW_AS_TRADED/PIT and execution-grade liquidity/sizing blockers remain intact.
