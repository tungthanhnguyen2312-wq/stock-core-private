...
`RELEASE_NOT_YET_AUTHORIZED`, local checkpoint only, not pushed
(`canonical_current_product_projections.py`, `tests/test_canonical_current_product_projections.py`;
`daily_producer_pipeline.py`, `dashboard_release_publisher.py` modified).

1. **Decision: extract/reuse the existing builder logic, never call the historical one-off
   tools from Daily.** `investment_decision_workspace_projection.py::build_artifacts()` and
   `screener_master_projection.py::build_projection()` were already clean,
   dependency-injected, session-parametric functions -- the defect was entirely in the fact
   that only a hand-run, hardcoded-session CLI wrapper (`tools/run_*.py`) ever called them.
   `current_valuation_opportunity_integration.py::build_artifacts()` (which produces the
   `opportunity_context/v1` + `security_decision_context/v1` pair the Workspace joins) is
   equally clean and equally only ever called by its own one-off tool. All three are reused
   unmodified; only a new orchestration layer (`canonical_current_product_projections.py`) was
   added, chaining them together with real current-session inputs. The historical tools are
   untouched and still work for manual reproduction.

2. **Decision: reuse `daily_research_session_operations.resolve_inputs()`'s own
   already-validated selection for `tactical` (`watchlist_tactical_entry_classifier`) and
   `valuation` (`market_wide_current_valuation`), rather than re-resolving them.** Both are
   already loaded, in memory, inside `daily_producer_pipeline.run_daily_producer()` at the
   exact point this milestone hooks in -- reading them a second time from disk would duplicate
   logic and risk a second, differently-behaved resolution path for inputs the governed
   session-input registry already owns.

3. **Decision: one explicit, deterministic path formula per remaining supplementary axis
   (`resolve_supplementary_inputs`), never a search.** `liquidity`
   (`market_wide_current_liquidity_research`), `leadership`
   (`current_market_sector_leadership_context`), and `financial_analysis_product_v2` all have
   genuine, real, per-session `operations-review/<slug>-v1-<YYYYMMDD>/` directories today
   (confirmed for 2026-09-11 by direct inspection before writing any code) -- the exact same
   naming convention `config/daily_research_session_input_registry.json` itself already encodes
   for its own registered axes, just not registered there (that registry's `REQUIRED`/`OPTIONAL`
   tuple is a fixed, separately-validated contract for a different, already-shipped product
   (`daily_research_session_operation/v1`); extending it for unrelated new keys was rejected as
   scope creep onto a contract this milestone does not own). A missing file at its one
   deterministic path resolves to `None`, never an error and never a substitute file.

4. **Decision: `feature_store` (`market_wide_fundamental_feature_store`) and
   `tactical_behavior` (`tactical_behavior_context`) are passed as `None`, not resolved from
   their only existing snapshot.** Both exist only as a single one-off 2026-08-31 snapshot,
   retained solely inside a now-detached feature worktree
   (`worktrees/stock-core-tactical-behavioral-engine-v2-20260831`,
   `worktrees/stock-core-market-wide-fundamental-feature-store-v1-20260831`) -- there is no
   recurring canonical-runtime materialization of either at all. Reading from that worktree
   path would (a) reintroduce, on two different axes, exactly the "frozen historical snapshot
   silently reused as current" defect this milestone exists to remove, and (b) create a
   canonical materializer with a hard runtime dependency on a specific sibling worktree
   continuing to exist on this machine, which fails this milestone's own "no historical
   search-root/dated-fallback in the canonical path" requirement in spirit even if not in the
   letter (a single hardcoded worktree path is not meaningfully more "canonical" than a
   dated-fallback list). **Consequence, verified empirically, not assumed**: because
   `security_decision_context.py`'s stance derivation requires at least one of the
   `fundamental`/`tactical` MAJOR_AXES to be `usable` (which is gated strictly on
   `bool(feature_record)`/`bool(behavior)`, independent of whether `watchlist` already supplies
   the *display* value for those axes), the retained-2026-09-11 replay's top-level
   `research_stance`/`entry_state` fields resolve to `INSUFFICIENT_EVIDENCE`/`NONE` for all
   1,683 tickers. This is reported as a known, disclosed capability gap (see
   `docs/canonical_current_product_projections_replay_20260912.md`), not hidden or worked
   around by reaching into the fragile worktree snapshot -- the milestone's actual target (the
   session-identity binding defect) is fixed either way; a follow-up milestone to build a
   recurring materializer for these two specific axes is recommended, not opened here.

5. **Decision: `dashboard_release_publisher.py` gains two new domains
   (`investment_workspace`, `screener_master`) rather than redefining `screening`'s existing
   meaning.** `screening`'s `CURRENT`/`EXACT_SESSION` claim (screen_snapshot/breadth currency)
   was itself never wrong -- the defect was that the Screener UI's *actual* primary data
   source (`screener_master_projection.json`) had no domain of its own at all, so its own
   staleness was invisible to `build_info.json`. Both new domains follow the exact session-gated
   copy-or-remove pattern the `cockpit` domain already used: bind only when the artifact's own
   `as_of_session` matches the release session; on any mismatch or absence, remove the existing
   web-root file rather than leave a stale one silently in place, and report an explicit
   `UNAVAILABLE`/reason code. Both new filenames were added to `DASHBOARD_RELEASE_ALLOWLIST`,
   so they are automatically included in `build_info.json`'s `files` hash/size manifest exactly
   like every other bound artifact -- no separate manifest-writing code was needed.

6. **Verification.** Retained-2026-09-11 replay (no network, no provider acquisition): both
   products materialize `as_of_session=2026-09-11` from real governed evidence (full stats in
   `docs/canonical_current_product_projections_replay_20260912.md`). 16 new tests
   (`tests/test_canonical_current_product_projections.py`), all pass; one retained-evidence
   replay test skips gracefully in a checkout without local `operations-review` evidence
   (e.g. a fresh worktree or hosted CI) rather than failing. All existing
   ...
   `daily_producer_pipeline` test suites pass unmodified (3 pre-existing
   `test_daily_producer_pipeline.py` failures are a worktree-local retained-evidence gap,
   confirmed present in the primary checkout too by direct file check -- unrelated to this
   milestone). Full updated W1-W4 focused-regression command (393 tests, matching
   `.github/workflows/producer-ci.yml`'s `focused-regressions` job with the 5 new files added)
   reproduces green locally in 71s. `py_compile`, JSON parse, `git diff --check`, and
   `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. No production analytical
   behavior, provider/source authority, or recommendation semantics changed anywhere. Not
...
`queued_next=[]` (explicit override, `AI_RULES.md` rule 11). Implementation + local checkpoint
only -- not pushed, not merged, not deployed; no live provider acquisition was run.

**Decision.** Moved KBS/VCI transport for canonical exact-session acquisition into one bounded
worker subprocess per operation, using the approved architecture (parent keeps ALL policy; worker
is transport only) unmodified from the brief -- repository evidence did not surface any actual
incompatibility, so the high-level design was not reopened. New modules: `vnstock_worker_
protocol.py` (stdlib-only wire schema/exceptions, shared by both sides), `vnstock_worker_
process.py` (the worker subprocess entrypoint -- the ONLY file in the exact-session path that
imports `vnstock`/`vnai` now), `vnstock_worker_client.py` (parent-side `VnstockWorkerFetcher`, a
drop-in replacement for `vn_stock_pipeline.fetch_single_source`'s call signature/return shape).
Integration is exactly two lines of injection into `daily_session_level2_package.
ensure_exact_session_snapshot`'s existing call to `resolve_exact_session_with_autorecovery`
(`fetch_single_source=worker_fetcher.fetch, rate_governor=worker_fetcher`) plus a `finally` for
deterministic worker shutdown and active-governor restoration -- zero changes to
`multi_source_exact_session_resolver.py`'s or `multi_source_market_evidence_contract.py`'s own
Pass 2-6 policy logic, because both already exposed `fetch_single_source`/`rate_governor` as
injectable parameters (this milestone's whole design rides that pre-existing seam rather than
inventing a new one).

**Behavior-freeze finding (documented in `docs/STATE.md`'s new entry, not repeated here): the
brief's assumed ordinary-vs-degraded KBS/VCI asymmetry does not exist in the actual code** -- both
paths are already KBS-first/VCI-on-genuine-error-only via one shared constant. Treated as a
premise correction, not a stop condition: the equivalence suite asserts symmetry (matching current
code), and the architecture/scope were unaffected.

**Rate-governor ownership.** The worker installs and owns the real `VnstockRateGovernor` for the
operation's lifetime (45 req/60s shared budget, KBS 2-worker/0.25s cap, VCI sequential --
unchanged numbers). The parent's `_DailyRecoveryRuntimeGuard` and evidence-embedded governor
diagnostic are served by `VnstockWorkerFetcher` itself acting as a duck-typed shim:
`estimated_minimum_seconds_for` is pure arithmetic over fixed constants (reproduced exactly, no
round-trip needed) and `.diagnostic()` round-trips to the worker's real governor -- deliberately
never a parent-side governor object that would "pretend" to gate a child process's requests (the
brief's explicit prohibition). A real bug was found and fixed during implementation: an explicit
non-None `rate_governor=` makes the resolver's existing wrapper skip restoring the module-global
active-governor pointer afterward (by design, for its OWN caller-owned-governor use case) --
`ensure_exact_session_snapshot` now captures and restores it explicitly in its own `finally`,
without which a shut-down worker reference would leak into any later same-process
`vn_stock_pipeline` call. Caught via a real, reproducible test-suite cross-contamination (not a
hypothetical): running the existing `test_vnstock_rate_governor.py`/`test_vn_stock_pipeline.py`
files immediately after this milestone's own tests failed with `AttributeError: 'VnstockWorkerFetcher'
object has no attribute 'acquire'` until fixed.

**Failure semantics.** Five typed `VnstockWorkerFailure` subclasses (startup failure, protocol
violation, timeout, process-exit, adapter-exception) cover: startup failure, `SystemExit`, hard
crash, hang (bounded per-request timeout), and malformed/duplicate/unknown-request-id responses --
all fail closed, all raise rather than fabricate a provider outcome, none silently restart the
worker or reset its budget. Verified against a deterministic fake worker
(`tests/fixtures/fake_vnstock_worker.py`) implementing the identical protocol -- no real
`vnstock`/`vnai`/network anywhere in the new test files.

**Import containment.** `tests/test_vnstock_worker_import_containment.py` runs its check in an
isolated subprocess (`tests/fixtures/check_parent_import_containment.py`) specifically because an
in-process `sys.modules` assertion would be contaminated by other test files in the same pytest
session that legitimately import `vn_stock_pipeline`/`vnstock` directly -- confirmed: `vnstock`/
`vnai` never enter that subprocess's own `sys.modules` after importing `multi_source_exact_
session_resolver`/`daily_session_level2_package`-equivalent parent modules and driving a real
worker round-trip against the fake worker.

**Verification.** New tests: 13 (`test_vnstock_worker_client.py`) + 13 (`test_vnstock_exact_
session_worker_equivalence.py`, covering acceptance cases A-I and N with golden BEFORE/AFTER
equivalence, worker-only diagnostics excluded from the comparison per the brief) + 1
(`test_vnstock_worker_import_containment.py`) = 27, all passing. Existing focused-domain suite
(`test_multi_source_exact_session_resolver`, `test_multi_source_market_evidence_contract`,
`test_daily_session_level2_package`, `test_vnstock_rate_governor`, `test_vn_stock_pipeline`): 201
passed, 4 pre-existing failures unrelated to this milestone (3 gitignored-evidence-absent-in-a-
fresh-worktree gaps; 1 cross-test active-governor leak reproduced identically with none of this
milestone's files present). Hosted Producer CI's exact 3 jobs reproduced green locally
(`daily_session_level2_package.ensure_exact_session_snapshot` is mocked out entirely above its own
call site by the production-call-shape smoke, so this milestone's worker-spawning code is never on
that smoke's path and needed no CI changes). `python tools/stocklookup_roadmap.py --check`:
`ON_TRACK`. `py_compile`, `git diff --check` clean.

**Scope discipline / unchanged.** `macro_sync.py` (VCB/SJC, a separate later milestone, "W4"),
financial/company/profile/ownership VNStock uses, provider authority, source thresholds, the
tactical system, Dashboard, AI handoff semantics, portfolio sizing, and PIT/RAW_AS_TRADED are all
untouched. Canonical source identity remains `DNSE`/`KBS`/`VCI` only -- the worker is never
promoted into source authority or embedded as a fourth "source". This worker exists for exact-
session KBS/VCI recovery/corroboration only; it is not a generalized VNStock daemon and no
package-removal is queued. Not a security sandbox -- documented explicitly in both new worker
files' own docstrings and in `docs/STATE.md`.

**Disposition.** `COMPLETE_LOCAL`. `RELEASE_NOT_YET_AUTHORIZED` -- push/merge/deploy/publication
and hosted-CI observation belong to a later, separately authorized release step.
...
directly in chat, executed by Claude Code from a fresh worktree/branch rooted exactly at
`origin/main = 6bbce33af42ccebd40eb8da917d1b7af669b0a7d`.

**Root cause of the AI-handoff macro gap**: read directly from the retained 2026-09-11
`ai_research_session_bundle.json`, `market.macro` genuinely was
`{"status": "UNAVAILABLE", "reason": "NO_EXPLICIT_MACRO_ARTIFACT_BOUND"}`. Tracing
`canonical_daily_operation.py` confirmed why: it never passed a `macro=` argument into
`daily_producer_pipeline.produce()` at all, and its separate `macro_sync.py` refresh call ran
strictly *after* `produce()` returned (line ~735, after the Producer call at line ~712) -- even a
correctly-wired `macro=` would have been too late. No retained `current_macro_regime/v1` artifact
exists anywhere (`grep -rl current_macro_regime operations-review` returns zero files), and
`current_macro_regime.py` performs live FRED/SBV network calls with no offline acquisition path --
so **Option A** (bind an existing qualified `current_macro_regime` artifact) was not available
under this milestone's explicit no-network-call constraint.

**Selected Option B**: the runtime `macro_sync.py` snapshot (`data/macro_snapshot.json`) is real,
already retained, and structurally distinct from `current_macro_regime/v1` (no regime
classification, no retained raw per-observation provider payload). New `macro_presentation_context.
py` (`macro_presentation_context/v1` -- a contract_version deliberately distinct from
`current_macro_regime/v1`, so nothing downstream can mistake a descriptive snapshot wrapper for
evidence-bound regime research) wraps it for AI-consumer presentation only: per-series `source`,
an honest `source_authority` tag (FRED/World Bank -> official; Vietcombank/SJC -> first-party quote
over an unofficial transport; Yahoo Finance -> unofficial market data -- freshness never upgrades
authority), and a cadence-aware `freshness_history.py` envelope per series. `freshness_history.py`
gained four new domain rules (`macro_annual`, `screening`, `valuation`, `market_flow`,
`integrated_decision`) purely additively; every existing rule is untouched.

**Single-refresh ordering fix**: the existing macro-sync subprocess call was *moved*, never
duplicated, from strictly-after-the-Producer to strictly-before-it (right after `enrichment`, right
before `produce()`), so the same retained snapshot the Dashboard later reads from disk is also the
one available to the AI handoff. A dedicated production-call-shape regression
(`test_production_call_shape_macro_refresh_and_load_run_once_before_producer`) proves the refresh
runs exactly once, strictly before the Producer, and that the Producer receives the identical
presentation-context object later surfaced on the operation record -- the same seam that let the
2026-09-11 `load_registry` NameError and the original macro-after-Producer gap both ship unnoticed.
A second regression (`test_macro_refresh_failure_does_not_block_core_daily_and_reports_unavailable`)
proves a failed refresh degrades to an explicit unavailable presentation context without blocking
or weakening the exact-session equity core. The new context threads additively (new
`macro_presentation_context=` parameter, default `None`) through `run_daily_producer` ->
`run_session_operation` -> `build_operation` -> `ai_research_session_delivery.build_delivery`,
landing as `primary.market.macro_presentation_context` alongside the untouched, still-unbound
`primary.market.macro` (`current_macro_regime/v1` -- Option A remains available, unused, for a
future milestone that actually acquires a qualified artifact).

**New `ai_handoff_source_freshness_matrix.py`** (`ai_handoff_source_freshness_matrix/v1`)
additively classifies all 11 requested domains -- market price/descriptive, breadth and sector
(both descriptive sub-fields, not separately tracked session inputs today), tactical, screening,
...
`UNAVAILABLE_INTERNAL`, plus a mechanically-derived `online_enrichment_guidance`
(`INTERNAL_SUFFICIENT`/`INTERNAL_PARTIAL`/`EXTERNAL_CONTEXT_MAY_BE_REQUIRED`). It is built entirely
from evidence `build_operation` already resolves -- no new acquisition, no recomputed analytical
value, and it never emits an investment conclusion (a dedicated test asserts no buy/sell/target/
probability/sizing/execution token appears anywhere in its output). Two domains are classified from
genuine per-artifact evidence rather than session presence alone: **fundamentals** reads
`PARTIAL_INTERNAL`/historical because the registered `market_wide_current_fundamental_research`
artifact identity is confirmed byte-identical across every session from 2026-08-21 through
2026-09-11 with no embedded per-session timestamp -- a real, permanent evidence-shape limit, not a
defect, so it is never falsely asserted exact-session current; **catalyst** reads its own real
`research_session` field (confirmed: `2026-08-20`, 22 days stale as of 2026-09-11) through
`freshness_history`'s existing `corporate_events` domain rule and correctly resolves
`STALE_INTERNAL` with an `EARLIER_RETAINED_CATALYST_CONTEXT` reason code, never silently current.

**Market-flow / foreign-flow (items 8-9)** stay three distinct, never-collapsed evidence classes:
`qualified_dnse_value_flow` (bound to `inputs.market_flow_positioning` only when a genuinely
same-session artifact is registered; for 2026-09-11 this is honestly `UNAVAILABLE_INTERNAL` /
`NO_COMPATIBLE_CURRENT_MARKET_FLOW_ARTIFACT`, matching the retained bundle's real
`flow_coverage: null` exactly -- the stale 2026-08-24 artifact the weekly audit flagged is never
revived or relabeled current), `macro_snapshot_foreign_flow` (`macro_sync.py`'s own snapshot-level
foreign-flow slot, honestly always unavailable in every retained snapshot inspected), and
`proprietary_flow` (explicitly `NO_PROPRIETARY_FLOW_CAPABILITY_THIS_MILESTONE`, never inferred).

**Real, read-only, no-network validation** against the actual retained 2026-09-11
registry/evidence and the actual retained `data/macro_snapshot.json` (7 weeks stale in this dev
environment, since nothing has run `macro_sync.py` here under this milestone's no-network
constraint): macro status moves from a bare `null`/`UNAVAILABLE` to `macro_presentation_context.
status = PARTIAL` (2 of 17 series `current`, 15 honestly `stale`/`expiring` under their own cadence
rules -- a true reflection of local snapshot age, not a code defect); 7 of the 11 domains resolve
`CURRENT_INTERNAL`/`INTERNAL_SUFFICIENT` (market price/descriptive, breadth, sector, tactical,
screening, valuation, corporate intelligence); `catalyst_event` and `market_flow_positioning`
correctly flag `EXTERNAL_CONTEXT_MAY_BE_REQUIRED`. The validation script itself did not wire the
real Producer's `integrated_delivery` object, so its `integrated_decision` reading
(`UNAVAILABLE_INTERNAL`) reflects that script's own scope, not a production claim -- the dedicated
`canonical_daily_operation.py` regressions exercise the real production call shape for that path
instead. Nothing under `stocklookup-ai-handoffs/sessions/2026-09-11` or the Dashboard's own
2026-09-11 build was touched, read for anything but the *before* comparison, or republished.

**Tests/validation**: 34 new focused tests (15 `test_macro_presentation_context.py`, 17
`test_ai_handoff_source_freshness_matrix.py`, 2 new ordering/failure-isolation regressions in
`test_canonical_daily_operation.py`) all pass. Because this feature worktree's own
`operations-review/` is gitignored and worktree-local (same known gap the prior milestone
documented), the directly-touched modules' existing suites (`test_daily_research_session_
operations.py`, `test_daily_producer_pipeline.py`, `test_ai_research_session_delivery.py`,
`test_ai_research_ticker_extractor.py`, `test_financial_analysis_v2_product_integration.py`,
`test_canonical_daily_operation.py` -- 62 tests total) were verified against real retained evidence
read-only-copied from the primary checkout rather than accepted on the fresh-worktree baseline
alone; zero regressions. The sole remaining failure
(`test_isolated_2026_08_26_full_replay_reaches_published_without_dispatch`) is the identical
pre-existing gitignored-evidence gap the prior milestone already documented, untouched by this
diff. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`)
all clean.

**Guardrails held**: no new market-data provider; no numerical analytical output change anywhere
(source-freshness metadata is purely additive presentation); `current_macro_regime/v1` binding,
RAW_AS_TRADED, historical PIT, liquidity/position-sizing authority, intrinsic/reverse valuation,
portfolio sizing, active-universe authority, Dashboard UI, the VNStock rate governor, and every
tactical-reversal shadow/prospective/registry-reconciliation/session-comparison chain are
untouched; no provider or network call was made; no Daily run was executed; no historical AI
handoff or Dashboard build was rewritten or republished; not pushed, not merged, not deployed. No
successor is queued.

## 2026-09-12 - Session Registry Promotion and Comparison-Semantics Corrective V1

`SESSION_REGISTRY_PROMOTION_AND_COMPARISON_SEMANTICS_CORRECTIVE_V1 = COMPLETE`. Owner-directed
corrective following the 2026-09-12 weekly full-system rebaseline audit, executed by Claude Code
from a fresh worktree/branch rooted exactly at `00adacef99c338c00e18c8dea147479176bec2d2`
(`78457cd -> d758814 -> 3cd7c88 -> 00adace`), origin/main unchanged at `78457cd`.

**Root cause**: `validate_and_freeze_completed_session` already froze a real, correct result on
every completed Daily operation -- confirmed directly from the retained 2026-09-10 operation
record's own embedded `freeze.status == "FROZEN"` with zero `incompatible_inputs` -- but that
freeze is a local file write with no automatic `git commit`, and the registry file is already an
explicitly allowlisted "known operational diff" in `docs/ROADMAP_STATE.json`, so an uncommitted
local registration is not treated as anomalous. 2026-08-27/28 and 2026-09-03/04 ran from ephemeral
integration/release-candidate worktrees whose local registry write was never reconciled back;
2026-09-10 ran against local `main` at `bf11dbf` (never pushed). Downstream AI-handoff/Dashboard
publication gates only on that run's own local `LOCAL_COMPLETE`/`READY` state, not on the registry
write reaching the shared checkout, so publication proceeded regardless. The forward
register-then-freeze-then-downstream ordering inside a single `canonical_daily_operation.py` run
was already correct (no ordering bug); this is now covered by a new production-call-shape
regression that proves `load_registry(root)` -- the exact registry object the comparison resolver
and `comparison_metadata` are built from -- is read strictly after `freeze`, using the same
`producer_fn=None` real call shape the 2026-09-11 `load_registry` NameError slipped through on.

**New `session_registry_gap_reconciliation.py`** classifies each of the 8 audited sessions
(`GOVERNED_COMPLETED` / `QUALIFIES_FOR_RETROACTIVE_GOVERNED_REGISTRATION` /
`PUBLISHED_BUT_NOT_PROVABLY_GOVERNED` / `NONCURRENT_CHECKPOINT_REQUIRES_OWNER_REVIEW` /
`INSUFFICIENT_RETAINED_EVIDENCE` / `NOT_APPLICABLE`) by literally re-running today's real
`register_session_inputs`/`validate_and_freeze_completed_session` against whatever retained
evidence still exists -- publication is never itself accepted as proof. Result: 2026-08-21/24/25/
26 and 2026-09-11 already `GOVERNED_COMPLETED`; 2026-08-27 and 2026-09-03
`INSUFFICIENT_RETAINED_EVIDENCE` (their retained descriptive-research artifact is gone from disk
entirely -- left unregistered, fail-closed, not blindly promoted); 2026-08-28, 2026-09-04, and
2026-09-10 `QUALIFIES_FOR_RETROACTIVE_GOVERNED_REGISTRATION` and are now registered in
`completed_sessions` with real frozen input identities. 2026-09-10's noncurrent `bf11dbf`
checkpoint was neither auto-accepted nor auto-rejected: its `REQUIRED_REGISTRY_KEYS`/
`REGISTRY_KEY_TO_LEVEL2_KEY` contract is confirmed byte-identical to today's
(`git show bf11dbf:canonical_post_close_pipeline.py`), and the real, current
`validate_and_freeze_completed_session` was actually re-run against its retained evidence and
succeeded cleanly -- genuine re-derived evidence, not an inference from the checkpoint. Bounded
audit artifact:
...
**Consequence, not a design goal**: the nearest valid comparator for 2026-09-11 is now 2026-09-10
(`FRESH_ADJACENT`, `IMMEDIATE_PREVIOUS_GOVERNED_SESSION`) purely because that session's evidence
independently passed today's exact registration contract -- 2026-09-10 was not special-cased to
produce this outcome, and had it failed reconciliation the comparator would have stayed
2026-08-26 with the gap explicitly reported.

**New `session_comparison_semantics.py`** adds `comparison_metadata` (`comparison_session`,
`comparison_session_role`, `session_gap_trading_sessions`,
`is_immediate_previous_completed_session`, `comparison_fitness`, `comparison_reason_codes`,
`skipped_known_sessions`, `notice`) to `next_session_decision_brief`'s existing
`previous_qualified_session` field -- additive, never renamed -- propagated unchanged into
`daily_integrated_decision_brief.py` and into `ai_handoff_publication.py`'s package `lineage` and
published `LATEST.json`. The session gap is counted only from what the registry's
`completed_sessions`/`sessions` maps already record as having happened; it is never derived from
calendar-day subtraction, and reports zero/`FRESH_ADJACENT` rather than a fabricated gap when the
registry holds no evidence one exists. A direct regression pins the exact historical shape this
milestone exists to fix (2026-09-11 vs. a 2026-08-26 comparator with five known intervening
sessions) so it can never again read as an ordinary adjacent transition.

**Guardrails held**: no provider/network acquisition; no runtime database mutation; no AI handoff
publication or Dashboard deployment; no repository-visibility change; no push, merge, or rebase;
the protected stash and local `main`/`bf11dbf` lineage were inspected read-only and never touched;
no unrelated worktree was repaired. Current-session breadth, tactical, valuation, fundamentals,
corporate intelligence, and Integrated Decision outputs are unchanged (`comparison_metadata`
touches only comparison interpretation/lineage). The tactical-reversal shadow chain, Candidates
A/B/C, and the historical bootstrap mapping are unmodified.
`IMMUTABLE_SESSION_OPERATION_CONTENT_CONFLICT` was not reopened: no new evidence of a
nondeterministic rerun, competing lineage, or identity instability was produced, so the prior
`EXPECTED_IMMUTABILITY_PROTECTION` classification stands unchanged.

**Tests**: 26 new focused tests (9 registry-gap-reconciliation, 7 comparison-semantics unit, 6
self-contained comparison-metadata end-to-end through the real unmocked
`next_session_decision_brief.build_artifact`, 2 `daily_integrated_decision_brief` propagation, 1
`ai_handoff_publication` propagation, 1 production-call-shape ordering regression). Full
before/after test-file comparison across every affected module reproduces the identical
pre-existing fresh-worktree baseline (22 failed / 17 errored, all previously documented
gitignored-`operations-review`-evidence gaps) with zero new failures; 197 passed (171 baseline +
this milestone's 26). `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check`
(drift `PASS`) are all clean. No successor is queued.
...
Owner authorization records a bounded exception to the empty roadmap queue, not a
successor-policy inference. This wires only the two candidates the counterfactual evaluation
supported (momentum-bucket R8 subset, two-session R8 persistence) as a read-only annotation
layer over the unchanged, already-produced classifier output; Candidate C stays excluded and
named, never silently dropped or reintroduced. `tactical_reversal_shadow_probe_policy.py`
imports and calls `tactical_reversal_probe_policy_counterfactual_evaluation.evaluate_candidate`
directly rather than re-deriving the two candidates' conditions, so the shadow layer is
structurally incapable of drifting from the evaluated policy without also changing the
evaluation module those functions were validated against.

Every shadow record is an annotation, not a signal: it echoes the source classifier's
`entry_state`/`entry_action`/`rule_id` read-only, never invokes the classifier, and never
computes or emits a probability, target price, or sizing quantity. Three distinct "cannot
confirm" states are kept apart rather than collapsed: `NOT_EVALUABLE` (the source classifier
itself had no technical-feature-based state that session, R0), `PARTIALLY_UNEVALUABLE`/
`UNEVALUABLE` (one or both candidate dimensions could not be confirmed -- chiefly persistence
without a retained adjacent prior session), and a genuine prior-session gap versus a caller that
never attempted the lookup at all, which carry different reason codes precisely so a future
caller cannot mistake "we don't know" for "no". PAN's real retained 2026-09-08/09/10 signals are
locked in as an exact regression fixture: the shadow layer reproduces
`NOT_PROBE_ELIGIBLE` for PAN's 2026-09-09 episode for the same reasons the counterfactual
evaluation found (momentum bucket below the upper half; the immediately prior session was not
itself R8), confirming the production wiring did not accidentally loosen anything the evaluation
tightened.

Runtime discovery was treated as a recoverable precondition rather than a separate milestone:
the runner verifies `vn_stock.db` is present and opens read-only under the canonical
`dashboard-runtime` path, optionally injects `STOCK_LOOKUP_RUNTIME_ROOT` into its own process
only, and reports a missing database as an explicit blocker string rather than raising --
though the shadow computation itself never needed to query the database further, since its
inputs are already-retained real daily tactical artifacts. Two real sessions were run
end-to-end (2026-09-09/08 and 2026-09-10/09) against genuine ~1,683-ticker retained artifacts,
not synthetic fixtures, to confirm the wiring holds at production scale.

No provider/API/network call, Daily execution, runtime/database write, classifier-policy
change, Integrated Decision or portfolio mutation, price-basis/RAW_AS_TRADED/PIT promotion,
...
(`push = NO`, started from owner checkpoint `f78bf4069abe7b0173b5aecbc350fcd270351087`).

1. **Root-caused rather than re-measured the prior milestone's unresolved corpus.** The 2026-08-31
   `MARKET_WIDE_STRUCTURED_FINANCIAL_PERIOD_SEMANTICS_V1` closeout quantified 39,568
   `UNKNOWN_DURATION` and 94,252 timestamp-missing facts but did not explain why. Live-data
   investigation (three parallel research agents plus direct verification against the real
   retained corpus at the production runtime, read-only) found: the retained corpus has grown to
   261,360 facts / 1,492 tickers on the same underlying `data_bctc` evidence (no new
   acquisition); of the now-46,594 `UNKNOWN_DURATION` facts, 44,760 are zero-silent-drop
   placeholders for (ticker, metric, period) slots with no raw observation at all -- not
   duration-ambiguous, non-existent -- while only 1,595 (VCI) and 239 (thin cash-flow depth) are
   genuine semantic-ambiguity cases. `structured_financial_period_semantics.py` now exposes
   `period_duration_root_cause`/`timestamp_root_cause` (the owner directive's A-H taxonomy) on
   every fact; verified zero uncaused facts across the full corpus by direct computation, not
   spot-check.
2. **VCI's absent duration marker is a proven source ceiling, not a mapping gap -- confirmed
   against the installed library, not this repo's own prior claim about it.** Reading
   `vnstock/explorer/vci/financial.py` (the actual installed third-party library backing
   `bctc_sync.py`, the pipeline that built the live 195,552/261,360-fact corpus -- not the
   separate, never-scaled-out `vci_financial_statement_retention.py` direct-HTTP path) confirmed
   VCI's endpoint carries no standalone-vs-YTD cumulative-basis marker at all, and that even the
   direct-HTTP path's richer `publicDate`/`createDate`/`updateDate` fields still carry no such
   marker. The prior "VCI duration remains UNKNOWN" decision (2026-08-28) is reaffirmed by fresh
   evidence, not merely repeated. No VCI-specific duration rule was added; `UNKNOWN_DURATION`
   correctly remains the terminal state for these facts.
3. **Fixed a real timestamp-representation defect distinct from the missing-timestamp count.**
   `bctc_sync.py`'s `normalize_report` stamped `scraped_at` via `vn_now().strftime("%Y-%m-%d
   %H:%M")` -- a naive Asia/Ho_Chi_Minh string with no UTC offset. This passes
   `structured_financial_period_semantics.py`'s loose `_unknown()` check (non-empty string) but
   is rejected by the separate, already-built `bitemporal_semantic_contract._parse_aware()`
   (requires `tzinfo is not None`), silently making a real, correctly-dated timestamp unusable
   to any strict bitemporal/PIT consumer. Fixed at the canonicalization boundary
   (`canonical_financial_facts._normalize_observed_at`), which reattaches the known-correct
   offset to an already-retained value -- never fabricates a new time, and raw `data_bctc`
   evidence is untouched (immutability preserved). `bctc_sync.py` now emits
   `vn_time.vn_now_iso()` directly so future syncs need no repair. `MAPPER_VERSION` bumped
   `1.3.0` -> `1.4.0`, required because `canonical_fact_store`'s incremental fingerprint keys on
   that string, not on the mapper's actual behavior -- omitting the bump would have left every
   existing shard reporting `unchanged` forever, exactly the failure mode that module's own
   changelog already documents for a prior, unrelated field addition. Measured against a full
   isolated rebuild (read-only junction to the retained runtime's Layer 1 observations, written
   only to a session-local scratch root, never to the shared runtime): 126,070 facts moved from
   unparseable to genuinely timezone-aware; the 135,135-fact missing-timestamp count is
   unchanged by design.
4. **A real regression was found and fixed before this checkpoint, not shipped.**
   `financial_flow_semantics_ttm_bridge.py` (deterministic Q-from-YTD de-cumulation and rolling
   TTM, built and unit-tested since 2026-08-28) was reachable by import from
   `market_wide_financial_analysis_v2_scaleout.py` but never invoked by any of its four real
   callers -- each would have had to hand-derive the bridge's `facts_by_ticker`/
   `entity_type_by_ticker` inputs itself, and none did. The first wiring attempt added a
   `build_qualified_flow_artifact` helper that passed `structured_financial_period_semantics`
   rows to the bridge directly; the bridge reads raw-canonical-fact field names (`value`,
   `status`, `provider`, `currency`, `scale`, `cumulative_state`) that are renamed, nested, or
   (for `cumulative_state`) entirely absent on that reshaped row, so every bridge field resolved
   to `None`, `_usable()` rejected every fact, and the bridge silently qualified nothing. Because
   `build_scaleout` substitutes the bridge's rows for the raw flow rows once
   `qualified_flow_artifact` is supplied, this regressed `current_research_ready_count` from
   1,380 to 1,276 -- caught by the pre-existing `test_engine_artifact_reproduces_regression_
   locked_figures` regression lock, not by inspection. Fixed with an explicit field adapter
   (`market_wide_financial_analysis_v2_scaleout._bridge_fact`) and a `reported_cumulative_state`
   passthrough added to the period-semantics projection (additive; no existing field changed
   meaning). The regression test now passes at exactly 1,380. Wired into the live Daily path
   (`canonical_daily_financial_v2_materialization.build_engine_artifact`) and the three replay
   tools that share its exact `build_scaleout` call pattern.
5. **The now-correctly-wired bridge has zero measured market-wide impact on this corpus --
   reported as such, not inflated.** `qualified_flow_replaced_raw_flow_rows=True` confirms the
   wiring is live, but `revenue_ttm`/`net_income_ttm`/etc. READY-or-proxy counts are identical
   with and without the bridge (93/123/etc.), and the bridge's own `ttm_ytd_bridge_by_metric`
   coverage is genuinely empty. Root cause, verified not assumed: KBS income-statement facts
   already arrive as direct standalone quarters (nothing to de-cumulate), and VCI's duration is
   structurally unknown (decision 2 above), so no retained fact currently has a YTD-cumulative
   basis for the bridge's subtraction math to operate on. `PARTIAL_BY_EVIDENCE`, not a defect:
   the wiring is architecturally correct and will contribute automatically, with no further code
   change, the moment any ticker retains >=2 consecutive same-year YTD income-statement facts.
6. **New `canonical_financial_analytical_panel.py` consolidates rather than duplicates the
   requested canonical boundary.** Surveyed `multi_period_financial_panel.py` (the closest named
   prior analog, "Phase 2 / P2-CLOSEOUT") in full: it operates over a citation-based, ~12-ticker
   hand-curated evidence set (`load_all_authoritative_citations`), uses an incompatible `scale`
   representation (numeric multiplier vs. this pipeline's string enum), has no `fact_id`/
   `cumulative_state` concept, defaults `statement_scope` to `consolidated` by presence rather
   than deriving it from evidence, and has zero live consumers outside its own closed P2/P3c-e
   milestone loop. Building the requested panel on it would have both inherited a weaker
   evidence model and left the real, market-wide 1,492-ticker corpus unaddressed. The new module
   instead joins three already-governed layers deterministically:
   `structured_financial_period_semantics.py` (now with root causes),
   `bitemporal_semantic_contract.py` (valid-time + knowledge-availability -- previously wired
   only into the unrelated `asof_query_ledger.py`/`temporal_retention.py`, never the financial-
   fact path), and the TTM bridge's derived rows (explicit `derived_from` lineage, never merged
   into an observed fact). It cross-references `feature_input_fitness_contract.py` by family
   name for "which uses are allowed" rather than re-deriving fitness verdicts. Explicitly not a
   new authority tier: `authority_boundary.pit_or_raw_as_traded_promoted=False`,
   `official_or_owner_promotion=False`.
7. **Currency/scale/statement-scope were audited and confirmed, not silently left unaddressed.**
   `canonical_financial_resolvers.resolve_currency_and_scale`'s only implemented authority is
   official-citation digit-agreement, existing for HPG/VNM only (confirmed by reading the module
   in full) -- this is a genuine source-evidence ceiling for the market-wide VCI/KBS lineage, not
   an unresolved propagation gap, and was left exactly as blocked as before. `resolve_statement_
   scope` can only ever resolve `consolidated` (non-zero minority interest) or `UNKNOWN`;
   `separate` is structurally unreachable from these payloads by the resolver's own design. No
   inference from entity type, magnitude, or issuer nationality was added anywhere.
8. **Advanced the pinned Financial V2 input authority to the new snapshot, per its own governance
   rule -- never edited a resolved identity in place.**
   `financial_v2_current_input_authority.AUTHORITY_VERSION` advanced `2026-09-02.1` ->
   `2026-09-05.1`; `_SEMANTICS_DIRNAME`/`_EXPECTED_SEMANTICS_IDENTITY` now point at a new,
   force-tracked `operations-review/market-wide-structured-financial-period-semantics-v1-
   20260905/` directory (rebuilt from the same retained raw evidence, read-only, no network).
   The original `-20260831` directory is preserved untouched as historical evidence. The
   feature-store and classification-diagnostics pins are unaffected by this milestone and remain
   resolved against their original `-20260831`/`-20260901` directories.
9. **Retained 2026-08-25 temporal replay: zero future leakage, verified by explicit assertion
   not by omission.** 155 of 261,360 facts are the genuine 2026-09-01-dated HPG parallel-
   retention pilot sample, correctly rejected by the existing post-target gate for a 2026-08-25
   replay -- confirmed by direct inspection of their timestamps/ticker/status, not assumed to be
   safe. `current_research_ready_count` is unchanged at 1,380/1,492: this milestone improves
   evidence quality and lineage, not eligibility, exactly as authorized. No provider, financial
   formula, valuation conclusion, target price, probability, authority promotion, database
   write, production DB write, or remote/push operation occurred. 266 focused/adjacent tests
   pass (0 new failures); `py_compile`/`git diff --check`/`tools/stocklookup_roadmap.py --check`
   clean. Evidence: `operations-review/financial-temporal-semantic-normalization-and-analytical-
   panel-v1-20260905/`. No successor is queued.
...
at `a032c49`, implementation checkpoint `ec86754`.

1. **A self-recorded `COMPLETE` milestone is not the same as owner acceptance for push.** The
   local candidate `TACTICAL_MOMENTUM_PARTICIPATION_CONFIRMATION_V1` (`6f08c17`/`a032c49`) had
   already recorded itself `COMPLETE` in `docs/ROADMAP_STATE.json`/`docs/STATE.md` via its own
   commits. This milestone's directive explicitly treated that self-report as WIP-under-acceptance
   pending an independent review under new feature-fitness rules -- not a reinterpretation of
   `STATE.md`'s cached truth (which correctly recorded what that milestone's own commits claimed),
   but an exercise of `AI_RULES.md` rule 4's standing requirement that `READY_FOR_NEXT_MILESTONE`
   (or a self-reported `COMPLETE`) never itself authorizes push. Disposition: `ACCEPT_AS_IS`,
   recorded independently in
   `operations-review/.../tactical_momentum_candidate_acceptance.json`.

2. **The Daily execution graph was verified against code, not assumed from documentation.** Traced
   `stocklookup.ps1 -> stocklookup.py -> daily_analysis_pipeline.py --canonical-post-close ->
   canonical_daily_operation.py -> canonical_post_close_pipeline.py -> daily_session_level2_
   package.py -> daily_producer_pipeline.py -> ai_handoff_publication.py -> dashboard_release_
   publisher.py` stage by stage. Confirmed canonical Daily never touches `vn_stock.db` or calls
   `vn_stock_pipeline.py`'s `update`/`backfill` (those live only in the `LEGACY_NON_CANONICAL`
   branch of `daily_analysis_pipeline.py`, unreachable because `stocklookup.py` always passes
   `--canonical-post-close`). Catalogued 8 independent idempotency guards explaining warm-rerun
   speed. Full graph: `operations-review/.../daily_execution_graph.json`.

3. **A real, previously-undiscovered defect was found by generalizing an already-fixed pattern,
   not by inventing a new check.** `technical_structure_context.py`'s prior corrective (checkpoint
   `9ab5a55`) proved that a DNSE technical-history recovery series must only be adopted when its
   own target-session close agrees exactly with the multi-source-resolved snapshot's close, and
   extracted this into a shared `resolve_target_session_observations()` function correctly reused
   by `tactical_momentum_context.py` and `market_wide_relative_volume_research.py`. Sweeping for
   "any other consumer that blindly accepts a recovered multi-session series" (this milestone's
   own Section 8 mandate) found a fourth, *earlier* consumer -- `market_wide_current_descriptive_
   research.py`, whose `technical_features.values`/`trend_state` feed breadth/screening -- that
   adopted the recovery on state alone with no such check. Fixed by reusing the identical shared
   function (not a new copy); a rejected ticker is tracked and exempted from the module's own
   `RECOVERABLE_SAME_SESSION_TECHNICAL_HISTORY_GAP` assertion for the same reason a structurally-
   insufficient recovery already is. `technical_structure_context.py`/`tactical_momentum_
   context.py` were not contaminated in practice (they use `technical_features` only as an
   eligibility gate and re-resolve their own close series independently) -- the real exposure was
   limited to descriptive research's own directly-exposed fields. One pre-existing test's fixture
   had accidentally relied on two disagreeing close series (undetected before this guard existed);
   fixed to agree, plus one new dedicated rejection-path test. 255 tests across every direct/
   indirect consumer pass with zero new failures. Commit `ec86754`.

4. **Feature-specific input fitness is a consolidation registry over existing authorities, not a
   new one.** `feature_input_fitness_contract.py` names the 19 owner-specified use-case families
   and, for each, records the exact existing module/function this repository already uses to
   answer its fitness question (`multi_source_market_evidence_contract.resolve_ticker`,
   `technical_structure_context.resolve_target_session_observations`, `monetary_basis_contract.
   compatible`, `financial_entity_applicability.metric_applicability`) -- deliberately never
   re-implementing any of them, per `AI_RULES.md`'s "consumers pass through Producer verdicts,
   never widen them" and this repository's own "edit over proliferate" convention. Thin
   `evaluate_*` wrappers are tested byte-identical to the authority they delegate to; a separate
   test asserts every registry pointer actually resolves in the real codebase, so the catalog
   cannot silently drift from what it describes. This closes the concrete confusion Section 9 of
   the directive named: a `CURRENT_SESSION_PRICE` resolution from a non-DNSE source can never be
   mistaken for `RELATIVE_VOLUME` fitness -- the two are governed by structurally different
   authorities, now named explicitly rather than left to be rediscovered per consumer.

5. **A genuine local-only Daily mode required fixing two different publish functions in two
   different ways, not one shared flag.** `ai_handoff_publication.publish()`'s existing `push`
   parameter only ever gated the final `git push` -- `mkdir`/copy/two `git add`/`git commit` calls
   ran unconditionally, so `--replay-local`'s `push=False` still left two real local commits.
   `local_only=True` (new) now returns before any of that runs. `dashboard_release_publisher.
   publish_dashboard_release()`'s `push` parameter already correctly gated its own Git calls, but
   its local artifact writes into `web_root` (the real `market-dashboard` working tree) ran
   unconditionally regardless of `push` -- so `local_only=True` (new) instead redirects every write
   to a throwaway staging directory, seeded only with the one pre-existing file the function would
   otherwise read (`data/build_info.json`), so the real checkout is never touched at all, not even
   left dirty. Both `local_only` parameters always take precedence over `push` -- including
   `push`'s own default of `True` in `ai_handoff_publication` -- so a caller never needs to
   remember to also pass `push=False`; a first attempt at a mutual-exclusion `raise` was reverted
   after discovering it made `local_only=True` alone (the natural call) trip on `push`'s own
   default. `stocklookup.py --local-only` wires both and prints `REMOTE_PUBLICATION=
   SKIPPED_LOCAL_MODE` plus a `LOCAL_AI_ARTIFACTS` block reusing the existing `ai_research_bundle_
   manifest.json`/`build_package()` identity, not a second local-pointer format. 8 new tests each
   diff a real Git repository's `HEAD`/status/file-listing before and after the call -- proving
   zero mutation structurally, not merely asserting it in prose -- and prove `local_only` still
   performs genuine validation (identical package/release identity to a real publish). Normal
   (no-flag) Daily publication is unchanged; 40/40 tests across both publish modules pass.

6. **Provider routing for 2026-09-04 was reported from the literal retained artifact, not
   reconstructed from memory.** Extracted `operations-review/p3f9b-market-wide-exact-session-
   scaleout-20260904/multi_source_exact_session_market_evidence.json` directly: DNSE 0/1683 exact
   at the 15:31:51 ICT acquisition instant, KBS 1506 attempted/957 recovered, VCI 2 attempted/1
   recovered (both inside the small quality sentinel -- confirming VCI never fires on a clean KBS
   miss), final 958/1683 resolved, rate governor holding its 45-effective/60-hard rpm ceiling
   throughout (`max_observed_window_utilization=45`, never breached). Every figure matches this
   file's own previously-documented 2026-09-04 narrative exactly, now traced to its literal source.
   No new live acquisition was performed to build this report, per the directive's own instruction.

7. **A real residual risk was found and flagged, not silently absorbed into "no bypass found."**
   `vnstock_rate_governor`'s single chokepoint (`vn_stock_pipeline._bounded_send_request_direct`)
   genuinely covers every VCI/KBS call inside canonical Daily's own execution graph -- but roughly
   15 standalone maintenance/sync scripts (`bctc_sync.py`, `meta_sync.py`, `company_profile_
   sync.py`, and others) call `vnstock` directly in separate processes without ever installing that
   monkeypatch. These scripts are outside canonical Daily's own execution graph and therefore out
   of this milestone's scope to fix, but a script run concurrently with Daily could still
   contribute to the same account-wide `vnai` 60-rpm ceiling outside the governor's own
   accounting. Recorded as a residual limitation rather than silently omitted.

8. **`TECHNICAL_HISTORY_BACKUP = NOT_QUALIFIED` for KBS/VCI was recorded as an explicit, standing
   fact, not left implicit.** The extended-lookback technical-history recovery is DNSE-only by
   design; when DNSE cannot supply a complete 20-session window, the recovery record's own state
   (`INSUFFICIENT_HISTORY_AFTER_EXTENDED_LOOKBACK`) is explicit and dependent features correctly
   report `INSUFFICIENT_HISTORY` -- KBS/VCI historical OHLC is never substituted. No new provider
   was added or use-case-qualified; this is a documentation decision, not a code change.
...
1. **`_previous()` gained exactly one filter, not a rewrite.** The existing candidate loop (glob retained manifests, keep those strictly before the current session with a session-matching bundle) was left completely intact; the only change is ANDing in `frozen_input_identities(registry, prior) is not None` before a candidate is accepted. This reuses the exact same public qualification check `next_session_decision_brief._require_qualified` already calls on its own `current_session`/`previous_session` arguments -- no new qualification concept, no duplicated logic, and the fix is provably backward compatible for every session that WAS already qualified (the filter only ever removes candidates, never adds one that existence-based selection wouldn't already have found).

2. **The fix was validated against real data without running Daily.** Rather than trust the synthetic unit tests alone, `stocklookup._previous` was called directly (read-only) against the real primary-checkout registry for both the exact real scenario (`current="2026-09-03"`) and the test suite's own claimed scenario (`current="2026-08-28"`). This confirmed, byte-for-byte, that the OLD code's selection for `2026-09-03` was `2026-08-28` -- matching the morning's real `SESSION_NOT_GOVERNED_QUALIFIED:2026-08-28` symptom exactly -- and that the NEW code correctly walks back to `2026-08-26` (the actual latest governed-qualified session; neither 2026-08-27 nor 2026-08-28 carry `COMPLETED_RETAINED_EVIDENCE` in the current real registry). This is stronger evidence than any synthetic fixture could provide and cost zero live acquisition.

3. **`tests/test_next_session_decision_brief.py`'s own real-data suite is now broken by an unrelated, pre-existing cause, and was deliberately left alone.** That file hardcodes `ROOT` to the primary checkout's absolute path and asserts `LATEST_COMPLETED_RUN.json` still points at `2026-08-28` as a fixture precondition -- true when the file was written, false today because the owner's own real 2026-09-03 run advanced that pointer. This breaks the test at its very first line, before `_previous` is ever reached, and reproduces identically on the untouched primary checkout with or without this milestone's changes. Updating that test's hardcoded session pair was out of scope (not one of the two named defects) and was not attempted; the gap is documented instead.

4. **DNSE quarantine is a new, separate function (`resolve_ticker_degraded_dnse`), not a parameterization of `resolve_ticker`.** An earlier design considered adding a `quarantined_sources` parameter to the existing `resolve_ticker`, filtering DNSE out of its `observed` list before running the same tie-break logic. This was rejected: `resolve_ticker`'s tie-break (`preferred = ordered[0]`) is only ever safe when the remaining candidates AGREE; once DNSE is excluded, "VCI and KBS disagree with each other" becomes reachable in a way the existing tie-break would silently resolve to VCI by preference order -- exactly the "unjustified resolution" the owner's rule 3 (SOURCE_CONFLICT, unresolved) forbids. Writing an independent, exhaustive four-branch function keyed entirely on VCI/KBS (never consulting DNSE's value at all, not even to check agreement) is easier to audit against the owner's four literal rules and carries zero risk of perturbing `resolve_ticker`'s own already-tested, deliberately-untouched tie-break semantics for every non-degraded caller.

5. **`_project_to_p3f9_shape`'s dispatch was widened from a resolution-label check to a value check (`resolved_source is None`), which is provably a no-op for every existing caller.** For the ORIGINAL `resolve_ticker`, `resolved_source` is `None` in exactly one branch (`RESOLUTION_ALL_MISSING`) and a real source string in every other branch -- so `resolution == RESOLUTION_ALL_MISSING` and `resolved_source is None` were already equivalent conditions for every pre-existing caller, and widening the check changes nothing for them. It only newly activates for `resolve_ticker_degraded_dnse`'s `SOURCE_CONFLICT` branch, which deliberately also returns `resolved_source=None` to represent "no justified resolution" -- reusing the SAME "leave/downgrade the record, never fabricate a value" code path that already existed for the missing case, rather than adding a third branch.

6. **A quarantined ticker's projected disposition is downgraded to `SESSION_MISSING` with its target-session row stripped, never left as DNSE's own (still-EXACT_SESSION_RETAINED) record.** The pre-existing "ALL_MISSING leaves the record unchanged" shortcut is only correct when DNSE's own record was ALREADY non-retained; for a ticker DNSE originally claimed but is now quarantined for, "unchanged" would silently keep counting it toward `exact_session_observed_count` -- exactly the bug this corrective closes. Older lookback-window rows (if any) are preserved; only the now-untrusted target-session row is removed, so the 20-session completeness calculation elsewhere in the same function isn't corrupted by a row that no longer represents a trusted observation.

## 2026-09-04 - Multi-Source Daily Degraded-Provider Autorecovery And Idempotency Corrective V1

`MULTI_SOURCE_DAILY_DEGRADED_PROVIDER_AUTORECOVERY_AND_IDEMPOTENCY_CORRECTIVE_V1 = COMPLETE` (push = YES). Owner-directed bounded corrective closing two P0 defects terminal acceptance of the prior day's multi-source milestone exposed.

1. **Auto-recovery is a new orchestration layer on top of the existing resolver, not a rewrite of it.** `resolve_multi_source_exact_session_snapshot()` and `assert_dnse_quality_acceptable()` are byte-for-byte unchanged, including their existing 46+17+5-test coverage; the standalone `tools/run_multi_source_exact_session_resolver.py` diagnostic still gets the pre-corrective "report degraded, do not auto-expand" behavior verbatim. The new `resolve_exact_session_with_autorecovery()` calls the SAME function once or twice depending on the sentinel verdict, using a shared memoizing fetch cache so a second call over an overlapping candidate set never re-hits the network for a pair the first call already resolved. This was chosen over modifying the core resolver in place specifically to avoid touching its already-tested per-call contract and to keep the diagnostic tool's own documented "operator must explicitly expand scope" behavior intact for callers that still want it.

2. **The memoizing cache must own pacing, not just fetch deduplication.** An early design that simply re-called the resolver with `sentinel_cohort` expanded to the full DNSE-exact set, passing the real `sleep_fn`/`request_delay` through unchanged to both calls, would have been correct but catastrophically slow: Pass 3/4's gap-recovery loop sleeps after every ticker regardless of whether the fetch was a cache hit, so a second call over the identical DNSE-missing set (unavoidable, since Pass 3/4's own candidate set never depends on `sentinel_cohort`) would re-burn the entire original pacing budget -- thousands of seconds on a real ~1683-candidate day -- for zero new network activity. Fixed by having `_memoizing_fetch` perform the real sleep itself, exactly once per genuine cache miss, and passing a no-op `sleep_fn` to both underlying resolver calls. Verified by a dedicated test asserting sleep count, not just fetch count.

3. **The idempotency-escape fix is two cooperating gates, not one.** Simply moving the canonical-snapshot write to happen strictly after a sufficiency decision would not, by itself, protect an *already-existing* on-disk artifact written by pre-corrective code (or any future bug) -- reuse paths would still trust it on file-presence alone. Instead: (a) `ensure_exact_session_snapshot()` no longer runs `assert_dnse_quality_acceptable()` in the fresh-acquisition path at all (the auto-recovery wrapper's own completion IS the decision now, with final sufficiency left entirely to the caller's unchanged coverage-ratio gate), and (b) every path that could reuse an *existing* snapshot (`ensure_exact_session_snapshot`'s own idempotent-if-exists check, and `canonical_post_close_pipeline.assert_post_close_eligible`'s reuse contract) now independently re-derives trust from the sibling evidence artifact's retained sentinel verdict cross-checked against a self-declared `degraded_provider_recovery` marker on the snapshot itself. Both checks default to "trust" when no companion evidence exists (never a positive claim of untrustworthiness from mere absence), which is what keeps every pre-existing test/fixture in this codebase passing unmodified.

4. **`assert_post_close_eligible` gained a 6th point rather than a new function, and `PreCutoffArtifactError` was reused rather than introducing a second exception class.** That function and exception already generalize "this existing artifact fails the reuse contract" across five unrelated reasons (session identity, lineage hash, contract version, scope, unattempted candidates, coverage ratio) -- a provider-health failure is the same shape of fact (existing artifact, not reusable, redirect to a fresh attempt), and reusing the class means `resolve_acquisition_root`'s existing fresh-attempt-directory redirect mechanism needed zero changes to also cover this new trigger. `artifact_root` was added as an optional keyword (default `None`, skips the new check) specifically so no pre-existing call site or test needed to change.

5. **The real 2026-09-03 evidence validates the design without a new live probe, and revealed a favorable coincidence, not a general guarantee.** `DEFAULT_DNSE_EXACT_SAMPLE_SIZE=18` happened to equal the true DNSE-exact count that day, so the existing small sentinel cohort already queried VCI/KBS for 100% of the affected universe -- the degraded-expansion pass finds nothing left to do and the corrected policy certifies the day using only already-retained evidence. This is validated by a synthetic replay test built from the real retained numbers, plus a SEPARATE general-case test (partial sentinel/DNSE-exact overlap) proving the expansion pass genuinely queries the delta when the coincidence does not hold -- the real-evidence test alone would not have caught a bug in the general expansion path, since it never exercises it.

6. **`resolve_ticker`'s own per-ticker policy (contract module) was deliberately left untouched, including the BMP/TNI-shaped case where VCI and KBS disagree with each other and DNSE's flagged-but-unresolved value is retained as the projected row.** That is a pre-existing, already-documented, already-tested design choice from the prior milestone, not one of this corrective's two named P0 defects (`DEGRADED DNSE STILL REQUIRES OPERATOR INTERVENTION`, `FAILED SENTINEL ARTIFACT CAN BYPASS GATE ON RERUN`). Reopening it would have been scope creep into "broad provider qualification," which this milestone's own brief explicitly forbids.

## 2026-09-03 - Multi-Source Exact-Session Market Evidence And Daily Resilience V1

`MULTI_SOURCE_EXACT_SESSION_MARKET_EVIDENCE_AND_DAILY_RESILIENCE_V1 = COMPLETE` (push = YES). Owner-directed product-critical architecture milestone, following directly from the same-day DNSE 17/1683 investigation.

1. **Bounded qualification came back decisively positive for both candidate sources, and immediately surfaced a real DNSE price-basis risk that turned out to be the actual headline finding, not a footnote.** 18/18 live probe requests (9 representative tickers x VCI/KBS, including HPG/VCB/FPT/MBB/SSI/MWG named in the task and DNSE-SESSION_MISSING names) returned clean HTTP 200 with the exact 2026-09-03 bar present. But two of the 17 tickers DNSE itself DID resolve that day (MWG, GMD) showed open/high identical to VCI/KBS while low/close diverged by more than one HOSE order-price tick (MWG: DNSE close 74.5 vs VCI/KBS 73.2, native "thousand VND" scale; GMD: 79.2 vs 77.6), and volume differed ~30-44x (DNSE far lower). A full sentinel audit (decision 3 below) later confirmed this was not two isolated names -- it was DNSE's entire resolved set for the day.

2. **The resolved snapshot reuses `p3f9_exact_session_mva_snapshot/v2`'s exact contract_version string rather than minting a new one.** A direct audit of every consumer of this artifact (breadth foundation, universe status, liquidity research, technical recovery, descriptive research, valuation, tactical classifier, sector leadership, `assert_post_close_eligible`) found exactly one place (`assert_post_close_eligible`'s own equality check) that inspects the string at all. Reusing the literal string makes the projection a true drop-in -- zero downstream call sites changed -- while per-record honesty is preserved exactly where every consumer already looks: each observation row's own `provider` field, plus additive top-level/per-record fields nothing existing reads or is broken by.

3. **A DNSE quality sentinel was added after the qualification-phase MWG/GMD finding exposed a real structural gap: session-date equality is coverage, not correctness.** The original four-pass design (DNSE full-universe, then VCI/KBS for DNSE's own gaps only) never re-checks a ticker DNSE already claims to have resolved -- meaning a systematically stale-but-same-dated DNSE feed would be silently accepted forever, with zero corroboration, for exactly the tickers a naive reader would trust most (DNSE "succeeded"). Added Pass 5: `select_sentinel_cohort()` (owner watchlist-11 + top-5 governed-liquid + top-3-per-exchange + up to 18 of DNSE's own exact tickers, deterministic and versioned, no random sampling) queries VCI/KBS for sentinel members even when DNSE already resolved them. `classify_dnse_provider_health()` reduces the result to one of four states with a minimum-sample-size gate (>=5 assessable) so isolated conflicts can never trip the systemic classification alone. **Real validation, run twice independently against the same retained 2026-09-03 DNSE snapshot with identical results: 18/18 of DNSE's own exact-session tickers conflict with a corroborated VCI==KBS pair (0/18 agree) -> `DNSE_BROAD_STALE_OR_INCOMPLETE_EOD`.** This is strictly stronger evidence than the qualification-phase MWG/GMD sample suggested: not "DNSE is thin," but "DNSE's own claimed coverage cannot be trusted at all for this session." `assert_dnse_quality_acceptable()` is called only after both evidence artifacts are persisted (never before -- discarding real, hard-won live-call results by raising too early was explicitly rejected), and canonical Daily's real wiring lets the resulting `DnseProviderWideQualityDegraded` propagate uncaught, so 2026-09-03 is correctly left uncertified rather than shipped as a plausible 45.87%-coverage day silently containing 18 wrong bars. This is genuinely additional scope beyond the milestone brief's literal text (which asked for coverage recovery, not DNSE self-consistency auditing), justified by what the qualification phase actually found; it never blocks a healthy DNSE day (`sentinel_cohort=None` for any caller that doesn't opt in preserves pre-sentinel behavior exactly, and a healthy day's sentinel cohort costs on the order of dozens, not thousands, of extra bounded requests).

4. **A real arithmetic bug in the cross-source price comparison was found and fixed before it was ever exercised by a live validation.** The original `_price_fields_agree()` compared each observation's `normalized` field, but `NATIVE_PRICE_UNIT_SCALE` deliberately leaves DNSE at its raw undocumented scale (`normalized == native`, ~74) while VCI/KBS are scaled to literal VND (~74000) -- a 1000x mismatch that would have shown *every* DNSE-vs-VCI/KBS pair as wildly conflicting by construction, regardless of whether the real prices agreed. This never manifested in the milestone's own first test suite because every test either used same-scale pairs (VCI vs KBS) or a pair that was *already* supposed to be a genuine conflict (the real MWG divergence), so the bug produced the "correct" answer for the wrong reason. Fixed by comparing `native` values instead (empirically confirmed same real-world magnitude for a given bar in this milestone's own qualification data), with `hose_tick_size`'s VND-denominated tolerance scaled back down to native units for the comparison -- mathematically identical to the old check whenever both sides already share one scale (VCI vs KBS), and changes behavior only for the DNSE-vs-VCI/KBS case this fix targets. Also caught: the real live 2026-09-03 sentinel run (decision 3) would have been meaningless without this fix, since every DNSE-vs-corroborated-pair comparison would have shown "conflict" unconditionally.

5. **VCI/KBS recovery (both Pass 3/4 gap-fill and Pass 5 sentinel) stays strictly sequential, paced identically to `vn_stock_pipeline.py`'s own existing `update`/`backfill` commands -- no concurrency was added.** Audited: zero evidence anywhere in this repository (source, tests, or `docs/DECISIONS.md`'s own `MARKET_WIDE_ENRICHMENT_AND_CANONICALIZATION_V1 = PAUSED_RATE_LIMIT_CONSTRAINED` record) that VCI/KBS tolerate concurrent access. `vn_stock_pipeline.fetch_one()`'s inner per-source retry/classification logic was extracted into a new `fetch_single_source()` (behavior-preserving refactor, verified byte-identical against the existing 46-test/14-subtest suite before any other change), reused directly by the resolver.

6. **The 20% `MIN_EXACT_SESSION_COVERAGE_RATIO` gate and its denominator (`attempted_candidate_count`, the full raw canonical candidate list) are both unchanged; the sentinel is a separate, additional gate, not a replacement.** `current_universe_status_and_session_coverage_resolution.py` already computes a narrower "current active equity" denominator, but it is structurally downstream of the coverage gate (it consumes the very P3F9B snapshot the gate is validating) -- using it as the gate's own pre-check denominator would be circular. The coverage gate answers "is there enough data"; the sentinel separately answers "is the data DNSE claims to have actually correct" -- 2026-09-03 shows these are genuinely different questions with genuinely different answers (coverage recoverable to 45.87% via Pass 2-4; DNSE's own claimed 18/1683 slice simultaneously 0% trustworthy per the sentinel).
## 2026-09-03 - Canonical Daily Financial V2 And Current Research Enrichment V1

`CANONICAL_DAILY_FINANCIAL_V2_AND_CURRENT_RESEARCH_ENRICHMENT_V1 = COMPLETE / COHERENT_PARTIAL_BY_RETAINED_EVIDENCE` (`push = YES`, `f3880c589d345ff20e67fd2df84acdaf57f4681e`).

1. **Closes `DAILY_INTEGRATED_DECISION_BRIEF_AND_PROSPECTIVE_FEEDBACK_V1`'s decision 5.** That milestone's `docs/DECISIONS.md` entry named the two options for fixing Financial V2's missing daily-materialization path: "building the full Financial V2 engine chain ... fresh inside the canonical pipeline every session, or establishing a new 'latest Financial V2 snapshot' canonical-path convention." This milestone does both together: `canonical_daily_financial_v2_materialization.py` builds the engine chain fresh each session (first option), sourced through `financial_v2_current_input_authority.py`'s pinned, versioned canonical-path convention (second option) rather than a filesystem scan. Combining them avoids the two failure modes either option alone would risk: a pure "build fresh every time" approach with no pinned authority would have re-invited exactly the "which retained snapshot is canonical" ambiguity this milestone's own resolver exists to remove; a pure "latest snapshot pointer" approach with no live entity-classification layering would have reproduced the stale-diagnostics-snapshot trap `CORE_FUNDAMENTAL_VALUATION_AND_PEER_CONTEXT_V1` already found and fixed once.
2. **The resolver is deliberately a pinned manifest, not a "latest folder" scanner.** The task's own instruction ("No filesystem 'pick newest folder' behavior") and this codebase's repeated pattern of dated evidence directories accumulating without a "current" pointer (confirmed directly: `operations-review/market-wide-current-valuation-v1-*` alone has at least four differently-dated variants) made an mtime/glob-based resolver an attractive but explicitly rejected shortcut. `financial_v2_current_input_authority.py` instead hardcodes the three exact retained directory names plus their expected artifact identities (verified at load time, `FINANCIAL_V2_INPUT_AUTHORITY_IDENTITY_DRIFT` fails closed on drift). Advancing to newer retained evidence requires a deliberate code change bumping `AUTHORITY_VERSION`, never happens automatically, and is intentionally the only way this resolver's answer can change.
3. **A second real production defect was found and fixed in the same closure, not deferred.** The task instruction explicitly required empirically verifying the valuation wiring rather than assuming it was correct ("Determine whether canonical_post_close is currently feeding: raw valuation artifact OR the correctly evaluated: current_research_valuation_context ... Do not assume the current wiring is correct. Verify empirically."). It was not correct: `current_valuation_artifact` was the raw `market_wide_current_valuation_input_scaleout` artifact (price/EPS/BVPS-shaped, no `methods`/`peer_relative_context` keys at all), never evaluated through `current_research_valuation_context.evaluate_ticker_valuation()`/`attach_peer_relative()` before being handed to `integrated_investment_decision_product.evaluate_valuation_context()`. Confirmed by direct comparison against `tools/run_integrated_investment_decision_replay.py`'s own `load_inputs()`, which already does this evaluation step correctly and was the reference this milestone's `build_evaluated_valuation_artifact()` was built to match. Fixed in the same commit as the fundamental-artifact fix since both live in the identical closure and share the identical new engine artifact as an input (avoiding a second ~20-second engine rebuild).
4. **The contract-shape assertion is deliberately lenient on absence, strict on presence.** `financial_analysis_product_projection.py` already has a strict `validate_product_context()` (requires an exact re-derived `artifact_sha256` match), but adopting it wholesale inside `integrated_investment_decision_product.build_artifact()` would have broken every existing lightweight test fixture across this codebase that constructs a minimal `{"artifact_identity": ..., "records": {...}}` dict without a full, correctly-hashed identity (confirmed by running the existing test suite against that stricter check before choosing this design). The narrower assertion added instead -- reject only a *present* `contract_version` that does not equal the compact contract, permit an *absent* one -- still catches the exact shape of both real historical bugs (the legacy artifact and the raw engine artifact both stamp their own real `contract_version`), while remaining a pure additive change to every existing caller.
5. **`export_ai_bundle.py`'s pre-existing opt-in contract for the financial-analysis-v2 attach was preserved exactly, not replaced with the integrated-decision-product attach's contract.** The two functions look similar but have genuinely different pre-existing behavior when `include=False` and an `artifact_path` is also supplied: `attach_integrated_investment_decision_product` always honors an explicit path regardless of `include`; `attach_financial_analysis_v2_product_context`'s existing test (`test_export_opt_in_is_off_without_io_and_on_requires_valid_explicit_context`) requires `include=False` to mean *no I/O at all*, ignoring `artifact_path` entirely. A first attempt at this change copied the former pattern and broke that existing test; the final implementation keeps `include` as the sole gate for the explicit-path branch and only adds `root`/`reference_session_date` as a *new*, independent auto-resolve branch that activates when `include` is False -- exactly reproducing the original contract for every existing caller while adding the new capability.
6. **Previous-session semantics (`previous_qualified_session`) needed no code change, only an audit.** Read `stocklookup.py:_previous()` and `next_session_decision_brief.py`'s `_require_qualified()` directly rather than assuming the immediately-preceding milestone's cited `2026-08-25` figure still held. It does not, and correctly does not: real retained evidence shows `2026-08-26` now has both a self-consistent `ai_research_session_bundle.json` and a `COMPLETED_RETAINED_EVIDENCE` registry entry, so `_previous()` resolves to it today for any later session. `2026-08-27`/`2026-08-28` have real bundles from earlier ad-hoc/replay-tool sessions but are not yet registry-complete, so they correctly do not qualify. The two authorities (bundle existence, registry completion) are structurally guaranteed to agree for any session produced through the real governed `run_canonical_post_close()` sequence, since registration and bundle creation happen in that same run -- no fix was needed, only the finding, reported per the task's explicit instruction rather than silently accepted.
...
1. **A version-bump omission, not a data gap.** `current_assets`/`current_liabilities`/finance-lease candidates were added to `canonical_financial_facts.METRIC_REGISTRY` by the depth-recovery milestone, but `MAPPER_VERSION` stayed `1.0.0`, so `canonical_fact_store`'s own incremental-invalidation contract (which the module's docstring already warns about) never fired: the 2026-08-10 persisted store kept reporting every shard `unchanged`. Bumped to `1.1.0`; every shard rebuilds; canonical facts grow 195,552 -> 249,872, all `POINT_IN_TIME_BALANCE_SHEET`. The raw VCI observations and the mapping were both already correct -- verified directly against `HPG_balance_sheet_quarter.csv` and the retained layer-1 shard before touching any code.
2. **A stale artifact does not get to keep contradicting itself.** `structured_financial_depth_recovery.py`'s hardcoded `capability_matrix` still said `SOURCE_EXPOSES_NOT_RETAINED` for the same three metrics its own per-ticker `missing_components` logic had already stopped reporting as missing -- a self-contradiction inside one artifact. Fixed to `RETAINED_AND_CANONICALIZED`. Its finance-lease missing-entry was also unconditional for every VCI ticker (unlike the working-capital check beside it), which is now individually correct but was about to become **wrong** the moment the fix above the fix above made Layer 3 start emitting `unavailable`-status rows for these metrics on every ticker -- so mere presence in `existing_metrics` stopped proving retention. Both checks now key off a new `usable_metrics` set instead.
3. **Reuse the ratio/direction primitives; add only what genuinely differs.** `net_working_capital`/`current_ratio` and their YoY directions are built entirely from the engine's existing `_same_period_pair`, `_ratio`, and `_pit_ratio_direction_for` (the last generalized to accept a `method` string, its one existing call site behaviorally unchanged). The one new primitive, a plain point-in-time value delta (`_difference`/`_pit_component_direction`), exists because net working capital can be negative and cross zero between periods, where a percentage-of-prior formula is not meaningful; a ratio-of-ratios delta already covers `current_ratio`'s trajectory correctly, so it needed no new helper.
4. **`current_ratio` is a legitimate `current_research_ready` contributor, but it changes nothing today.** It is a same-provider point-in-time balance-sheet ratio, structurally identical in kind to `equity_to_assets`/`cash_to_assets`, which already count. Added to `readiness_features`. Real replay shows `current_research_ready` unchanged at exactly 1,380/1,492 before and after, because every ticker that gains a READY `current_ratio` already had another READY ratio from the same balance-sheet payload -- reported honestly rather than assumed either way.
5. **No new investment-score surface.** `working_capital_state`, `working_capital_trajectory_state`, and `current_ratio_trajectory_state` are descriptive identity states only; `POSITIVE_NET_WORKING_CAPITAL` is never mapped to a health verdict. Cross-family debt/working-capital pairings (e.g., leverage worsening alongside working capital worsening) are appended to the existing free-text `positive_evidence`/`negative_evidence`/`conflicting_evidence` lists exactly like every other thesis-context tag already there -- not a new score, gate, or formula.
6. **Regression proof used the real cohort, not a smaller stand-in.** The real legacy-523 engine artifact and the real entity-classification diagnostics both still exist in sibling milestone worktrees on this workspace (not this repo's own git history); using them instead of a partial in-repo reproduction is what let this milestone reproduce the exact 1,382/85/25 entity-family split and the exact frozen six-label security-stance distribution (183/344/30/52/188/902) byte-for-byte, before and after.
...
1. **Projection contract.** `market_wide_structured_financial_period_semantics/v1` is a versioned, retained-only projection over canonical provider facts. It emits `ANNUAL`, `STANDALONE_QUARTER`, `YTD_CUMULATIVE_INTERIM`, `POINT_IN_TIME_BALANCE_SHEET`, or `UNKNOWN_DURATION`, preserving source facts and lineage rather than making a second canonical store. A period label or start/end dates alone are never duration evidence; KBS income-statement quarter semantics and the retained cash-flow cumulative-state resolver are bounded, source-contract-specific exceptions.
2. **Corpus result.** The read-only retained corpus comprises 1,493 gzip JSONL files, 195,552 facts, and 1,492 tickers. All facts are emitted (zero silent drops): 86,912 P-I-T balance-sheet, 68,575 standalone-quarter, 497 YTD, and 39,568 unresolved-duration records; no native annual record appears. 94,252 incomplete-lineage and 12,481 conflict records remain explicit. Currency/scale is unresolved on 195,550 records, so no same-period YoY or margin candidate is declared compatible; 19,702 P-I-T trajectory candidates are an evidence count only.
3. **Authority boundary.** This work does not calculate ratios, growth, margins, scores, valuation, ranking, recommendation, targets, probability, sizing, execution, or backtest outputs. It does not modify the canonical financial facts or raw corpus, promote a provider/official/owner fact, choose conflicts, erase blockers, or treat missing timestamp/unit/scope as known. `authoritative_financial_eligible`, PIT, and actionability remain false for every projected record.
4. **Next gate.** `MARKET_WIDE_FUNDAMENTAL_FEATURE_STORE_V1` is now the sole `NEXT` dependency-linked gate. It is not opened or authorized by this closeout.

## 2026-08-31 - Owner Financial Data-Path Rebaseline and Governance Sync V1

`OWNER_FINANCIAL_DATA_PATH_REBASELINE_AND_GOVERNANCE_SYNC_V1 = COMPLETE` (`push = YES`).

1. **Architecture correction.** The former default sequence of official PDF -> parser -> ticker-by-ticker proof -> OCR -> authority before research is superseded as a default. The current Tier-1 default is structured provider/API financial data with retained raw provenance, identity, period/scope/unit/currency/observation semantics, canonical operational records, explicit fitness-for-use, and deterministic research. `NOT_AUTHORITATIVE != NOT_USABLE`: `OPERATIONAL_PROXY / RESEARCH_ELIGIBLE` may support explicitly labelled fundamental, cross-sectional, valuation, and AI research, but never silently gains official, PIT, audit, or backtest authority.
2. **Escalation, not deletion.** Tier-2 official PDF/Excel/issuer/exchange evidence is a triggered verification/reconciliation asset for owner focus, high-priority opportunities, conflicts, anomalies, missing decision-critical metrics, and thesis-sensitive values. Tier-3 authoritative evidence remains reserved for conflict resolution, PIT/backtest-sensitive facts, corporate actions/ex-dates/share basis, material accounting-identity disputes, audit-sensitive claims, and explicit owner verification. No provider or financial-fact authority is promoted by this rebaseline.
3. **PDF/OCR and HNX disposition.** `PDF_DEFAULT_FINANCIAL_INGESTION = NO`; `TICKER_BY_TICKER_PDF_PROOF_DEFAULT = NO`; `OFFICIAL_DOCUMENTS = TRIGGERED_VERIFICATION_ASSET`; `OCR_DEFAULT_SCALEOUT = DEPRIORITIZED`; `OCR_CRITICAL_PATH_BLOCKER = NO`; `UNLIMITED_OCR_GPU_BENCHMARK = OPTIONAL_SHADOW_EXPERIMENT`. The terminal HNX raw-acquisition closeout (`a0fcf8957c6e2d1bb541b93c136ce65b6d4223a2`) retains 155 immutable official PDFs for 160 targets (30 text-native, 125 image-only; ICC/LQN/MRF residuals; DVT/NHD no-route) as verification assets only. Readability is never an investability classification or a market-wide research blocker.
4. **Feature dispositions.** Claude's unmerged `TEXT_NATIVE_OFFICIAL_FINANCIAL_TABLE_EXTRACTION_SCALEOUT_V1` checkpoint `2507c86a3a8b07f6c09133097e799c5bd194f063` is terminal `PARTIAL_BY_EVIDENCE`: 30 documents processed, 14 statement tables, 10 recognition successes, 0 candidate facts, and 0 promotion candidates. `OFFICIAL_FINANCIAL_VERIFICATION_UTILITY = RETAINED_UNMERGED`; it is not a financial critical-path dependency and no PDF-layout/OCR follow-up is selected. Codex's unmerged Consumer checkpoint `48cc2d4` is `OPTIONAL_VERIFIED_EVIDENCE_CONSUMER_PATH = PREPARED_UNMERGED`; candidate evidence remains optional and non-authoritative, never a default source or gate.
5. **Period correction and primary sequence.** The retained provider corpus is described as `retained_period_depth`, never annual history unless duration is actually evidenced. Where the historical 523-name artifact applies, approximately 449 proxy-ready corporate tickers and 17,276 retained provider facts remain available research context with duration semantics bounded by their contracts. The completed `MARKET_WIDE_STRUCTURED_FINANCIAL_PERIOD_SEMANTICS_V1` confirms that boundary without creating a feature calculation. `MARKET_WIDE_FUNDAMENTAL_FEATURE_STORE_V1` is the sole next gate, with `CURRENT_VALUATION_RESEARCH_PROXY_SCALEOUT_V1`, `MARKET_FUNDAMENTAL_OPPORTUNITY_INTEGRATION_V1`, and `PORTFOLIO_RISK_AND_CAPITAL_POLICY_RESEARCH_V1` remaining dependency-linked later backlog, not simultaneous work. Existing RAW_AS_TRADED/PIT and execution-grade liquidity/sizing blockers remain intact.

## 2026-08-30 - Owner Integration Release Review (Interrupted-Session Takeover)

`OWNER_INTEGRATION_RELEASE_REVIEW = COMPLETE` (`push = YES`).

1. Continuation, not a new milestone: Codex's implementation (`ce52e7c` through `449366e` "version AI handoffs by research build" and `38981ab` "bind runtime release to current producer run") was taken over after a quota interruption, verified rather than re-derived, and finished. `38981ab` was already a correct, complete commit on both PRIMARY and the feature worktree -- not the uncommitted WIP the interruption briefing assumed. Two gaps in its own focused-test matrix (a `run_identity` matching no candidate must fail closed; selecting the current run must not disturb other retained runs for the session) were closed (`3d5f74f`), and one pre-existing test stub's fixed three-argument signature was updated to accept the new `producer_run_identity` keyword so the 2026-08-26 full-replay integration test exercises the real binding path (`47406df`).
2. A second, distinct runtime-materialization defect was found live, not anticipated by the interruption briefing: `_p3_snapshot` required a live-acquisition `post-close-attempt-*` ancestor under the frozen descriptive input's own path, so a session registered through reused retained evidence rather than a fresh acquisition attempt (2026-08-28, `source_acquisition_result=REUSED_RETAINED_EVIDENCE`) had no such ancestor even though the identical P3F9B scaleout snapshot exists at its normal deterministic retained path (`operations-review/p3f9b-market-wide-exact-session-scaleout-<session>/`). Fixed (`52a31f0`) with the same exact-identity, single-candidate discipline the existing code already used: fall back to the session-derived retained directory only when no attempt-root exists at all. Sessions with a genuine live acquisition attempt resolve exactly as before.
3. A real `stocklookup.ps1 daily` run (no explicit session, no replay mode) auto-resolved 2026-08-28 -- today, 2026-08-30, is non-trading -- and published for real: canonical Producer run `daily_producer_run:20b916f95720d5113a85e39c94aaf1bde542e03c44eb83f7303e0e13d7c306d0`, runtime release `READY`, trusted subset `READY`, and a genuine commit-and-push to the private transport repository (`handoff_build_5c41b201c84b12e9c535f8d9915e824c6ef6dc15b73e22986a4f999fb664c07d`, remote `459bb36e24217c1753ffdf69060b92fcc8c01361`). Read-back confirms the pre-existing immutable 2026-08-28 legacy package (`7489406c2e0cd617aae10b0b05c4635c2958781a`) is byte-identical, `LATEST.json` points to the new build, session-bundle/opportunity-queue/manifest hashes match the Producer's own output exactly, `previous_session=2026-08-27`, and the published files carry no secrets or local paths. A required second `stocklookup.ps1 daily` run correctly returned `NO_OP_ALREADY_PUBLISHED` with the remote HEAD unchanged.
4. The primary-checkout registry diff (`config/daily_research_session_input_registry.json`) was present, unchanged in content and SHA-256 (`4f1b0b3a...`), before this session began and remains so after two live owner-flow invocations; it is normal Producer-run bookkeeping (per-session `COMPLETED_RETAINED_EVIDENCE` records appended by the canonical pipeline), never staged or committed here.
5. Authority effect is exactly `OPERATIONAL_SERVING_ONLY`, matching the milestone's own scope; no research, scoring, recommendation, valuation, liquidity, PIT, RAW_AS_TRADED, provider, Dashboard, or Consumer authority changed, and no historical Producer run or legacy handoff was deleted or overwritten. `ROADMAP_EXECUTION_CONTROL_PLANE_V1` and `ONE_COMMAND_DAILY_RESEARCH_TO_AI_HANDOFF_V1` are both production-active. Next gate: `OWNER_ROADMAP_SELECTION`; no new milestone is opened here.
...
## 2026-08-31 - Tactical and Behavioral Engine V2

`TACTICAL_AND_BEHAVIORAL_ENGINE_V2 = COMPLETE_LOCAL / COHERENT_PARTIAL / OWNER_REVIEW_REQUIRED` (`technical_structure_context.py`, `tactical_setup_tags.py`, `tactical_confirmation_invalidation_boundaries.py`, `tactical_behavior_context.py`, four matching `tools/run_*.py` runners, four new test files; `push = NO`).

1. The unmodified, still-primary `watchlist_tactical_entry_classifier.entry_state` nine-state taxonomy is neither forked nor replaced. All four new modules are secondary evidence joined onto it by `build_artifact()` arguments (not a second competing recommendation/action vocabulary); `entry_action`/`action` remain the only "what to do" fields anywhere in this milestone's output.
2. `technical_structure_context.py` supersedes `price_structure_breakout_context.py` for this governed lineage (the old module is untouched; nothing live depended on its `build()` signature, only its `NEAR` constant, which is imported verbatim rather than redefined). It consumes the retained P3F9B exact-session snapshot directly and reports an explicit `close_history_depth` per ticker rather than assuming a uniform 20-session window: cross-checking 15 real tickers against the retained 2026-08-28 `market_wide_current_descriptive_research` artifact proved the existing `ma_20`/`momentum_20d` fields are actually computed over each ticker's full retained observation history (24 to 250 sessions in the sample), not a true 20-session window -- a pre-existing characteristic of `mva_daily_research_bundle.market_features()`, left unmodified per this milestone's own scope boundary (`AI_RULES.md` / no broad legacy cleanup).
3. Per the retained `RETAINED_HIGH_LOW_SCALE_INCOMPATIBLE_NOT_USED` finding, no true ATR, Donchian channel, or other high/low-geometry feature is computed anywhere. Every eligible record instead carries a localized `high_low_basis: {status: NOT_COMPATIBLE, reason: HIGH_LOW_BASIS_NOT_COMPATIBLE, affected_feature_classes: [...]}` block naming exactly which feature classes are unavailable, without blocking that record's close-only structure/contraction/breakout facts. New self-referential facts with no prior governed-lineage equivalent: MA20 slope (needs >=25 retained sessions), self-relative realized-volatility contraction (this ticker vs its own prior 20-session window, needs >=31 sessions; distinct from the existing cross-sectional `volatility_20d` regime), base duration, and a session-over-session breakout/failure/breakdown/re-entry read (needs >=21 sessions).
4. `tactical_setup_tags.py` is a 16-tag multi-label registry in the exact style of `research_setup_classification.py`. It deliberately sources market/sector relative-strength and regime facts from `current_market_sector_leadership_context.py` (the canonical `(below + 0.5*equal)/n` percentile formula) rather than `current_market_screening_opportunity_comparison_foundation.py`'s `_position()`, whose different tie-break formula is a pre-existing, un-refactored drift (`(less + (equal+1)/2)/n`, algebraically always exactly `0.5/n` higher) documented but not fixed in this milestone, matching the "no wholesale legacy refactor" boundary. Relative-volume membership reuses the existing `RELATIVE_VOLUME_ABOVE_COHORT_MEDIAN` cohort-median flag verbatim; no tag anywhere uses a fixed RV multiplier, and no tag is forced to appear. `current_macro_regime.py` is a deliberately separate, unconflated concept -- not read by this module at all.
5. `tactical_confirmation_invalidation_boundaries.py` imports `action_instrumentation._boundary()` directly (the proven generic envelope) rather than duplicating it, and adds two new structural boundary types (`FUTURE_CLOSE_GT_RESISTANCE_LEVEL` / `FUTURE_CLOSE_LT_SUPPORT_LEVEL`, sourced from `technical_structure_context.py`) beyond `action_instrumentation.py`'s original MA20/momentum-only anchors, falling back to those original anchors when no qualified structural level exists. Unlike `action_instrumentation.py`'s own `execute()`, this module's `build_artifact()` takes live artifacts as arguments with no dependency on the frozen 13-issuer `operations-review/*-20260828` snapshot or its issuer count. Each state's boundary honestly mirrors the classifier's own published confirmation/invalidation text: `READY` with a real baseline where the text names one clean numeric trigger, `CONDITIONAL` where it is genuinely disjunctive (`BASE_BUILDING`'s confirmation, `SIDEWAYS_NEUTRAL`'s), `UNAVAILABLE` where a state carries no thesis to confirm/invalidate (`SIDEWAYS_NEUTRAL`'s invalidation, `DOWNTREND`'s confirmation). No arbitrary fixed stop percentage is introduced.
6. `tactical_behavior_context.py` composes all four sources plus `current_market_sector_leadership_context.py` into one compact per-ticker record (primary_entry_state, setup_tags, trend/structure/contraction/breakout/pullback/relative-strength/price-volume-behavior context, market/sector context, both boundaries, coverage, blockers, reason codes, authority boundary) with no full price history embedded; detailed calculation records stay in each producing module's own artifact.
7. Real run against the retained cross-artifact-consistent 2026-08-28 evidence set (`market_wide_current_descriptive_research`, `current_market_screening_opportunity_comparison_foundation`, `current_market_sector_leadership_context`, `watchlist_tactical_entry_classifier`, all independently identity-verified plus session/lineage-checked against each other): 1,683 P3F9B candidates, 942 same-session technical-eligible (exact match to the reference pipeline's own count), 1,507 official-universe/leadership-covered. `entry_state`: DOWNTREND 261, SELLING_PRESSURE_EASING 239, UPTREND_CONFIRMED 185, EARLY_REVERSAL_CANDIDATE 63, BREAKDOWN_RISK 58, SIDEWAYS_NEUTRAL 61, DISTRIBUTION_RISK 25, BASE_BUILDING 33, BREAKOUT_READY 17. `structure_status`: BREAKDOWN_CONFIRMED_BY_RULE 66, BREAKOUT_CONFIRMED_BY_RULE 43, IN_RANGE 370, NEAR_RECENT_RESISTANCE 243, NEAR_RECENT_SUPPORT 220. `range_state`: COMPRESSION 339, EXPANSION 291, STABLE 312. `self_relative_volatility_state`: CONTRACTION 202, EXPANSION 95, STABLE 639. `ma20_slope_state`: RISING 519, FALLING 409, FLAT 12. `base_status`: IN_BASE 830, NOT_IN_BASE 112. `breakout_event`: BREAKDOWN_CONFIRMED 47, BREAKOUT_CONFIRMED 27, BREAKOUT_FAILURE 22, RE_ENTRY_ABOVE_SUPPORT 13, NONE 833. Setup tags: NEAR_RESISTANCE 243, NEAR_SUPPORT 220, PULLBACK_TO_SUPPORT_IN_UPTREND 28, RANGE_COMPRESSION 339, RANGE_EXPANSION 291, BREAKOUT_CONFIRMED_BY_RULE 43, BREAKOUT_FAILURE 22, EARLY_REVERSAL_STRUCTURE 63, RELATIVE_STRENGTH_LEADER 236, RELATIVE_STRENGTH_LAGGARD 235, TECHNICAL_DETERIORATION 620, PRICE_VOLUME_DISTRIBUTION_RISK 301; `MARKET_REGIME_TAILWIND`/`HEADWIND` and `SECTOR_LEADING`/`WEAKENING` were confirmed real zeros for this session (retained market and all 24 sector groups were genuinely `MIXED_BREADTH`/`MIXED`/`DATA_LIMITED` on 2026-08-28), not a code gap. Confirmation boundary READY 402 / CONDITIONAL 279 / UNAVAILABLE 1002; technical-invalidation boundary READY 642 / CONDITIONAL 239 / UNAVAILABLE 802; structural levels used for 942/942 eligible tickers.
8. No score, rank, target price, probability, position size, or universal fixed indicator gate (no RV multiplier, no RSI/ADX/MACD threshold) exists anywhere in any of the four artifacts (verified by dedicated tests, not just review). All identity hashes exclude `requested_at`; same inputs at completed sessions plus config/engine version reproduce an identical `artifact_sha256`. Zero silent ticker drops in every artifact (every candidate in the source cohort receives a record, eligible or not); a ticker missing from the P3F9B snapshot, or absent from `current_market_sector_leadership_context` (176 non-official-universe tickers: 1,683 candidates minus the 1,507 official-universe cohort it covers), degrades to an explicit localized `UNAVAILABLE`/`NOT_ELIGIBLE` record rather than being omitted or raising.
9. 52 new focused tests (unittest, real synthetic fixtures built through the actual sibling `build_artifact()` functions where practical, not hand-duplicated logic) plus the directly-affected existing suites (`test_watchlist_tactical_entry_classifier`, `test_price_structure_breakout_context`, `test_action_instrumentation`, `test_market_wide_foundation`, `test_current_market_screening_opportunity_comparison_foundation`, `test_current_market_sector_leadership_context`, `test_shadow_action_readiness`) all pass. One pre-existing failure in `test_price_structure_breakout_context.py` (a missing dated `operations-review/p3f9b-market-wide-exact-session-scaleout-20260820/` evidence file, absent because this branch's ancestry does not include the commit that generated it) was confirmed unrelated to this milestone's changes -- `price_structure_breakout_context.py` itself was not touched. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` are clean.
10. Not wired into `export_ai_bundle.py` or any `--include-*` flag, and no Consumer (`ai-core-private`)/Dashboard/runtime path was touched. That wiring is a separate, later, explicit decision, not assumed by this milestone. No network call, new provider, financial redesign, recommendation-vocabulary redesign, valuation, portfolio implementation, exact-ADTV20/execution-liquidity dependency, historical PIT/backtest, or broad legacy cleanup occurred. `RESEARCH_LIQUIDITY_AND_EXPLICIT_PORTFOLIO_V1` (already registered, `dependencies: [TACTICAL_AND_BEHAVIORAL_ENGINE_V2]`) is the sole recorded NEXT; it is not opened here.

## 2026-08-31 - Research Liquidity and Explicit Portfolio V1 (retroactive narrative sync)

`RESEARCH_LIQUIDITY_AND_EXPLICIT_PORTFOLIO_V1 = COMPLETE_LOCAL / COHERENT_PARTIAL / RESEARCH_PORTFOLIO_CONTEXT` (`research_liquidity_portfolio.py`, `tools/run_research_liquidity_explicit_portfolio.py`; checkpoint `af0e9e4bcd18c938c702fbc605beee8e3c828245`; push = NO).

`docs/ROADMAP_STATE.json` recorded this milestone `COMPLETE` but this narrative doc had no matching entry. Added here for parity before the successor milestone below closes; see [[feedback_verify_milestone_claims_against_code]].

1. `research_liquidity_and_explicit_portfolio/v1` is a compact current-session projection over already-computed liquidity and portfolio-risk evidence. `build_liquidity_research_context()` projects per-ticker descriptive volume into `LIQUIDITY_RESEARCH_PROXY` / `LIQUIDITY_RESEARCH_UNAVAILABLE` with a separately fail-closed `exact_execution_capacity_status`; it computes no ADV, turnover, or sizing.
2. `build_portfolio_research_context()` requires an explicit user portfolio (`portfolio_id`, `as_of_session`, `positions[]` with exactly one of `quantity`/`explicit_weight`/`explicit_market_value`) matched to the same session as the retained `current_portfolio_risk_research/v1` risk artifact it joins against; it fails closed on session mismatch (`PORTFOLIO_SESSION_MISMATCH`). It reports sector/tactical concentration, the selected completed-session joint-risk horizon (`L20`/`L60`/`L120`/`L250`), and user policy-limit breaches -- all derived from already-computed weights and risk context; no new correlation or volatility math.
3. Descriptive liquidity research proxy is usable; exact execution capacity remains separately blocked. Explicit portfolio context is availability-only and is explicitly not a security-attractiveness input.
4. This unlocked `CURRENT_VALUATION_AND_OPPORTUNITY_INTEGRATION_V1`, which in turn unlocked `INVESTMENT_DECISION_WORKSPACE_V1` (below).

## 2026-08-31 - Current Valuation and Opportunity Integration V1 (retroactive narrative sync)

`CURRENT_VALUATION_AND_OPPORTUNITY_INTEGRATION_V1 = COMPLETE_LOCAL / COHERENT_PARTIAL / OWNER_REVIEW_REQUIRED` (`current_valuation_opportunity_integration.py`, `opportunity_context.py`, `security_decision_context.py`, `current_research_valuation_context.py`, `tools/run_current_valuation_opportunity_integration.py`; checkpoint = this milestone's own base, `2bdb4b09c464e8875ba808153e6aa492fe9f043e`; push = NO).

Also missing from DECISIONS.md despite `ROADMAP_STATE.json` recording `COMPLETE`; added for the same reason as the entry above.

1. Producer-only join of retained fundamental-feature-store, tactical-behavior, watchlist, current-valuation, liquidity, corporate-event, thesis-case, sector-leadership, and portfolio-risk artifacts into two governed per-ticker contracts: `opportunity_context/v1` (7-axis interpretable evidence, no score/rank) and `security_decision_context/v1` (a deterministic research-stance state machine over those axes: `INITIATE_RESEARCH_CANDIDATE` / `ACCUMULATE_RESEARCH_CANDIDATE` / `WAIT_FOR_CONFIRMATION` / `HIGH_RISK_SPECULATION_ONLY` / `AVOID_NEW_ENTRY` / `INSUFFICIENT_EVIDENCE`).
2. Real 2026-08-28 run: denominator 1,699, zero silent ticker drops, 1,406 tickers with >=3 usable major axes, 945 with >=5. Method-level valuation carries explicit share basis (`CURRENT_SHARE_RESEARCH_PROXY` / `EXACT_OR_QUALIFIED`) and entity-class applicability (bank/securities/insurance/finance_company correctly exclude P/S and EV/EBITDA). Exact liquidity execution capacity is asserted, and tested, to never force `WAIT_FOR_CONFIRMATION`.
3. Dashboard/Consumer unchanged this milestone. `INVESTMENT_DECISION_WORKSPACE_V1` was the sole recorded NEXT ("Combines `security_decision_context` with `portfolio_fit_context`"); it is opened and closed in this same session, below.

## 2026-08-31 - Investment Decision Workspace V1

`INVESTMENT_DECISION_WORKSPACE_V1 = OUTCOME_A_PRODUCT_CONVERGENCE_COMPLETE` (`investment_decision_workspace_projection.py`, `tools/run_investment_decision_workspace_projection.py`, `tests/test_investment_decision_workspace_projection.py`; Dashboard `investment-workspace.html`, `assets/js/investment-workspace.js`, `assets/css/investment-workspace.css`, `tools/build_investment_decision_workspace.py`; push = NO).

1. `investment_decision_workspace_projection/v1` is a pure join over the already-verified `opportunity_context/v1` + `security_decision_context/v1` pair (plus optional sector-leadership, explicit-portfolio-research, and prospective-thesis-lifecycle enrichment) into one compact per-ticker workspace card. It requires no requalification of any raw upstream artifact and computes no new fundamental/technical/valuation/liquidity/portfolio number anywhere; it fails closed on a lineage mismatch between the two primary inputs (`OPPORTUNITY_DECISION_LINEAGE_MISMATCH`) and on a ticker-set mismatch (`SILENT_TICKER_DROP`).
2. **Corrective finding, verified against real production data, not review**: the "market cap is size context, never a relative-value input" invariant (`docs/DECISIONS.md`, 2026-08-27 entry) was already correctly enforced in `current_valuation_research_proxy.py`, but **not** in `current_research_valuation_context.py::attach_peer_relative()` -- the function that actually feeds `opportunity_context.py`. `_peer_key()` had no method-id filter, so a cheap-looking `market_cap` percentile alone could set `relative_research_state = ATTRACTIVE_RELATIVE_RESEARCH` / `EXPENSIVE_RELATIVE_RESEARCH` even with zero usable true multiples (P/E, P/S, P/B, EV/Sales, EV/EBITDA). Confirmed live on the retained 2026-08-28 `opportunity_context_artifact.json`: **440 of 1,699 tickers (25.9%)** carried a market-cap-only attractive/expensive label (472 total attractive/expensive, only 32 apparently multiple-supported -- and that 32 itself was not fully clean, since a computable-but-non-extreme true multiple could still be overridden by an extreme market-cap percentile). Fixed at the source: `attractive`/`expensive`/`in_line` now filter `relatives.items()` to `method_id in RELATIVE_METHODS` (the constant already used correctly elsewhere in the same file for `usable_relative_method_count`). Regenerated locally from the same retained inputs (no network): **0 of 1,699** market-cap-only mislabels after the fix; total attractive/expensive-labelled tickers dropped from 472 to **6**, all now backed by a real ready relative-valuation method (verified per-ticker, e.g. QNS's `ATTRACTIVE_RELATIVE_RESEARCH` is EV/Sales-backed, percentile 0.083, 6 peers). New regression test `test_market_cap_percentile_alone_cannot_produce_attractive_or_expensive_relative_state`. The new projection module also applies the same guard defensively at display time (`market_cap_semantic_guard_applied` field, unit-tested against a hand-rigged pre-fix-shaped record) so a workspace built from an un-fixed or future-regressed artifact still cannot display the mislabel.
3. Per ticker, the card exposes: research stance/readiness, entry_state/entry_action/setup_tags, a WHY block (fundamental/valuation-with-method-and-basis/tactical/market-sector/catalyst evidence plus the deterministic stance reasons), counter-thesis (warnings, key counter-thesis, unavailable dimensions), the full confirmation boundary, technical + fundamental invalidation, liquidity readiness separate from exact execution capacity, portfolio fit (see point 4), a prospective-case block, and a lineage/authority block (per-axis source session, per-axis freshness, per-axis proxy-or-qualified state, blockers, and an explicit `DEEP_EVIDENCE_ARTIFACT_NOT_MATERIALIZED_LOCALLY` state that never blocks the card). Real 2026-08-28 run: denominator 1,699, zero silent drops, `research_stance_distribution` identical to the upstream contract (183/344/47/68/757/300), `valuation_relative_state_distribution` {ABSOLUTE_RESEARCH_ONLY: 36, ATTRACTIVE_RELATIVE_RESEARCH: 4, EXPENSIVE_RELATIVE_RESEARCH: 2, PE_NOT_MEANINGFUL: 274, UNAVAILABLE: 1383}, `stale_axis_present_count` 1,699 (the catalyst axis is genuinely `STALE_BUT_RESEARCH_USABLE` for every ticker this session -- a real finding, not a bug), `prospective_cases_available_count` 0 (no retained prospective-lifecycle artifact for this session; reported as the honest `CASE_DATA_UNAVAILABLE`, not fabricated).
4. Portfolio fit is a small new categorical vocabulary (`NOT_EVALUATED` / `NO_CONCENTRATION_FLAGGED` / `ALREADY_HELD` / `ADDS_SECTOR_CONCENTRATION` / `EXCEEDS_USER_POLICY_LIMIT`) derived only from already-computed `portfolio_research_context` fields (ticker lookup, existing `user_limit_breaches`, existing `sector_concentration` weights) -- no new risk math. It never reads or mutates `security_decision_context`'s stance (tested both Producer- and Dashboard-side). `tools/run_research_liquidity_explicit_portfolio.py` gained an optional `--portfolio-input <path.json>` (default demonstration behaviour byte-preserved when omitted) and was validated against four real scenarios over the retained 2026-08-25 risk/liquidity evidence: the existing top-3-by-weight demonstration; a 6-sector diversified portfolio (0 breaches); a 100%-single-sector concentrated portfolio with no policy limits set (0 breaches, by design, to isolate "concentrated" from "breached"); and the same concentrated holdings with `max_single_name_weight`/`max_sector_weight` limits, producing both a `MAX_SINGLE_NAME_WEIGHT` and a `MAX_SECTOR_WEIGHT` breach. All four confirm `L20`'s joint-risk matrix is (correctly, per `current_portfolio_risk_research.py`'s own `T >= N + 5` guard) never selected while `L60` is ready.
5. Dashboard: `investment-workspace.html` + `assets/js/investment-workspace.js` (dual browser/Node export, the repo's existing zero-dependency convention) join the existing shared shell/nav (`vs-shell`/`vs-topbar`/`vs-sidebar-link`, linked from `dashboard.html` and cross-linked from `decision-cockpit.html`) rather than a disconnected app, and reuse `decision-cockpit.css`'s card/badge/state vocabulary. Opportunity list with 15 client-side filters (research stance x4, tactical setup x3, profitable/turnaround, valuation-available/attractive/expensive, liquidity-proxy-available, catalyst-available, stale-evidence) implemented as pure, unit-tested predicates that never mutate the source cards. Seven-section Decision Card (Current Stance / Why / Counter-thesis / Confirmation / Invalidation / Portfolio Impact / Data-Authority). The existing browser-only Portfolio Editor (`portfolio.html`/`portfolio.js`) is preserved byte-for-byte; its LocalStorage key (`stocklookup.portfolio-research.v1`) is reused read-only, and a loaded `portfolio_research_context.json` re-joins per-ticker portfolio fit client-side using the identical ticker/breach/concentration-lookup logic as the Producer module -- zero correlation, volatility, or optimization math runs in JavaScript anywhere. "Export T0 candidate" downloads a small versioned (`t0_candidate_export/v1`) snapshot explicitly labelled as not-yet-a-retained-case; no production/runtime write occurs, and retention still requires the existing `durable_prospective_research_case_store.py` contract with real provenance.
6. Verified live against the real 1,699-ticker dataset (`python -m http.server`, not `file://`, to satisfy `fetch()`): the opportunity list, all 15 filter chips, and the ticker-selected Decision Card render correctly with the real coverage numbers (e.g. clicking the "Attractive (relative)" chip narrows the list to exactly 4/1,699, matching `valuation_relative_state_distribution.ATTRACTIVE_RELATIVE_RESEARCH`); QNS's real EV/Sales-backed attractive label renders end to end. One real UI defect was found and fixed during this verification: the initial ticker selection at page load was unconditionally calling `scrollIntoView`, auto-scrolling away from the opportunity list on first paint; a second was a color-classifier that defaulted most of this system's own governed vocabulary (`PROFITABLE`, `WAIT_FOR_CONFIRMATION`, `SELLING_PRESSURE_EASING`, ...) to the alarm-red "unavailable" bucket -- replaced with a vocabulary-aware tri-state classifier. Both fixed and re-verified in-browser.
...
1. `prospective_case_admission_policy/v1` takes a same-session `investment_decision_workspace_projection/v1`, admits every governed non-insufficient stance without discretionary selection, assigns role (`ACTIVE_RESEARCH_THESIS`, `WATCH_FOR_CONFIRMATION`, `SPECULATIVE_RESEARCH_THESIS`, or `NEW_ENTRY_VETO_OBSERVATION`), and freezes only current Workspace fields plus the governed close-price basis. It excludes `INSUFFICIENT_EVIDENCE` as `NO_EVALUABLE_RESEARCH_THESIS` and leaves exact execution-capacity/valuation gaps localized rather than gating admission.
2. The existing durable case store gains a policy-admission write path rather than a competing database or fictional AI/human-review record. It remains content-addressed, append-safe, and replayable. Unchanged active ticker/role/material-state signatures resolve `CASE_ALREADY_ACTIVE`; a material semantic transition can form a new case.
3. The current 2026-08-31 DNSE working-date probe did not mark 2026-08-31 as eligible. The latest local governed retained session was 2026-08-28, but the owner directive forbids retrospectively admitting that old Workspace projection as a new T0 on 2026-08-31. No network market acquisition was attempted after that Phase-A refusal and no T0 case was fabricated.
4. The new one-shot operator runner validates the retained completion evidence, exact projection-session equality, admission, durable retention, and initial outcome state. It has no polling, scheduler, daemon, Dashboard, provider, or production-runtime behavior. No successor is queued; a future genuine qualified current session is required before the cohort can activate.

## 2026-09-02 - Daily Integrated Decision Brief And Prospective Feedback V1

`DAILY_INTEGRATED_DECISION_BRIEF_AND_PROSPECTIVE_FEEDBACK_V1 = COMPLETE / COHERENT_PARTIAL_BY_RETAINED_EVIDENCE` (`push = YES`, `90e3260a5596fa02c210fcc13bb3d1b28d8b329f`).

1. **New contract name, not `current_daily_decision_research_product/v3`.** The milestone directive's preferred direction was to upgrade `current_daily_decision_research_product/v2`, with an explicit fallback to "an equivalent clean successor version if existing compatibility requires it." That fallback was invoked: `current_daily_decision_research_product/v2`'s shape (per-ticker research-card browsing product -- `detailed_research_cards`, `research_cohorts`, `owner_focus`, `high_priority_full_universe_review_set`) is genuinely disjoint from what a daily posture/transition/opportunity-set brief needs (`market_summary`, `sector_summary`, `opportunity_sets`, `watchlist`, `decision_transitions`, `feedback_status`), and v2 has 19 real dependent files (`daily_research_session_operations.py`, `export_ai_bundle.py`, `ai_research_session_delivery.py`, plus tools/tests) that a shape-incompatible v3 bump would either break or force into an awkward dual-shape module. `daily_integrated_decision_brief/v1` is a new, separate contract; `current_daily_decision_research_product.py` is completely untouched, byte-identical to before this milestone.
2. **T0 case admission for the new posture vocabulary was scoped out, not built as a second system.** The existing durable T0 case store (`durable_prospective_research_case_store.py` + `prospective_case_admission_policy.py`) hard-gates on `workspace.get("contract_version") == "investment_decision_workspace_projection/v1"` and an `ADMISSIBLE` set drawn from that contract's older 6-value `research_stance` vocabulary -- disjoint from `integrated_investment_decision_product/v1`'s 9-value `research_action_posture`. Rewiring that hand-verified, fail-closed admission gate (material-state signature dedup, boundary condition mapping, etc.) to also accept integrated-decision-shaped input is real, separate, higher-blast-radius work that was out of proportion for this milestone's time budget. Instead: every session's `integrated_investment_decision_product` artifact is already immutable, content-addressed, and session-keyed by construction (no admission/store step needed for retention itself), and a new lightweight module (`integrated_decision_prospective_feedback.py`) adds only the genuinely missing piece -- a forward-outcome walk over the session's own retained P3F9B price history, reusing `prospective_decision_outcome_measurement.classify_feedback_taxonomy()` directly (same engine, same labels) rather than duplicating it. Closing the real admission-policy gap (so genuine new integrated-decision T0 cases accumulate in the durable store, not just via this session-keyed bridge) remains a legitimate, separately-scoped future task.
3. **Two real production defects found in the immediately preceding integration (`5227207`) were fixed in this milestone; a third was found and deliberately left unfixed.** All three are documented in full in `docs/STATE.md`'s 2026-09-02 entry for this milestone and in `operations-review/daily-integrated-decision-brief-prospective-feedback-v1-20260902/REPORT.md`. Summary: (1) `market_regime`/`sector_leadership` key-path mismatch, fixed; (2) `technical_structure_artifact`/`relative_volume_artifact` fed structurally incompatible legacy artifacts (made every ticker's posture come out `INSUFFICIENT_CURRENT_RESEARCH` in production), fixed; (3) Financial V2 has no canonical daily-materialization path anywhere in this pipeline, so the fundamental input to the same closure is a stale, wrong-shaped legacy artifact -- found, documented, and deliberately not fixed here, since it degrades gracefully (per Section 18/doctrine's "one missing family doesn't erase other context" invariant -- `fundamental_state` honestly reports `INSUFFICIENT` rather than corrupting posture computation) and properly closing it (wiring Financial V2's own daily materialization into the canonical pipeline) is a correctly-sized undertaking on its own, not a same-milestone patch. Flagged as a follow-up task, not started.
4. **`posture_transition`'s named-transition table is a priority-ordered lookup, not an enum-complete mapping.** `next_session_decision_brief.py`'s new `_classify_posture_transition()` checks the tactical-phase-based `UPTREND_TO_BREAKDOWN` first (it can co-occur with several posture pairs), then the explicitly-named posture-pair transitions the milestone directive listed (`WAIT_TO_INITIATE`, `EARLY_WATCH_TO_INITIATE`, `INITIATE_TO_HOLD`, `INITIATE_TO_EXTENDED`, `BREAKOUT_FAILED`, `AVOID_TO_RECOVERY_WATCH`), then the more general `NEW_BREAKOUT`/`NEW_EARLY_WATCH`/`NEW_RETEST_CANDIDATE` buckets for any other origin reaching that posture, and finally an explicit `POSTURE_CHANGED_OTHER` catch-all -- never forcing a real transition into a named bucket it does not semantically fit, and always retaining the exact prior/current posture/tactical-phase/decision-identity fields alongside the coarse label regardless of which branch fired.
5. **Financial V2 wiring gap is a separate follow-up, not this milestone's scope.** Recorded here as the durable decision record for why `_integrated_investment_decision_product()`'s `financial_analysis_artifact` was left pointed at the legacy `market_wide_current_fundamental_research/v1` artifact rather than being rewired to `financial_analysis_product_projection/v1` inside this milestone: doing so properly requires either building the full Financial V2 engine chain (structured-period-semantics + fundamental-feature-store + entity-classification snapshots) fresh inside the canonical pipeline every session, or establishing a new "latest Financial V2 snapshot" canonical-path convention this pipeline does not have today -- either is a correctly-sized milestone of its own. `tools/run_daily_integrated_decision_brief_replay.py` and `daily_integrated_decision_brief.py` demonstrate the correct end state (real fundamental_state variety) by building the projection fresh from retained snapshots, the same way `tools/run_integrated_investment_decision_replay.py` already does, so the intended behavior is proven even though production's own daily materialization gap remains open.

## 2026-09-04 - Daily 2026-09-04 Terminal Corrective and Canonical Operational Acceptance

`DAILY_20260904_TERMINAL_CORRECTIVE_AND_ROADMAP_RESUME_V1 = ACCEPTED` (`push = NO`, local corrective `c7982bd1ec1b4589346da7f7547886e7b8778111`, base `282666015c7945786a098a259876f7cffb162786`).

1. **The incident report's fan-out/runtime claims did not match retained evidence and were not treated as fact.** An owner-directed incident brief described a single 24,246.5-second canonical Daily process exiting 1 with an implied KBS fan-out defect (1506 requests off a 0-exact DNSE pass). Tracing every field back to real artifacts (`operations-review/p3f9b-market-wide-exact-session-scaleout-20260904/`, `.../current-universe-status-and-session-coverage-resolution-v1-20260904/`) found no canonical-operation run directory and no registry entry for 2026-09-04 anywhere -- the evening's real artifacts came from individual stage tools run standalone while validating that day's two earlier correctives (`a6c8b9a` activity-aware recovery, `2826660` Vnstock rate governor). `24,246.5s` reconciles exactly to wall-clock time between the first same-day P3F9B snapshot (15:31:51.387672) and the final standalone-tool exception (~22:15:57.9), not one process's runtime. The 1506 KBS requests were the residual-yield sentinel correctly detecting real recoverable value (`POSITIVE_YIELD_EXPAND`, 7/16 sampled, 0 provider errors) against a DNSE pass that genuinely returned 0 exact records, and correctly expanding -- KBS recovered 957/1506 in 2064.3s, under the existing 45-minute budget, landing at 958 exact (matching three independent same-day manual DNSE readings). No fan-out or runtime defect existed; the activity-aware sentinel and rate governor both worked exactly as designed. Full lineage table: `operations-review/daily-20260904-terminal-corrective-and-roadmap-resume-v1/incident_reconciliation_and_root_cause_report.md`.
2. **The one real defect: `recovery_candidates()` read the prior session's baseline instead of the current session's own snapshot.** `market_wide_current_technical_coverage_scaleout.recovery_candidates()` decided which tickers needed an extended-lookback technical-history backfill from the PRIOR completed session's own `technical_features.status`, never the current session's resolved snapshot. On an ordinary night DNSE's own retained series is already deep, so this proxy holds; on 2026-09-04, ~957 tickers received their target-session bar via KBS/VCI gap-recovery instead, which projects only a single-bar observation (`multi_source_exact_session_resolver._project_to_p3f9_shape`, by design -- that recovery path exists to confirm one session's bar, not backfill a window). None of those tickers had ever been baseline-`MISSING`, so only 7 of the real ~951-958 affected tickers were selected, and `market_wide_current_descriptive_research`'s existing `RECOVERABLE_SAME_SESSION_TECHNICAL_HISTORY_GAP` safety gate correctly refused to proceed -- exactly its documented job. Confirmed by direct replay against the real retained 2026-09-04 artifacts before any fix was written (951 gap tickers reproduced, first 20 byte-identical to the incident report's own alphabetical list).
3. **Fix is additive to the existing extended-lookback contract, not a new recovery mechanism.** `recovery_candidates()` now falls back to computing `market_features()` directly from the current session's own snapshot observations whenever the baseline status was not already `MISSING`, so a ticker whose bar arrived via this-session gap-recovery is still routed into the existing, unmodified `RECOVERED_COMPLETE_TECHNICAL_HISTORY` / `INSUFFICIENT_HISTORY_AFTER_EXTENDED_LOOKBACK` contract. No change to the KBS/VCI resolver, the rate governor, the eligibility projection, or the residual-yield sentinel. The stale pre-fix `market-wide-current-technical-coverage-scaleout-v1-20260904/` artifact (7-candidate result) was moved, not deleted, to `operations-review/daily-20260904-terminal-corrective-and-roadmap-resume-v1/pre_fix_evidence/` so a fresh Daily invocation would regenerate it under the fix rather than silently reuse the incomplete result via the pipeline's own idempotent reuse-if-exists pattern.
4. **First-ever canonical Daily invocation for 2026-09-04 accepted with explicit partial coverage.** `.\stocklookup.ps1 daily` completed end to end (`DAILY_OPERATION_STATE=LOCAL_COMPLETE`, `DAILY_PRODUCER=COMPLETED`, zero errors). Current-Research coverage 958/1506 = 63.6%, `quality_state=PARTIAL_COVERAGE_EXPLICIT`. Technical-history recovery reached 952/958 (99.4%); the remaining 6 are genuine thin/newly-covered names for which even the extended DNSE lookback finds no 20-session window -- a data limitation, not a defect. No RAW_AS_TRADED, PIT, liquidity, sizing, execution, or provider-authority promotion occurred.
5. **Operational side effect: Daily's own pre-existing publish step pushed two commits, verified read-only and accepted as Daily's existing contract, not a new push capability.** Running the owner's own `.\stocklookup.ps1 daily` triggered its existing built-in publish step (the same behavior `ONE_COMMAND_DAILY_RESEARCH_TO_AI_HANDOFF_V1` and `DASHBOARD_CANONICAL_DAILY_COHERENCE_AND_VI_LOCALIZATION_V1` already established), pushing `stocklookup-ai-handoffs` commit `7da247be68131693f42028a316a899910928d26a` and `market-dashboard` commit `3329287833785e39734d7b6bd9a1e58729ec03e8`, both to their `origin/main`. Both were verified read-only (`git diff --stat` against their parent) to contain only the normal generated Daily/handoff/dashboard data artifacts (JSON/CSV/JS data exports, one dated HTML report snapshot, handoff manifests) -- no source, config, workflow, or architecture changes. This session's own instruction separately stated no publish was authorized; reconciled as this being Daily's own long-standing, owner-approved contract rather than a new capability this session exercised. Neither repository was reverted or amended, and no further push occurred anywhere, including `stock-core-private` itself (local corrective `c7982bd`, this closeout commit, both unpushed).

## 2026-09-05 - Core Valuation Method Coverage And Consistency V1

`CORE_VALUATION_METHOD_COVERAGE_AND_CONSISTENCY_V1 = COMPLETE` (`push = NO`, local commit `35bb7e066a7f2bf8d4db6dd924e6e10a241f7da6`, base `21d752f3561a43e6940d62a180c584db2fca11f1`).

1. **Activated an existing, complete, previously-unregistered valuation engine instead of designing a new one.** Inventory of this codebase's valuation capabilities found two independent lanes: `current_research_valuation_context.py` (P/E, P/B, P/S, EV/Sales, peer-relative, history-context -- complete via `CORE_FUNDAMENTAL_VALUATION_AND_PEER_CONTEXT_V1`, left entirely untouched by this milestone) and a separate, already-complete engine, `market_wide_calculation_readiness.py` (EBITDA, enterprise value, EV/EBITDA, P/E, P/B, ROE), with its own CLI (`tools/report_market_wide_readiness.py`) and an opt-in bundle section (`canonical_financial_bundle_section.py`, `attach(include=False)` by default). Neither the engine nor its CLI had ever appeared in `docs/ROADMAP_STATE.json`, and a real pre-fix run of the CLI showed every price-dependent capability reporting zero. This milestone's entire scope was: run the dormant engine for real, fix what real data revealed, and report honestly -- no new formula, ratio, or valuation model was written.
2. **Root cause of zero price-dependent coverage was two compounding omissions, not a missing capability.** (a) `canonical_financial_bundle_section._resolve_session_inputs` read `vn_stock.db`'s `ohlcv.close` -- DNSE's established thousands-of-VND OHLC convention -- with no unit conversion at all, on both its pre-supplied-entry and its SQLite-fallback path. This was already exposed by two pre-existing, already-failing tests (`test_p1h_valuation_readiness.py`: HPG price resolved to 21.8 instead of 21800.0, VNM to 61.2 instead of 61200.0) that nobody had investigated. (b) `tools/report_market_wide_readiness.py` never wired `session_price`/`effective_shares` into `evaluate_ticker()` at all -- always `None` -- so `market_capitalisation`/`enterprise_value`/`pe`/`pb`/`ev_ebitda` were trivially blocked by omission, not genuine data absence, for every ticker, every time the tool had ever been run.
3. **Fix reused the existing, purpose-built price-representation contract rather than a magnitude-blind multiply.** `price_representation_contract.to_canonical` (`source=DNSE, capability_id=ohlc_1D, instrument_class=VN_LISTED_EQUITY`) already existed specifically to prevent the non-uniform per-field price-scaling defect its own docstring documents (a prior pipeline once multiplied only `close` by 1,000, leaving `open`/`high`/`low` unconverted). Routing `_resolve_session_inputs`'s two price-reading paths through this one contract, rather than adding a local `* 1000`, keeps the conversion a single, versioned, inspectable table lookup instead of a second, independently-drifting copy of the same constant.
4. **The CLI's new `--session-date` flag is additive and defaults to the tool's exact prior behavior.** Omitting it leaves `session_price`/`effective_shares` `None`, exactly as before this milestone (regression-tested: `tests/test_report_market_wide_readiness.py::NoSessionDateUnchangedTests`). Supplying it reuses `canonical_financial_bundle_section._resolve_session_inputs` (now fixed) and the existing, untouched `market_wide_current_shares_resolver` -- no new price or share resolution logic was written anywhere in this milestone. `--price-basis-verified` is a separate, explicit, default-`False` opt-in: results stay `provider_reported`, never `qualified`, unless a caller asserts the price basis is independently verified, which nothing in this milestone does.
5. **A third, smaller defect was found only because the fix was verified against real data rather than trusted from the code reading well.** `evaluate_ev_ebitda`'s blocker logic named `ebitda_not_ready` only when `ebitda["readiness"] != READY`, never covering the case where EBITDA is genuinely `READY` and simply zero or negative. A full real replay found 13 real ticker-periods (AAH 2026-Q1; ABS 2025-Q3 and 2026-Q1; CVN, EIN, HID, HU3, KDM, PAP, PIV, and three more) hitting exactly this: `ev_ebitda` correctly `blocked` (no meaningless ratio computed) but `blocked_by: []` -- indistinguishable from a missing-input block. Added an explicit `negative_or_zero_ebitda_denominator` reason, mirroring how `PE_NOT_MEANINGFUL` already names negative earnings elsewhere in this codebase's other valuation lane. Re-verified against the real data afterward: all 13 cases now carry the explicit reason, and the two other branches (ebitda genuinely not ready; enterprise value not ready) are unaffected and independently unit-tested (`tests/test_market_wide_calculation_readiness.py`).
6. **Real, measured market-wide impact -- not a projected or claimed one.** 1,492 tickers, session 2026-08-25, price resolved for 850 and shares for 1,474 (both via the existing resolvers, no new acquisition): `market_capitalisation` 0 -> 848 ready, `enterprise_value` 0 -> 786, `pe` 0 -> 839, `pb` 0 -> 825, `ev_ebitda` 0 -> 84; `ebitda` 231 and `roe` 1,321 unchanged (both price-independent, so unaffected by either fix). All `ready` results are `status=provider_reported`; none are `qualified`. Bank/securities/insurance confirmed already correctly `NOT_APPLICABLE` for `ebitda`/`ev_ebitda` via the pre-existing, untouched `financial_entity_applicability.CORPORATE_ONLY_METRICS` -- this milestone confirmed that gate was already working at scale, it did not build it.
7. **Deliberately not opened, and why.** Flipping `canonical_financial_bundle_section.attach`'s `include=False` default to `True`, or wiring `market_wide_calculation_readiness.py` into any live product or Dashboard surface, is a distinct, larger, product/authority decision (which artifact carries it, at what tier, with what downstream Consumer/Dashboard implications) that a bounded coverage-and-consistency milestone should not decide unilaterally -- flagged for a future owner-authorized milestone if wanted. P/S and EV/EBIT were not opened: P/S is already served by the other, untouched valuation lane, and this engine does not implement either. Cash-flow valuation was not opened: no upstream evidence in this replay makes one legitimate yet, per this milestone's own "only where existing evidence makes it legitimate" instruction.

17 new/updated focused tests (9 `test_p1h_valuation_readiness` regression-fixed, 5 new `test_market_wide_calculation_readiness` -- the first dedicated unit coverage that module has ever had, 3 new `test_report_market_wide_readiness`) all pass. Adjacent suites (`test_canonical_financial_facts`, `test_p1g/p1i/p1j_*`, `test_canonical_market_evidence_integration`, `test_collect_market_evidence`, `test_export_ai_bundle`) show zero new failures, confirmed via exact `git stash` comparison. `py_compile` and `git diff --check` clean. Artifact: `operations-review/core-valuation-method-coverage-and-consistency-v1-20260905/`.

## 2026-09-05 - Market-Wide Fundamental Valuation Analytical Product V1

`MARKET_WIDE_FUNDAMENTAL_VALUATION_ANALYTICAL_PRODUCT_V1 = COMPLETE / PARTIAL_BY_EVIDENCE` (`push = NO`, local commit `5201fb9463d683d20f4cb1f6c02113eaa39afe15`, base `5438bc09dc288aaceb53e31b73e1bfdb610e2035`).

1. **This closes the exact loop `CORE_VALUATION_METHOD_COVERAGE_AND_CONSISTENCY_V1` left open.** That milestone's own decision record (2026-09-05, above) explicitly deferred "wiring `market_wide_calculation_readiness.py` into any live product or Dashboard surface" as "a distinct, larger, product/authority decision... flagged for a future owner-authorized milestone if wanted." This is that milestone. Inventory before writing any code confirmed the canonical Daily pipeline (`canonical_daily_financial_v2_materialization.build_calculation_readiness_context`) already computes and threads a `calculation_readiness_record` into `current_research_valuation_context.evaluate_ticker_valuation` for every ticker, every session -- but only for reconciliation display (`valuation_method_reconciliation`), never as a usable value. The wiring gap was one missing method entry, not a missing pipeline.
2. **New `EV/EBITDA_CALC_READY` method_id, deliberately never merged with the pre-existing `EV/EBITDA` method_id.** `current_research_valuation_context._calculation_readiness_method` builds a `_method_shell`-shaped entry directly from the readiness engine's own per-period verdict (value, formula, `blocked_by`), reusing the SAME `_method_applicability(entity, EV_EBITDA)` entity gate the legacy method already uses -- so the two identities can never silently disagree about which entities the metric applies to, only about whether a usable number exists for one that does. The legacy `EV/EBITDA` method (sourced from `market_wide_current_valuation_input_scaleout`, which structurally never retains an exact EBITDA figure) is untouched and remains always-`INPUT_BLOCKED`, exactly as `CORE_VALUATION_METHOD_COVERAGE_AND_CONSISTENCY_V1` found it. Added to `RELATIVE_METHODS` so it automatically participates in the existing `attach_peer_relative` cohorting and `usable_relative_method_count` -- no new peer-comparison code was written.
3. **Real market-wide impact measured on two sessions, not projected.** A live diagnostic (`market_wide_blocker_distribution.json`) on the primary 2026-09-04 session found `EV/EBITDA_CALC_READY` at 0/1,683 `RESEARCH_USABLE`, broken down by exact cause: 1,148 tickers have unresolved `entity_class` in the raw valuation record itself (`ENTITY_CLASS_UNRESOLVED`, a pre-existing gap shared identically by the legacy method, not introduced here); of the 452 entity-resolved corporate tickers, 1,251 fail EBITDA's own cross-statement sign/coherence contract (price-independent, a real data-coverage gap); the remainder fail the calculation-readiness-specific price/share resolution, which this session's own STATE.md entries independently document as DNSE-degraded (0/1,683 exact Pass-1). Re-running the identical wiring on the non-degraded 2026-08-25 session recovered **0 -> 51 tickers `RESEARCH_USABLE`**, while the price-independent `ebitda_not_ready` blocker count stayed byte-identical at 1,251 across both sessions -- direct, measured confirmation that the wiring is real and price-sensitive, not inert, and that the zero on 2026-09-04 is a session characteristic, not a defect.
4. **A second, independently real, previously-undiscovered defect: own-history valuation context has never activated in the Integrated Decision product, on any ticker, on any session, since it first shipped.** `integrated_investment_decision_product.evaluate_valuation_context` read `hist_ctx[feature].get("percentile_in_history")`. The sole real producer of this shape, `financial_analysis_engine_v2._history_entry` (passed through verbatim by `financial_analysis_product_projection._compact`), has only ever emitted a field literally named `percentile` -- confirmed by reading both the producer and the string `percentile_in_history` appearing nowhere else in this repository. This is the exact class of defect this codebase's own prior entries warn about repeatedly (a consumer silently reading a shape its producer never emits); it was found by deliberately reading both ends of the same data flow rather than trusting either side's own tests, since neither side's existing test suite exercised this specific cross-module field-name agreement. Fixed to read `percentile`. Real market-wide impact, verified by re-running the fixed code: `equity_to_assets`/`current_ratio` own-history moves from a permanent, universal `UNAVAILABLE` to genuinely `AVAILABLE` for 1,093/1,683 tickers -- the underlying `financial_analysis_engine_v2` computation was correct the entire time; only its consumption one layer up was broken.
5. **New `financial_composite_context` is a join over two UNCHANGED evaluators' own outputs, never a third, independently-derived read.** `evaluate_financial_composite_context` takes `fund_state`/`fund_supports`/`fund_counters` (from the existing, unmodified `evaluate_fundamental_direction`) and `val_summary`/`val_supports`/`val_counters` (from the existing, unmodified -- except for finding 4's bugfix -- `evaluate_valuation_context`) as plain arguments, computed inline inside `build_ticker_integrated_decision` immediately after both are already produced. This was a deliberate design choice over an earlier alternative (a standalone module recomputing its own fundamental/valuation classification): the codebase's own tactical-momentum milestone record documents "an earlier draft that derived stance directly from raw structure fields produced a real disagreement with `tactical_phase` already exposed on the same record... caught before commit and replaced" -- the identical risk applies here, and computing the composite from the SAME already-produced local variables that also populate `fundamental_state`/`valuation_context_summary` on the same record makes that class of disagreement structurally impossible rather than merely unlikely.
6. **The composite label is deliberately not a vote count across five indicators (owner directive section 14, verbatim).** `fund_state` already synthesizes earnings trajectory, profitability, balance sheet, and cash quality via its own governed, non-reopened logic. The composite reuses that label directly, with exactly one explicit override: a `DETERIORATING` read is never upgraded by cheap valuation (reported as a separate, still-visible axis on the same record, not blended away); an `IMPROVING`/`STABLE` read is downgraded to `MIXED` only when peer-relative valuation is corroborated `EXPENSIVE_VS_PEERS`. `TURNAROUND_EVIDENCE`/`INSUFFICIENT_EVIDENCE` are never manufactured by valuation alone. This is one conditional branch, not a five-way tally.
7. **Zero policy retuning: `research_action_posture` and `decision_identity` are provably unaffected.** `decide_research_action_posture` does not take `financial_composite_context` as an argument and never reads it; a dedicated test (`test_wired_additively_into_build_ticker_integrated_decision_without_moving_posture`) exercises this. `decision_identity()`'s curated field subset (ticker, session, posture, `fundamental_state`, tactical phase, trigger/invalidation, source identities) does not include the new field, so existing decision identities are byte-unaffected -- the same precedent `TACTICAL_MOMENTUM_PARTICIPATION_CONFIRMATION_V1` already established for `momentum_context`/`tactical_confirmation_context`, reused rather than re-derived.
8. **An incidental, out-of-scope finding was surfaced by regenerating retained evidence, and is recorded rather than silently absorbed or quietly fixed.** Producing this milestone's own "before" market-wide baseline required rebuilding `operations-review/canonical-post-close-v1/2026-09-04/enrichment/integrated_investment_decision_product.json` via `canonical_post_close_pipeline.build_enrichment_components` -- the sole production writer of that exact path. The rebuild showed `momentum_context_available: 0` and `tactical_confirmation_state_distribution: {INSUFFICIENT_EVIDENCE: 1683}`, even though `TACTICAL_MOMENTUM_PARTICIPATION_CONFIRMATION_V1` added that capability. Inventory (`grep` for every call site passing `momentum_artifact`/`tactical_confirmation_artifact` into `integrated_investment_decision_product.build_artifact`) found exactly two: this repository's tests, and a separate, dedicated, milestone-scoped replay tool (`tools/run_tactical_momentum_participation_confirmation_replay.py`) whose own output lives at its own path (`operations-review/tactical-momentum-participation-confirmation-v1-20260905/`, confirmed still fully intact and untouched by this milestone). `canonical_post_close_pipeline.py` itself has not been edited since checkpoint `dbad7c3`, which predates the tactical-momentum commit entirely -- so this gap is provably pre-existing, not introduced or worsened by this milestone's own regeneration. Not fixed here: closing it is tactical/momentum pipeline wiring, a different milestone's concern, and this milestone's owner directive scopes it to fundamental/valuation only.
9. **Retained real replay used a `git stash`-isolated before/after design specifically because the shared enrichment path made a naive single-run diff meaningless.** An initial attempt to diff "the file that happened to already exist on disk" against a fresh run conflated this milestone's own contribution with every other already-merged commit's changes to the same code path (confirmed concretely: the pre-existing file's `research_action_posture_distribution` had only 2 distinct values across 1,683 tickers, while a fresh rebuild of the CURRENT, unmodified-by-this-milestone code already produced 8 distinct values -- the difference between a stale snapshot and current `main`, not this milestone's effect). Corrected by capturing a bounded summary (never the full multi-MB artifact) once against this milestone's code reverted via `git stash push` on exactly the three changed production files, and once restored, for both the primary and temporal-boundary sessions -- four full canonical-pipeline rebuilds in total. `fundamental_context_available`/`fundamental_state_distribution` came back byte-identical in all four runs where expected (before vs. after, and 2026-09-04 vs. 2026-08-25), which is itself the strongest available confirmation that Financial V2's periodic, session-independent evidence chain behaves correctly and that this milestone introduced no session-dependent leakage.
10. **Deliberately not built, and why.** DuPont: owner directive section 7 explicitly permits leaving it out when not already product-critical; confirmed absent, not built. FCF TTM / FCF yield: `financial_analysis_engine_v2` has no `_ttm_sum` call for CapEx or the FCF proxy; adding one would be new surface inside the regression-locked engine core, not a wiring task -- reported as an explicit `fcf_yield_ttm: BLOCKED` (`FCF_TTM_NOT_RETAINED_STANDALONE_QUARTER_PROXY_ONLY`) rather than fabricated. Own-history for `EV/EBITDA_CALC_READY`: the upstream `canonical_daily_financial_v2_materialization.build_calculation_readiness_context` pipeline retains only the latest reporting period per ticker, by `canonical_financial_bundle_section.build_section`'s own deliberate, pre-existing contract; extending that shared module's retention behavior is a distinct, larger decision outside a wiring-scoped milestone. Insurance specialist family, inventory/receivables trends: out of scope per AGENTS.md and confirmed absent from `canonical_financial_facts.py`'s canonical metric set respectively. The tactical/momentum pipeline gap in point 8 above.

182 focused tests pass (108 new: 11 in `test_current_research_valuation_context.py` for `EV_EBITDA_CALC_READY`/`earnings_yield_ttm`/`fcf_yield_ttm`; 11 in `test_integrated_investment_decision_product.py` for the own-history bugfix and the full `financial_composite_context` state machine; 2 in `test_daily_integrated_decision_brief.py` for AI-facing passthrough; the remaining 74 are pre-existing tests in those three files re-run green). Adjacent suites (`test_canonical_daily_financial_v2_materialization`, `test_canonical_post_close_pipeline`, `test_current_valuation_opportunity_integration`, `test_financial_analysis_engine_v2`, `test_market_wide_calculation_readiness`, `test_p1h_valuation_readiness`, `test_stocklookup_daily_integrated_decision_brief`) show 186 passed, zero new failures. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. Artifact: `operations-review/market-wide-fundamental-valuation-analytical-product-v1-20260905/` (local-only, not force-tracked). No successor is queued.

## 2026-09-08 - Private Portfolio Context Foundation V1

`PRIVATE_PORTFOLIO_CONTEXT_FOUNDATION_V1 = COMPLETE / LOCAL_ONLY_PRIVATE_OWNER_INPUT` under `OWNER_AUTHORIZATION_2026_09_08_PRIVATE_PORTFOLIO_CONTEXT_FOUNDATION_V1`.

1. **Private workbook data has a hard boundary.** The default owner workbook and every derived artifact live only under `%USERPROFILE%\.stocklookup\portfolio\`; the repository receives source code, documentation, and synthetic test generation only. `portfolio import` emits a content-addressed ledger, snapshot, optional policy, optional account snapshot, and manifest tied to the workbook SHA-256, while `portfolio status` reads only the private latest pointer.
2. **Accounting and economic-recovery semantics are separate.** `WEIGHTED_AVERAGE_CARRYING_COST` derives the current position basis from buys, fees, and taxes; partial sales realize an allocated carrying cost and cash dividends/sale proceeds do not rewrite it. `LIFETIME_NET_CASH_OUTFLOW_PER_CURRENT_SHARE` separately incorporates sale proceeds, cash dividends, ticker financing, and ticker fee/tax cashflows. Account-level margin cost remains unallocated, and `Total` can only raise a reconciliation warning.
3. **No competing portfolio or market engine was created.** Existing `current_portfolio_risk_envelope/v1` remains the future explicit-exposure research boundary. This foundation does not derive a market mark, unrealized P&L without an owner mark, market top/bottom labels, recommendation, sizing, execution, Daily input, Dashboard projection, AI handoff, provider call, or authority promotion. The only proposed future gate is `PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1`.
4. **Owner-input semantics corrective remains within this completed foundation.** `AccountSnapshot` retains current margin debt, available-margin minimum, available-margin maximum, and annual rate as four distinct values; availability never implies debt, and `13.5` in the rate field is `13.5%` per annum. VND is owned by monetary field identity, never by Excel cell formatting or amount magnitude. `SYSTEM_DEFAULT_POLICY_V1` supplies only individually absent policy fields, while `portfolio_policy_v1` retains owner-supplied, default, effective, and provenance values. A cap breach is deliberately not an automatic sell instruction; decision and sizing remain the named successor scope.
5. **Repository convergence (2026-09-08).** Developed on `feature/private-portfolio-context-foundation-v1` (worktree commits `5b689ce`/`6264b8a`) from base `f8532b7678f3f17b1a13c357c787fd1fde453c72`, which was verified as Producer main's exact HEAD at convergence time with zero divergence. Ported onto `main` via a dedicated `integrate/private-portfolio-context-foundation-v1` integration worktree by clean cherry-pick, no conflicts; integration checkpoint `f5771bbd51be50111c48bbcf3f523ca2fbb12c94`. No real owner workbook exists yet at `%USERPROFILE%\.stocklookup\portfolio\portfolio_input.xlsx` (`REAL_PRIVATE_IMPORT_STATUS = OWNER_INPUT_NOT_PRESENT`); a dummy workbook was deliberately not created, so real-private-input validation and `PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1` both remain not started. Focused suite (14/14), `py_compile`, `git diff --check`, roadmap-state JSON parse, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean; 15 pre-existing `test_operate_stocklookup.py` failures confirmed `ENVIRONMENTAL_LIMITATION` (missing `ai-core-private` sibling beside the worktree path, not present beside the main checkout either — same suite passes 25/25 from the primary checkout), not a regression from this milestone.

## 2026-09-05 - Corporate Event Canonical Data Refresh And Ledger Consolidation V1

`CORPORATE_EVENT_CANONICAL_DATA_REFRESH_AND_LEDGER_CONSOLIDATION_V1 = COMPLETE / PARTIAL_BY_EVIDENCE` (`push = NO`, local commit `99d9bf4dcd55a8ab16d7c979f7ceb9d99542eee7`, base `5550f0ac7e430563a47625b8d37e56aea8bcd388`).

1. **The frozen 2026-08-21 official corporate-event evidence has a proven, registry-level root cause, not a guessed one.** `current_official_event_context.build_artifact()`'s only real caller anywhere in the repository is the standalone `tools/run_current_official_event_context.py`, executed exactly once (directory dated 2026-08-24) and never since; its own HNX/HOSE inputs are themselves one-off 2026-08-24 snapshots, not a recurring acquisition. `config/daily_research_session_input_registry.json` (read-only, protected, unmodified by this milestone) records the identical `current_official_event_context:ba62a516...` artifact hash for every real Daily session from 2026-08-24 through 2026-09-04 -- six real sessions, zero change. Classified as `MATERIALIZATION_NOT_RUNNING` + `LATEST_POINTER_STALE` + `SUPPLEMENTAL_EVENTS_DISCONNECTED` + `ACQUISITION_NOT_RUNNING` (compounding, per `operations-review/corporate-event-canonical-data-refresh-ledger-consolidation-v1-20260905/frozen_session_root_cause.json`).
2. **Fix is a session-binding and pointer-resolution repair, not a new acquisition.** `canonical_post_close_pipeline._corporate_event_context()` called `current_corporate_event_context.build_artifact(..., research_session=<today>)`, which can never equal the frozen evidence's own `research_session=2026-08-21` -- guaranteeing `EVENT_CONTEXT_SESSION_MISMATCH` and a silent daily fallback to a stale `PRIOR_AS_OF_CONTEXT` copy, and it never activated `supplemental_events`, a gap the prior `CORPORATE_INTELLIGENCE_CATALYST_EVENT_RISK_DECISION_INTEGRATION_V1` milestone explicitly found and deliberately left open for this successor. Fixed by binding to the evidence's own session and activating `supplemental_events`, mirroring the already-correct `current_corporate_intelligence_axis` build pattern one function below it in the same file. `daily_session_level2_package.session_artifact_paths()`'s `official_event_context` key was a hardcoded literal directory, never resolved from a "latest available" pointer; a new `_latest_official_event_context_dir()` helper resolves the newest dated snapshot automatically -- byte-identical behavior today (no fresher snapshot exists), automatic pickup the next time raw acquisition is ever re-run by a future, separately-authorized milestone. Re-acquiring the raw HNX/HOSE official rights-event index itself was explicitly out of this milestone's bounded scope (no new provider, no broad web scrape/scaleout) and is reported as a residual, not fabricated.
3. **The two raw corporate-action ledgers are genuinely distinct, not diverging -- no merge.** `corporate_action_ledger.py` (PIT/price-basis MVP over `semantic_evidence_bridge`) and `official_corporate_action_ledger.py` (pillar-B official-document cross-reconciliation over `corporate_action_events.py`) have different upstream evidence lineage and zero overlapping consumers; `official_corporate_action_ledger.py`'s own module docstring already documents the separation as deliberate prior architecture. Neither ledger is, or has ever been, consumed by the Current-Research corporate-event path (`current_corporate_event_context.py`/`current_corporate_intelligence_axis.py`/`market_wide_current_corporate_intelligence.py`) -- confirmed by grep and import-list reading, locked in by a new `LedgerBoundaryTests` regression test. No code change to either ledger; no destructive migration; no dropped evidence.
4. **Taxonomy fix closed 3 of 5 real "bonus issue" spellings, and corrected a stale claim from the prior milestone's own summary rather than propagating it.** Direct grep found `bonus_issue`, `bonus_shares`, `bonus_share`, `BONUS`/`BONUS_OR_STOCK_DIVIDEND`, and `stock_dividend_bonus_issue` as the five real spellings; two already converged via the existing `current_corporate_intelligence_axis._RAW_EVENT_TYPE_MAP`, three did not (`bonus_shares`, `bonus_share`, `stock_dividend_bonus_issue`) and would have silently fallen into `OTHER_MATERIAL_EVENT` as an independent family. Fixed with three additive map entries; no raw ledger's own vocabulary was rewritten. The prior milestone's own summary had named a sixth spelling, `share_issuance`, that this session's direct grep found does not exist as a literal anywhere in the codebase (the closest match, `SHARE_ISSUANCE`, is a genuinely distinct canonical taxonomy bucket for new capital-raising issuance) -- not carried forward as a bonus-issue alias, since doing so would have been exactly the false convergence the milestone directive warns against. This follows the standing verify-milestone-claims-against-code discipline: a prior session's summary is a claim to re-verify against current code, not a fact to build on unchecked.
5. **Real replay proves the fix without moving any real number that shouldn't move.** 2026-09-04 and 2026-08-25 real replays: 1,507 universe, 1,103 tickers with retained evidence, 4,453 deduplicated events (0 conflicted), 0 active-catalyst/0 active-risk tickers -- byte-identical to the prior milestone's own measurement on the same still-frozen evidence (confirming the taxonomy/materialization fixes did not disturb the real corpus), and byte-identical between the two requested replay sessions themselves (both correctly resolve to the one real evidence session, 2026-08-21; `future_event_leak_admitted=0`). Required case review (`FPT`/`HPG`/`SSI`/`QNS`/`PVD`/`PNJ`/`VNM`/`VCB`) unchanged. The only thing that changed is that `corporate_event_context` now reports `status=BUILT` (not `PRIOR_AS_OF_CONTEXT`) for both sessions, including the HPG/VNM/VCB supplemental chains for the first time in the shared enrichment component. Integrated Decision pass-through, `research_action_posture`, and prospective T0 retention all verified unchanged (`decide_research_action_posture`'s signature untouched; `prospective_decision_retention.py`'s snapshot-copy mechanism untouched, since this milestone's fix is scoped to a separate `_attempt()` closure from the one that builds the axis threaded into the Integrated Decision record).

7 new/updated focused tests (`test_corporate_event_context_builds_fresh_with_supplemental_events`, `test_official_event_context_resolves_latest_dated_snapshot`, `test_official_event_context_falls_back_when_no_dated_snapshot_exists`, `test_bonus_issue_alias_convergence`, `LedgerBoundaryTests::test_canonical_event_boundary_does_not_import_either_pit_ledger`, plus one pre-existing assertion corrected to match the new, correct evidence-bound-session behavior). 246 total pass across the 7 directly-relevant suites (`test_canonical_post_close_pipeline`, `test_current_corporate_intelligence_axis`, `test_current_corporate_event_context`, `test_daily_session_level2_package`, `test_integrated_investment_decision_product`, `test_daily_integrated_decision_brief`, `test_prospective_decision_retention`), up from 241 before this milestone, 0 regressions. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. `config/daily_research_session_input_registry.json` verified untouched (its pre-existing operational diff predates this session by hours, matching `docs/ROADMAP_STATE.json`'s own `known_operational_diff_allowlist`). Artifact: `operations-review/corporate-event-canonical-data-refresh-ledger-consolidation-v1-20260905/` (local-only, gitignored). No successor is queued.

## 2026-09-05 - Official Corporate Event Incremental Acquisition And Freshness V1

`OFFICIAL_CORPORATE_EVENT_INCREMENTAL_ACQUISITION_AND_FRESHNESS_V1 = COMPLETE / PARTIAL_BY_EVIDENCE` (`push = NO`, local commit `23b35853dcff5aaa69e7c870368ba478553416b8`, base `27bd74181f9b8e5d7616715552f256be68b40b1c`).

1. **Converted a one-off manual tool into a repeatable, versioned acquisition capability by wrapping, not rewriting, the existing mechanisms.** New `official_corporate_event_incremental_acquisition.py` calls `hnx_enumerable_universe_kllh_event_disclosure_scaleout.build()` and `hose_public_xhr_and_periodic_series_recon.build()` exactly as they already existed, adding only the piece that did not exist: an explicit acquisition-session wrapper. Each attempt is retained as its own manifest under `operations-review/official-corporate-event-acquisition-sessions/{session}/`; raw evidence stays content-addressed and immutable via both modules' own pre-existing `_atomic()` writers (unmodified); `compare_captures()` -- a pure function taking no network access at all -- classifies every retained surface `UNCHANGED_REUSED`, `CHANGED_NEW_VERSION`, or `NEW_SURFACE_OR_PAGE` against the prior successful attempt, so idempotence and versioning are provable from fixtures alone. A failed attempt is retained honestly (never silently swallowed) and, unlike a successful one, may be retried for the same calendar session -- a successful attempt is permanent evidence and is never redone.
2. **`materialize_current_official_event_context()` requires zero additional downstream wiring, by design.** It writes to the exact `current-official-event-context-integration-v1-{YYYYMMDD}` directory-naming convention the prior milestone's `_latest_official_event_context_dir()` resolver already scans for. This was proven, not assumed: a real acquisition this session flowed all the way through `corporate_event_context` -> Corporate Intelligence axis -> Integrated Decision with no code change to any of those three layers.
3. **Two real bugs were found only by actually running the mechanism live, not by code reading.** (a) `hose_public_xhr_and_periodic_series_recon.py`'s `AS_OF_DATE` was a frozen `"2026-08-24"` module-level constant baked into its two disclosure-window URLs at import time -- every future re-run, forever, would have kept requesting the window ending 2026-08-24 regardless of when it actually ran. Fixed additively: `build()` now takes `as_of_date`, defaulting to `vn_time.vn_today()`, with the exact override capability preserved for deterministic replay. (b) A real first acquisition attempt failed with `SOURCE_FETCH_FAILED:hnx_listed_disclosures:441`: the HNX disclosure-index endpoints carry no date-range parameter (unlike HOSE's equivalent), so a full walk re-fetches HNX's *entire* historical corpus every time -- a genuine ~22x real-world growth since 2026-08-24 (19,726 -> 440,000+ records) made the unbounded walk fail partway through pagination. Root-caused before any retry: `current_official_event_context.py`'s own contract was confirmed (by direct source inspection) to read only `hnx_official_rights_event_index/v1`, never the disclosure index at all. `hnx_enumerable_universe_kllh_event_disclosure_scaleout.build()` gained an additive `include_disclosures` parameter (default `True`, zero behavior change for its only other -- in fact nonexistent -- caller); this milestone's own acquisition wrapper defaults it `False`, avoiding an unneeded, unbounded walk rather than adding resumability/page-capping logic that would have had to weaken that module's own correct accounting-integrity checks.
4. **A third real defect, found by the same live run, in a much higher-stakes file: `canonical_post_close_pipeline.register_session_inputs()`'s completed-session mutation-rejection check.** Re-registering the real, already-completed historical session `2026-08-25` against the real registry started raising `COMPLETED_SESSION_INPUT_MUTATION_REJECTED` once a genuinely fresh `event_context` artifact existed for the first time -- because that check compared `official_universe`/`event_context` identities as if they were session-locked, directly contradicting their own pre-existing, already-documented policy elsewhere in this codebase (`daily_session_level2_package.session_triage_status()`, `daily_research_session_operations.py`'s own input-manifest freshness labelling): `"ACCEPTED_CURRENT_ASOF_BUILD_NOT_SESSION_LOCKED"`. This tension existed latently since the prior milestone's `LATEST_POINTER_STALE` fix but never manifested because `event_context` never actually changed value before this session's real second acquisition attempt succeeded. Fixed by excluding exactly those two keys (`NOT_SESSION_LOCKED_REGISTRY_KEYS`) from that specific comparison -- verified directly against the real registry's 6 real completed sessions (`2026-08-21` through `2026-09-04`), and a new regression test proves a genuinely session-locked *required* key is still protected.
5. **The real advance is honest, not manufactured.** A real, live, bounded acquisition attempt (one fix-then-retry, not a blind loop, per the mission's own "do not loop/retry indefinitely" instruction) advanced `current_official_event_context`'s `research_session` from the frozen `2026-08-21` to `2026-09-05` -- `1,101` tickers, `4,444` event records, within `0.2%` of the frozen baseline's `4,453`, consistent with the same real underlying corpus now correctly evaluated as of today rather than a fabricated jump. `future_event_leak_admitted = 0` is proven two ways: with the real data (three different outer sessions -- `2026-08-25`, `2026-09-04`, `2026-09-05` -- all resolve to the one honestly-dated artifact, never fabricating agreement with whatever was requested), and with a stronger fixture-based test using two genuinely distinct retained sessions.
6. **Deliberately not fixed, and why.** HNX disclosure-index acquisition remains out of scope for routine incremental runs: its own no-date-filter server design does not scale to a full walk every time, and building resumability/pagination-capping into that module would risk exactly the kind of silent partial-completeness the mission's own Section 11 warns against; reopening it (if a future product genuinely needs disclosure-index coverage) is separately scoped work. `2026-09-05`'s own other Daily inputs (descriptive research, exact-session snapshot, etc.) are not yet materialized as of this session, so the full Integrated Decision downstream proof used the outer session `2026-09-04` instead -- an unrelated, pre-existing Daily-orchestration timing characteristic this milestone does not control. Neither `current_corporate_intelligence_axis.py`, `integrated_investment_decision_product.py`, nor either PIT/price-adjustment ledger (`corporate_action_ledger.py`, `official_corporate_action_ledger.py`) received any code change.

24 new/updated focused tests in `tests/test_official_corporate_event_incremental_acquisition.py` (incremental happy path, session-reuse refusal, failed-session retry, immutable retention, same-content idempotence, changed-content versioning, acquisition failure fail-closed, latest-session selection, canonical materialization, genuine historical temporal replay, downstream pass-through) plus 2 in `tests/test_hose_public_xhr_and_periodic_series_recon.py` and 1 new regression test in `tests/test_canonical_post_close_pipeline.py` for the mutation-check fix. **175 tests pass** across the 9 directly-relevant suites, 0 regressions. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. `config/daily_research_session_input_registry.json` verified untouched by this milestone's own commits. Artifact: `operations-review/official-corporate-event-incremental-acquisition-freshness-v1-20260905/` (local-only, gitignored). No successor is queued.

## 2026-09-09 - AI Daily Brief Lineage Corrective V1

`AI_DAILY_BRIEF_LINEAGE_CORRECTIVE_V1 = COMPLETE / RELEASED` (checkpoint `354361bb32a56a78af5581edd0b8d2a593824585`, base `c7097946f9af1e0a1271ee6fd924c3ff642b58e8`).

1. **Root cause: canonical orchestration sealed AI delivery before Daily Brief creation.** `next_session_decision_brief` was built independently of, and not necessarily before, the same-session AI-delivery seal, so a Brief materialized after that seal could never be reflected in that day's delivered content — a silent lineage gap between the Brief and the delivery it was meant to inform.
2. **Fix binds the canonical same-session Brief before final AI-delivery materialization, not a redesign.** `canonical_daily_operation.py`, `daily_producer_pipeline.py`, `daily_research_session_operations.py`, and `next_session_decision_brief.py` now thread the Brief's identity through to the delivery overlay before seal: Brief, session, and Integrated-Decision identities propagate exactly; a delivery whose bound Brief carries the wrong session or the wrong Integrated Decision identity is rejected outright, never silently relabeled to fit; and the pre-existing standalone no-Brief delivery path is exercised unchanged. Research content itself is otherwise untouched, and deterministic replay remains byte-deterministic.
3. **Three independent qualification passes reached the identical result, proving both correctness and clean separability from unrelated work.** Developed on `feature/ai-daily-brief-lineage-corrective-v1` (commit `8003658`, base `bf11dbf`), the corrective was ported unmodified onto two successive integration worktrees (RC v2 `e73881a`, RC v3 `59d6ae2`) for qualification, then released by cherry-picking that same commit directly onto `origin/main` at this checkpoint. `git patch-id` confirms all three instances (`8003658`, RC v3's `59d6ae2`, and this release's `354361b`) carry the exact same patch-id (`ce46cb77`) — a faithful port at every step, never a reimplementation. The release deliberately excludes three unrelated local-only Producer commits (`c88db84` valuation entity-class binding, `943c4bf` participation-narrative alignment, `d023c33` roadmap docs) that happened to share linear history with the corrective purely by commit order on the primary checkout, not by dependency: direct diff/grep confirmed none of the corrective's 8 changed files reference any symbol those three commits touch, and an empirical full qualification rerun directly against a fresh `origin/main`-based branch carrying only this one commit reached the identical 198-passed/11-failed/17-errors result as RC v3's own qualification, positively proving no hidden functional dependency rather than merely assuming one absent from a diff.
4. **Every non-passing result across all three passes traces to one pre-existing stale test fixture, not a product defect.** `tests/test_next_session_decision_brief.py` hardcodes `CURRENT_SESSION = "2026-08-28"` as a fixture assumption; the real, further-advanced `LATEST_COMPLETED_RUN.json` now correctly points at `2026-09-09` since real Daily operation has genuinely progressed past that date. All 11 failures and 17 errors trace directly to that one assertion line (`assert session == CURRENT_SESSION`) or its direct callers — confirmed by tracing each failure individually, not inferred from pattern-matching test names. Zero genuine regressions were found in any of the three qualification passes.
5. **Missing retained-evidence fixtures were bridged only with temporary read-only junctions to already-existing governed artifacts, never fabricated.** Each fresh worktree (RC v3, then the release branch) lacked some subset of operations-review session snapshots, the `data_bctc` runtime payload, and the `ai-core-private` consumer-repo sibling that one worktree-relative test path requires — all of which already existed, unmodified, in the primary checkout. Every gap was closed with a plain NTFS directory junction into the corresponding primary artifact, verified present before junctioning, and every junction was removed again once its qualification pass completed; nothing was copied, rewritten, or committed. One gap was found to be a genuine, unrecoverable absence rather than a missing junction: 2026-08-26 descriptive/screening/tactical/triage retained evidence does not exist anywhere in the primary checkout itself, confirmed by direct inspection, and was correctly left out of scope rather than manufactured.

`py_compile`, `git diff --check`, and JSON validation (`docs/ROADMAP_STATE.json`) are all clean; `tools/stocklookup_roadmap.py --check` reports drift `PASS`. The primary checkout's pre-existing uncommitted `config/daily_research_session_input_registry.json` diff (`known_operational_diff_allowlist`) was never touched, read, or staged at any point. RC v2 and the corrective's original worktree were never modified. No Daily run, provider acquisition, Dashboard publication, deployment, runtime mutation, or authority promotion occurred at any stage of qualification or release. Artifact trail: `worktrees/stock-core-ai-daily-brief-lineage-corrective-v1-20260909` (commit `8003658`), `worktrees/stock-core-preproduction-release-candidate-v2-20260909` (commit `e73881a`), `worktrees/stock-core-preproduction-release-candidate-v3-lineage-fix-20260909` (commit `59d6ae2`), released via a dedicated `worktrees/stock-core-release-daily-brief-preseal-v3-20260909` integration worktree (branch `release/daily-brief-preseal-corrective-v3-20260909`). No successor is queued.

## 2026-09-09 - Private Portfolio Context Foundation V1 (Real-Workbook Schema Adaptation)

`PRIVATE_PORTFOLIO_CONTEXT_FOUNDATION_V1 = COMPLETE / REAL_PRIVATE_INPUT_VALIDATED` (checkpoint `91b0cbf568c4c03231640f47602335be9a99222b`, base `35f6ac67a1e67a3662e0d38163c9be45da24e591`).

1. **The real owner workbook's own column labels, not a code defect in header-row detection, caused every header to fail.** The first real import against `%USERPROFILE%\.stocklookup\portfolio\portfolio_input.xlsx` failed closed on `TRADE_HEADER_UNRECOGNIZED`, `DIVIDEND_HEADER_UNRECOGNIZED`, `MONEY_HEADER_UNRECOGNIZED`, and `MARGIN_HEADER_UNRECOGNIZED` (`event_count=0`, `current_position_count=0`). A read-only, values-never-printed structural inspection (sheet names, header-row shape, exact column labels only) found the real workbook uses its own Vietnamese labels -- `KL khớp`/`Giá khớp` (matched trade quantity/price, not the sibling `KL đặt`/`Giá đặt` order-placed columns), `Mã CK`/`Cổ phiếu` (ticker), `Phí GD`/`Thuế TNCN` (fee/tax), `Số tiền`/`Hành động` (Money sheet amount/action), and `Loại Sự kiện` (dividend event type) -- none of which had any alias in `FIELD_ALIASES`. All are now mapped, always preferring the owner's actually-executed columns over merely-requested ones.
2. **A genuine, general-purpose normalization bug was found and fixed, not merely worked around.** `_normal()`'s NFKD-based diacritic stripping silently dropped Vietnamese `đ`/`Đ` (U+0111/U+0110) entirely instead of folding it to plain `d`/`D`, because that letter has no NFKD compatibility decomposition (unlike every other Vietnamese diacritic). `"Hành động"` normalized to `"hanhong"`, not `"hanhdong"`, corrupting any real label containing `đ`. Fixed at the root by folding `đ`/`Đ` to `d`/`D` before NFKD normalization; a regression test proves the fold directly.
3. **The `margin` sheet is a genuinely headerless legacy ledger in the real workbook, not a header-recognition failure.** Its real layout has no label row at all: a fixed date/amount/free-text-note column shape starting immediately at row 2, with a trailing summary row. A narrowly-scoped positional fallback (`_legacy_positional_margin_events`) recognizes this shape only when no label header and no account-snapshot-style fields exist for this specific sheet -- every other sheet, and this one when it does carry a header, is still always matched by label, never by column position. Every row is classified `MONEY_MOVEMENT`, deliberately never `TICKER_FINANCING_COST`/`ACCOUNT_MARGIN_COST`: the real notes describe deposits, loan draws, and debt repayments, and forcing them into a cost category from free text would misrepresent the ledger rather than merely leave it unclassified.
4. **Two more real gaps were found only by running the real import and then structurally auditing its own residual warnings, never by guessing.** A corporate-action row's defining date is only reliably present in `Ngày ĐKCC` (record date) or `Ngày GDKHQ` (ex-rights date) until the action actually settles, at which point `Ngày thực hiện` (execution date) is filled in too -- confirmed by type-only inspection showing populated date cells in the record/ex-rights columns on every row where the execution-date column was empty. `_dividend_events()` now falls back through these only when the primary date is genuinely absent. Separately, `Tiền cổ tức` (a nominal dividend-money label) was essentially always blank on real rows, while `Số tiền thực nhận trừ thuế` (amount actually received, net of tax) was reliably populated instead; `cash_amount` now prefers it. Neither fix reads or infers a value the workbook does not already, unambiguously provide.
5. **A new fail-closed check prevents a subtler, more dangerous failure mode: a syntactically clean import that silently extracted nothing.** `ZERO_EVENTS_DESPITE_RECOGNIZED_SOURCES` fires when every present Trade/Dividend/Money/margin sheet header is recognized yet zero events resulted; `portfolio_status()` then reports `RECONCILIATION_INCOMPLETE` with explicit reason codes instead of `READY`. This is exactly the failure mode a future schema regression would produce, and it would otherwise read identically to a workbook that genuinely records no activity.
6. **The import layout was bumped `PORTFOLIO_CONTEXT_IMPORT_LAYOUT_V2` -> `V3` -> `V4`, never overwriting a prior attempt.** The original failed `V2` attempt (workbook SHA `f994bfd49bba3373dda8cff1028a00d31568e7a61bdbda261bc4bebd241ab13e`) had already written its own immutable, empty-result artifacts; `V3` (header-recognition fixes) and `V4` (dividend row-level refinement, discovered from auditing `V3`'s own real residual warnings) each create their own immutable revision alongside it, per the existing versioned-layout mechanism -- nothing was deleted, mutated, or silently replaced.
7. **The real, validated result is honest, not manufactured.** The real import now reaches **370 events** and **25 current positions** (from 0/0). Every remaining warning (6 `CASH_DIVIDEND_AMOUNT_UNRESOLVED`, 3 `DIVIDEND_ROW_INVALID`, 2 `STOCK_DISTRIBUTION_QUANTITY_UNRESOLVED`, 2 `SELL_QUANTITY_EXCEEDS_DERIVED_HOLDING`, 1 `TOTAL_VIEW_HINT_UNUSABLE`, 1 informational `MARGIN_LEGACY_POSITIONAL_LAYOUT_USED`) was individually traced, via the same type-only structural inspection technique, to genuinely incomplete or pending real source data (a handful of rows with no date in any of three candidate columns, a corporate action awaiting settlement with only its theoretical entitlement recorded, a real trade-reconciliation question) -- never a remaining parser mapping gap. `Total` stays the accepted, explicitly non-authoritative reconciliation hint per the original contract.

7 new/updated focused tests in `tests/test_private_portfolio_context.py` (non-row-1 header detection with whitespace/case/punctuation normalization, the Vietnamese `đ` fold, a combined real-style Trade/Dividend/Money/margin fixture reproducing the real workbook's structure with fabricated data only, the record/ex-rights date fallback, fractional-entitlement-is-not-a-mismatch, and the new zero-events fail-closed status), plus one existing test's hardcoded layout-version string updated across both bumps. All 20 tests in the file pass; the pre-existing 4 failures in the sibling `current_portfolio_risk_envelope`/`current_portfolio_risk_research`/`portfolio_risk_analysis` suites were confirmed present on the unmodified baseline (via `git stash`) before this milestone, unrelated to this fix. `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. No real holding, quantity, price, amount, ticker, or other owner financial value was ever printed, logged, committed, or placed in a test fixture -- every diagnostic used during investigation reported only sheet names, header-row positions, normalized column-label text, cell *types* (never values), and warning/reason codes; all test fixtures are synthetic. The real private workbook and its derived artifacts remain solely under `%USERPROFILE%\.stocklookup\portfolio\`. No Daily run, provider/network call, Dashboard, AI handoff, publication, deployment, or authority promotion. `PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1` remains only a proposed successor, not started, not queued; owner authorization is still required before it begins.

## 2026-09-09 - Portfolio-Aware Decision And Risk Sizing V1

`PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1 = COMPLETE / PRIVATE_REAL_PORTFOLIO_VALIDATED / EXECUTION_AUTHORITY_UNCHANGED` (owner override `OWNER_AUTHORIZATION_2026_09_09_PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1`, local commit `6a447db9841db683b2c2e7433401bcf92c2f2a3a`, base `1c08b6201fc3462a97dd2dfcdf152caabcc44598`, fresh dedicated worktree per the directive).

1. **New `portfolio_aware_decision.py` joins the existing objective Integrated Decision against private state, without a second risk or security-attractiveness engine.** `build_ticker_portfolio_aware_decision()` reads `integrated_investment_decision_product/v1`'s `research_action_posture`/`trigger`/`invalidation` verbatim and joins them against `private_portfolio_context.py`'s `portfolio_snapshot/v1` (Foundation's own artifact, consumed read-only -- the owner workbook is never re-parsed). `current_portfolio_risk_envelope/v1`'s existing boundary is reused, not duplicated: `execution_qualified_quantity` stays unconditionally `None`/`NOT_QUALIFIED`, matching that module's own permanently `BLOCKED` `position_sizing_status`.
2. **PROBE is a deterministic read of two already-existing fields, never a manufactured state.** A ticker is probe-eligible only when `research_action_posture == "EARLY_WATCH"` (the existing product's own early bullish structural-reversal/base-compression state, *before* any breakout trigger) **and** that same product already supplies a non-null `invalidation_level` and a proposed entry/trigger reference (`_probe_eligible`). No oversold indicator, volatility measure, or subjective judgment is consulted. A probe uses `probe_risk_budget_multiplier` (new, owner-overridable, default `0.50`) of the normal risk budget and is subject to the identical concentration/cash/gross-exposure/risk-budget gates a full add faces -- never exempted from any of them.
3. **`margin_economics/v1` is honestly `NOT_EVALUATED` for every real record today, and that is the correct answer, not a gap.** Direct source verification (`security_decision_context.py`'s `no_target_price: True` invariant; `current_valuation_opportunity_integration.py`'s `target_price: "NOT_EMITTED"`) confirms no module anywhere in this repository emits a deterministic price target or reward boundary. The margin-economics calculation (financing-cost-adjusted net reward/risk, four research-capacity bands -- `NO_MARGIN` / `INTERPOLATED_MIN_TO_MAX` (the brief's separately-described "qualifying near 2.0" and "between 2.0 and 3.0" bands are the same continuous interpolation, not two states) / `MAX_BAND`, each capped again by margin-debt-to-NAV, concentration, gross-exposure, and the risk-budget ceiling itself) is fully implemented and exercised by 8 synthetic tests injecting an explicit `reward_boundary`, but the real-data run correctly reports `NOT_EVALUATED` (reason `NO_QUALIFIED_UPSIDE_OR_REWARD_BASIS_UPSTREAM`) for every one of the 282 real tactical candidates rather than inventing a target to produce a more interesting-looking result.
4. **Adding five new policy defaults required bumping `SYSTEM_DEFAULT_POLICY_VERSION`, a real correctness finding caught before it caused silent corruption.** The first attempt to add `probe_risk_budget_multiplier`/`tactical_margin_holding_days`/`min_net_reward_risk_for_margin`/`strong_net_reward_risk_for_max_margin`/`max_financing_cost_fraction_of_gross_upside` to `SYSTEM_DEFAULT_POLICY_V1`'s dict body under its *existing* version string caused a real `PRIVATE_PORTFOLIO_CONTENT_ADDRESS_CONFLICT` when re-running `import_workbook()` against the real, unchanged owner workbook: `_import_layout()`'s own identity folds in `SYSTEM_DEFAULT_POLICY_VERSION`, so the unchanged version string routed the enriched snapshot to the *same* immutable directory as the pre-existing V1 artifact, which now had different content. Fixed by bumping the version itself (`SYSTEM_DEFAULT_POLICY_V1` -> `V2`) so a fresh import lands in its own new, additional layout directory -- exactly the discipline `IMPORT_LAYOUT_VERSION`'s own V2->V3->V4 precedent already establishes, just not yet applied to this sibling version field. Two test assertions and the dict constant's name (`SYSTEM_DEFAULT_POLICY_V1` dict -> `SYSTEM_DEFAULT_POLICY_FIELDS`, decoupled from the version string it backs) were updated accordingly.
5. **A pre-existing `--help` crash on every `portfolio` subcommand was found and fixed while extending the same CLI.** `argparse`'s help formatter treats `%` in a help string as a format specifier; `"...default: %USERPROFILE%\\..."` (present on `portfolio import` and `portfolio status` since Foundation) crashed `--help` with `ValueError: unsupported format character 'U'` for every one of those subcommands, not just the new `evaluate`. Fixed by escaping to `%%USERPROFILE%%` in all four occurrences; verified `--help` now succeeds for `import`, `status`, and the new `evaluate`.
6. **Real validation against the actual owner portfolio required one real, authorized re-import, not a workaround.** The real retained `portfolio_snapshot_v1.json` on this machine predated this milestone's five new policy fields (content-addressed and immutable, so it could not silently gain them). Re-running Foundation's own already-shipped `import_workbook()` against the same, unchanged real workbook file already on disk (no new parsing logic, no provider call, no Daily run) produced a new V2-layout revision alongside the untouched original V1 one, updating only `latest_import.json`'s pointer. This is the intended, designed use of the versioned-layout mechanism, not an exception to it.

**REAL PRIVATE VALIDATION** (via `.\stocklookup.ps1 portfolio evaluate`, session `2026-09-09`, counts only -- no private value below): `portfolio_state_status = AVAILABLE`; `security_denominator = 1,683`; `active_position_count = 8`. By disposition: `ADD_WITHIN_RISK_CEILING = 78`, `ADD_ELIGIBLE_SIZING_UNAVAILABLE = 17`, `PROBE_WITHIN_RISK_CEILING = 148`, `PROBE_ELIGIBLE_SIZING_UNAVAILABLE = 39`, `HOLD_EXISTING_NO_ACTION = 6`, `AVOID_NO_PORTFOLIO_ACTION = 431`, `NO_ADD = 754`, `INSUFFICIENT_FOR_PORTFOLIO_DECISION = 210`; `binding_constraint = NONE` for all (no real sector/cash/gross-exposure/margin-debt cap is currently breached); `portfolio_constraint_completeness`: `FULL = 226`, `PARTIAL = 1,457` (sector not governed-classified for the remainder); margin economics `NOT_APPLICABLE = 1,401`, `NOT_EVALUATED = 282` (0 `EVALUATED`, as explained in point 3); `execution_qualified_count = 0`. The artifact is deterministic and idempotent: re-running produced the byte-identical `portfolio_aware_decision_identity` both before and after the V2 re-import. The full record set was written only to `%USERPROFILE%\.stocklookup\portfolio\portfolio_aware_decisions\2026-09-09\portfolio_aware_decision_v1.json`; nothing above, nor anywhere in this session's terminal output, git history, or test fixtures, contains any real holding, quantity, price, NAV, cash, or margin figure.

**TESTS**: 68 new focused tests in `tests/test_portfolio_aware_decision.py` (23 scenarios covering every item in the validation checklist: new/existing/over-cap positions, sector/cash/gross-exposure/margin-debt caps, probe sizing and its two guard conditions, missing invalidation, cash-only vs. margin-eligible tactical trades, all margin research bands, financing-cost disqualification, excluded positions, no-execution-qualification, determinism) all pass; 1 pre-existing `tests/test_private_portfolio_context.py` assertion updated for the additive policy-version bump, 20/20 pass. `tests/test_integrated_investment_decision_product.py` (62 tests, untouched module) and `tests/test_current_portfolio_risk_envelope.py`'s one environment-independent test pass unchanged, confirming zero regression; the sibling suite's other 2 tests fail only in this fresh worktree from a missing gitignored fixture directory never force-tracked in Git (confirmed pre-existing and environment-only by running the identical tests in the primary checkout, where they pass). `py_compile`, `git diff --check`, and `tools/stocklookup_roadmap.py --check` (drift `PASS`) all clean. No push occurred as part of this record; see `docs/STATE.md`/`docs/ROADMAP_STATE.json` for the committed-and-pushed disposition once fast-forwarded. No Daily run, provider/network call, Dashboard, AI/public handoff, deployment, or execution/liquidity-authority promotion at any point.

## 2026-09-09 - Empirical Setup Outcome Calibration V1

`EMPIRICAL_SETUP_OUTCOME_CALIBRATION_V1 = COMPLETE / CALIBRATION_PIPELINE_READY / EMPIRICAL_SAMPLE_INSUFFICIENT` (owner override `OWNER_AUTHORIZATION_2026_09_09_EMPIRICAL_SETUP_OUTCOME_CALIBRATION_V1`, local commit `bc3c293b1675ad652f40eded245752c33fdc32dd`, base `dcd713c3bbb25d440a9ae21b7988e71e816b4f85`, fresh dedicated worktree per the directive).

1. **This module deliberately builds nothing new for stages 1-4 of the required flow -- three existing modules already implement them, each explicitly deferring calibration to a successor.** `prospective_decision_outcome_feedback.build_feedback_artifact()`'s own authority boundary already says `"no_probability_or_calibration": True`; `integrated_decision_prospective_feedback.build_prospective_feedback_status()` says `"no_calibration_insufficient_sample": True`; `prospective_decision_outcome_measurement.cohort_observation_summary()` says `"INSUFFICIENT_SAMPLE_FOR_CALIBRATION": True` on every group, unconditionally. `empirical_setup_outcome_calibration.py` is exactly the successor those three boundaries point to -- it reads their outputs read-only and adds only the fifth stage, empirical calibration.
2. **Two genuine T0 lineages exist in this repository, not one, and they have very different real depth.** `durable_prospective_research_case_store` (the case-admission-gated lineage `prospective_decision_outcome_measurement.py` operates over) has **zero** real non-fixture cases -- confirmed both by a prior milestone's own finding (2026-08-31) and by direct inspection today (`operations-review/` has no `durable-prospective-research-case-store*` directory at all). The bulk per-session lineage (`prospective_decision_retention.py` immutable snapshots + `prospective_decision_outcome_feedback.py`'s broader canonical-handoff-qualified corpus discovery) is the one genuinely populated with real data: `discover_prospective_corpus()` finds exactly 2 genuine T0-qualified Integrated Decision sessions today (`2026-09-03`, `2026-09-08`), independent of the single immutable snapshot (`2026-09-09`) `prospective_decision_retention.discover_snapshots()` alone would report. This module builds on the richer, real lineage, not the empty one.
3. **T60 is added by calling the existing bridge's own private horizon primitives directly, not by mutating its shared production contract.** `integrated_decision_prospective_feedback.FORWARD_HORIZONS` (`{1,3,5,10,20}`) feeds the Daily Integrated Decision Brief in production; adding a sixth key there would change that shared artifact's shape for every existing consumer for a one-milestone-specific need. Instead, `_forward_horizon`/`_close_excursion` (already fully generic over an arbitrary `horizon_sessions` value) are called directly with `60`, over the **union** of the two genuine T0 sources' own governed chains (`resolve_combined_chain_and_snapshots`) -- a superset of, never a subset of, either branch's own chain, so this introduces no additional look-ahead risk versus what the existing bridge already established for T5/T10/T20 (which are reused verbatim from the existing `feedback_record`, never recomputed).
4. **R-multiples, cohort keys, and the sample-adequacy ladder are the genuinely new content.** `(entry - invalidation_level) / entry` -- the T0 decision's own already-retained trigger/invalidation levels, never a stop-loss -- is the sole R-unit denominator; a missing or non-positive downside is an explicit `UNAVAILABLE_*` status, never a zero-substituted R-multiple. Cohort keys prioritize contract versions (pulled from `integrated_investment_decision_product.CONTRACT_VERSION`/`prospective_decision_outcome_feedback.CONTRACT_VERSION` directly, so a future contract bump automatically forks the cohort rather than silently pooling incompatible data) ahead of posture/tactical/invalidation-method family and horizon; market regime is stratified only as an *additional*, never-replacing variant, and only when every member retains a real regime value and the split itself still clears the `DESCRIPTIVE_ONLY` floor. `sample_adequacy()` implements the exact three-tier policy from the directive (20/5, 50/10) with the distinct-T0-session requirement enforced independently of raw observation count (a cohort with 25 observations from one single T0 session is still `INSUFFICIENT_SAMPLE` -- verified by a dedicated test).
5. **The bounded Portfolio V2 hook is additive and inert by construction, not merely by policy.** `portfolio_aware_decision.build_ticker_portfolio_aware_decision()`'s new `calibration_artifact` parameter defaults to `None`; when supplied, `empirical_reward_context_for_cohort()` performs a pure, read-only lookup and returns `NOT_AVAILABLE` for anything short of `CALIBRATED_RESEARCH` adequacy. It is never passed into `_evaluate_margin_economics()`'s `reward_boundary` parameter (which remains exactly as explicit/caller-supplied as `PORTFOLIO_AWARE_DECISION_AND_RISK_SIZING_V1` left it) -- a direct test (`test_21b_...`) proves every execution/sizing/margin field on a record is byte-identical whether or not a matching `CALIBRATED_RESEARCH` cohort is supplied.

**REAL RETAINED CORPUS** (read-only, `tools/run_empirical_setup_outcome_calibration.py --root .`): T0 snapshots/artifacts found = 2 genuine T0-qualified sessions (`2026-09-03`, `2026-09-08`) via `prospective_decision_outcome_feedback.discover_prospective_corpus()`, plus 1 genuine immutable snapshot (`2026-09-09`) via `prospective_decision_retention.discover_snapshots()`; combined governed chain has 3 distinct sessions. `observation_count = 5,049` (1,683 tickers x 3 T0-source sessions); `t0_session_count = 3`; `cohort_count = 100` (25 base posture/tactical/invalidation-method combinations x 4 horizons); every one of the 100 cohorts is `INSUFFICIENT_SAMPLE` at every horizon (0 reach `DESCRIPTIVE_ONLY`, 0 reach `CALIBRATED_RESEARCH`) -- correct and expected, since maturation requires at least 5 distinct T0 sessions even for the lowest tier and only 2-3 exist today; no per-ticker or per-session value beyond these aggregate counts is reported here.
