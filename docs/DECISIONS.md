...
2. Case eligibility is bounded to the 446 records with both existing fundamental-quality and current technical/tactical research evidence.  Seventy-three technical-only records are explicitly `MARKET_ONLY_CASE`; eight records terminate `INSUFFICIENT_CASE_EVIDENCE`.  Missing valuation, TTM, growth, Piotroski, audit, or catalyst evidence is an explicit gap and is never converted into negative performance.
3. Only retained `OFFICIAL_QUALIFIED` `CONFIRMED_UPCOMING`/`CONFIRMED_RECENT` corporate-event records become catalyst context (45 entries: 29 dividend/corporate-action and 16 other-event categories).  Technical state is a separate `MARKET_CONFIRMATION_TRIGGER`, never a corporate catalyst.  The retained event contract remains non-event-driven: no price impact, probability, target, entry, sizing, or decision authority follows.
