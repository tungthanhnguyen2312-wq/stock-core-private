...
- **B1 is delivered, owner-approved and active** (2026-08-03). `config/official_source_registry.json` declares all four source classes with hosts, document types, rates, timeouts, retries, robots/terms, retention and failure classification; `official_source_registry.admit()` now admits the approved sources. B2/B3 remain deliberately bounded: the 2026-08-08 VCB VSDC acquisition produced one unpromoted `last_registration_date_notice` observation (`stock_dividend`, record date, explicit entitlement ratio). It has no official ex-date, execution confirmation, or share counts, so no factor, valuation, or market gate activates.
