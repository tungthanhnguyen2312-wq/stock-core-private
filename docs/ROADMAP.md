...
- **Pillar B — official corporate-action ingestion and price-adjustment engine** — `docs/official_corporate_action_ingestion_design.md`.
  ...
  **P1J.1 Current-Share Authority Integrity Repair shipped 2026-08-03**: the coverage counts P1H/P1I/P1J reported were literals in the operating report, and two of the three `qualified_official` share anchors were wrong (HPG's appears in no citation or ledger; VCB's is the citation mistyped by 40 shares). Anchors now read from the citation store, the session is a required input on every session-relative resolution, freshness is measured against the provider observation date via ex-right dates only, read failures fail closed as `unresolved_error`, and a market cap cannot be qualified while the price basis is unverified. Measured: **`qualified_official` 0**, provider-reported 1,680, unverifiable freshness 2, unavailable 1, universe 1,683. `operations-review/p1j1-share-authority-integrity-20260803/P1J1_OPERATIONS_REVIEW.md`.
...
- **B1 is delivered, owner-approved and active** (2026-08-03). `config/official_source_registry.json` declares all four source classes with hosts, document types, rates, timeouts, retries, robots/terms, retention and failure classification; `official_source_registry.admit()` now admits the approved sources. B2/B3 remain deliberately bounded: the 2026-08-08 VCB VSDC evidence now has a hash-verified record-date notice and a directly discovered listing-transfer notice. The latter explicitly states `trading_date=2025-05-09` and the same stated dividend purpose/record date, so the two observations may cross-link only on that composite document-stated reference. No official ex-date, listing-effective date, completion claim, or share-authority field exists; no factor, valuation, or market gate activates.
...
- **P2a -- historical point-in-time valuation: REOPENED and now BLOCKED (2026-08-04).** The cited HPG 2024-12-31 close of 19,830 was accepted because `adjustment_status: raw_as_quoted_no_adjustment_applied` and because HPG's 2024 corporate action had settled before that date. Both halves fail. The status records that *this repository* applied no adjustment, not that the provider returned none; and a back-adjustment is applied by events **after** the cited date -- HPG had share issues on 2025-06-26 and 2026-05-25 -- so reasoning about actions already settled looks in the wrong direction entirely. 19,830 is not a multiple of the 50 VND HOSE tick for its price band, so it was never a matched order price. Both production market-price citations (HPG and VCB, 2024-12-31) are now rejected at `semantic_evidence_bridge` with `provider_series_retrospectively_rewritten`. P2a becomes available again only from a price basis that is raw as-traded or documented-adjusted.
