...
def build(*, descriptive: Mapping[str, Any], tactical: Mapping[str, Any], peer_relative: Mapping[str, Any], fundamental: Mapping[str, Any], valuation: Mapping[str, Any], scenario: Mapping[str, Any], corporate_intelligence: Mapping[str, Any], market_flow_positioning: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for ticker in sorted(descriptive["records"]):
        ...
        flow_evidence = "FLOW_UNAVAILABLE" if not flow else "CONTRADICTORY_FLOW_EVIDENCE" if any("DIVERGENCE" in item or "SELL_PRESSURE" in item for item in relationships) else "SUPPORTING_FLOW_EVIDENCE" if any("CONFIRMATION" in item or "BUY_SUPPORT" in item for item in relationships) else "MIXED_OR_NEUTRAL_FLOW_EVIDENCE"
