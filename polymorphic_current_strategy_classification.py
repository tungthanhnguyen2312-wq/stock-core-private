...
def build(*, descriptive: Mapping[str, Any], tactical: Mapping[str, Any], peer_relative: Mapping[str, Any], fundamental: Mapping[str, Any], valuation: Mapping[str, Any], scenario: Mapping[str, Any], corporate_intelligence: Mapping[str, Any], market_flow_positioning: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for ticker in sorted(descriptive["records"]):
        ...
        flow = (market_flow_positioning or {}).get("records", {}).get(ticker) or {}
        relationships = flow.get("price_flow_relationships") or []
        flow_evidence = "FLOW_UNAVAILABLE" if not flow or not flow.get("coverage", {}).get("available_dimensions", 0) else "CONTRADICTORY_FLOW_EVIDENCE" if any("DIVERGENCE" in item or "SELL_PRESSURE" in item for item in relationships) else "SUPPORTING_FLOW_EVIDENCE" if any("CONFIRMATION" in item or "BUY_SUPPORT" in item for item in relationships) else "MIXED_OR_NEUTRAL_FLOW_EVIDENCE"
        record = {"ticker": ticker, "research_session": descriptive["session"], "strategies": strategy_map, "eligible_strategy_ids": eligible, "record_strategy_state": "MULTI_STRATEGY_ELIGIBLE" if len(eligible) > 1 else "SINGLE_STRATEGY_ELIGIBLE" if eligible else "DATA_LIMITED" if any(item["status"] in {"INSUFFICIENT_DATA", "BLOCKED", "PARTIAL"} for item in strategy_map.values()) else "NO_CURRENT_STRATEGY_FIT", "tactical_context": {"entry_state": t.get("entry_state"), "entry_action": t.get("entry_action")}, "scenario_context": {"scenario_disposition": s.get("scenario_disposition"), "probability_status": s.get("probability_status")}, "flow_evidence": {"status": flow_evidence, "relationships": relationships, "optional_only": True, "does_not_change_strategy_eligibility": True}, "fundamental_context": {"authority_tier": f.get("authority_tier"), "trajectory_status": (f.get("fundamental_trajectory_context") or {}).get("trajectory_status")}, "event_context": {"disposition": c.get("intelligence_disposition"), "event_ids": [event.get("event_id") for event in c.get("events") or []]}}
