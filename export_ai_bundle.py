...
def load_current_corporate_event_context_artifact(path: Path) -> Mapping[str, Any] | None:
    """Fail closed unless the explicit retained corporate-event artifact verifies itself."""
    try:
        artifact = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(artifact, dict):
            return None
        if artifact.get("contract_version") != "current_corporate_event_context/v1":
            return None
        recomputed = current_corporate_event_context_content_identity(artifact)
        if recomputed.get("artifact_sha256") != artifact.get("artifact_sha256"):
            return None
        if not isinstance(artifact.get("records"), Mapping) or not isinstance(artifact.get("coverage"), Mapping):
            return None
...
def build_current_corporate_event_context_for_ticker_safe(
    ticker: str, artifact: Mapping[str, Any],
) -> dict[str, Any] | None:
    """Pass through one current event context; never infer impact, eligibility, or a score."""
    try:
        records = artifact.get("records")
        record = records.get(ticker) if isinstance(records, Mapping) else None
        if not isinstance(record, Mapping) or record.get("ticker") != ticker:
        ...
        return {
            ...
            "blocked_outputs": copy.deepcopy(artifact.get("blocked_outputs")),
            "authority_boundary": copy.deepcopy(artifact.get("authority_boundary")),
            "status": "available" if record.get("events") else "data_limited",
            "is_actionable": False,
        }
    except Exception:
        return None
...
def attach_current_corporate_event_context(
    bundle_entries: dict[str, dict], include: bool, artifact_path: str | None,
) -> dict[str, dict]:
    """Attach an independent explicit-path context sibling without changing default bundles."""
    if not include or not artifact_path:
        return bundle_entries
    artifact = load_current_corporate_event_context_artifact(Path(artifact_path))
    if artifact is None:
        return bundle_entries
    attached: dict[str, dict[str, Any]] = {}
    for ticker in bundle_entries:
        result = build_current_corporate_event_context_for_ticker_safe(ticker, artifact)
        if result is not None:
            attached[ticker] = result
    for ticker, result in attached.items():
        bundle_entries[ticker]["current_corporate_event_context"] = result
    return bundle_entries
...
def load_current_research_scenario_context_artifact(path: Path) -> Mapping[str, Any] | None:
    try:
        artifact = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(artifact, dict) or artifact.get("contract_version") != "current_research_scenario_context/v1":
            return None
        if current_research_scenario_context_content_identity(artifact).get("artifact_sha256") != artifact.get("artifact_sha256"):
            return None
        return artifact if isinstance(artifact.get("records"), Mapping) else None
    except Exception:
        return None
...
def attach_current_research_scenario_context(bundle_entries: dict[str, dict], include: bool, artifact_path: str | None) -> dict[str, dict]:
    """Attach a verified scenario-context sibling without changing any decision-bearing field."""
    if not include or not artifact_path:
        return bundle_entries
    artifact = load_current_research_scenario_context_artifact(Path(artifact_path))
    if artifact is None:
        return bundle_entries
    for ticker, entry in bundle_entries.items():
        record = artifact["records"].get(ticker)
        if isinstance(record, Mapping) and record.get("ticker") == ticker:
            entry["current_research_scenario_context"] = {
                "ticker": ticker, "source_artifact_identity": artifact.get("artifact_identity"),
                "source_contexts": copy.deepcopy(artifact.get("source_contexts")),
                "scenario_context": copy.deepcopy(record), "authority_boundary": copy.deepcopy(artifact.get("authority_boundary")),
                "blocked_outputs": copy.deepcopy(artifact.get("blocked_outputs")), "is_actionable": False,
            }
    return bundle_entries
...
# Historical matched-trading-value / ADTV20 — opt-in, sibling of historical
# research context. Never nested into current-session liquidity. Default off.
def load_historical_matched_trading_value_artifact(path: Path) -> Mapping[str, Any] | None:
    try:
        artifact = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return None
    if not isinstance(artifact, dict):
        return None
    try:
        recomputed = historical_matched_trading_value_content_identity(artifact)
    except Exception:
        return None
    if recomputed.get("artifact_sha256") != artifact.get("artifact_sha256"):
        return None
    if artifact.get("contract_version") != "historical_matched_trading_value_authority/v1":
        return None
    return artifact


def attach_historical_matched_trading_value(
    bundle_entries: dict[str, dict], include: bool, artifact_path: str | None,
) -> dict[str, dict]:
    if not include or not artifact_path:
        return bundle_entries
    artifact = load_historical_matched_trading_value_artifact(Path(artifact_path))
    if artifact is None:
        return bundle_entries
    records = artifact.get("records")
    if not isinstance(records, Mapping):
    ...
    for ticker, entry in bundle_entries.items():
        ...
        result["source_artifact_identity"] = artifact.get("artifact_identity")
        result["authority_boundary"] = artifact.get("authority_boundary")
        result["adtv20_contract"] = artifact.get("adtv20_contract")
        result["status"] = "available"
        result["is_actionable"] = False
        entry["historical_matched_trading_value"] = result
    return bundle_entries
...
def main() -> int:
    ...
    parser.add_argument("--current-research-scenario-context-path", metavar="PATH",
                        help="Explicit path to a self-verifying retained current_research_scenario_context artifact.")
    ...
    parser.add_argument("--include-current-corporate-event-context", action="store_true",
                        help="Opt-in, disabled by default: attach a separately retained current "
                             "corporate-event research context. Event facts only: never price impact, "
                             "probability, EVENT_DRIVEN eligibility, research_priority, entry_action, "
    ...
    parser.add_argument("--market-wide-historical-research-context-path", metavar="PATH",
                        help="Explicit path to a retained"
                             ...
                             " inferred or hardcoded.")
    parser.add_argument("--include-historical-matched-trading-value", action="store_true",
                        help="Opt-in, disabled by default: attach"
                             " tickers[ticker].historical_matched_trading_value from a retained"
                             " historical_matched_trading_value_authority artifact. Requires"
                             " --historical-matched-trading-value-path; hash mismatch fails closed."
                             " Liquidity-input research only; never ADV-as-money, position sizing,"
                             " participation caps, PIT, RAW_AS_TRADED, or frozen-session rewrite.")
    parser.add_argument("--historical-matched-trading-value-path", metavar="PATH",
                        help="Explicit path to a retained"
                             " historical_matched_trading_value_authority artifact; never inferred.")
    ...
    parser.add_argument("--include-market-wide-current-fundamental-research", action="store_true",
                        help="Opt-in, disabled by default: attach"
                             ...
                             " presence, an explicit blocked reason, and only permitted same-provider"
                             " consecutive-quarter trend research -- never absolute provider facts)."
    ...
    attach_historical_matched_trading_value(
        bundle_entries, args.include_historical_matched_trading_value,
        args.historical_matched_trading_value_path,
    )
    # Own dedicated flag; order relative to the others does not matter since nothing else reads
