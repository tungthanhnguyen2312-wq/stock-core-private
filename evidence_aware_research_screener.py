...
def _eval(p:Mapping[str,Any],r:Mapping[str,Any])->tuple[bool,dict]:
 ...
 if t=='range_state':
  ...
  return v in states,{'predicate':p,'value':v,'reason':'MATCHED' if v in states else 'STATE_NOT_MATCHED'}
 if t=='setup':
  setup=p.get('setup_id');v=r['values'].get('active_setup_ids',[])
  if not isinstance(setup,str):return False,{'predicate':p,'reason':'UNSUPPORTED_SETUP'}
  return setup in v,{'predicate':p,'value':v,'reason':'MATCHED' if setup in v else 'SETUP_NOT_PRESENT'}
 if t=='and':
...
def _specs(session):return [
 ...
 ,{'name':'TREND_CONTINUATION_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'TREND_CONTINUATION_CONTEXT'}}
 ,{'name':'NEAR_RESISTANCE_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'NEAR_RESISTANCE_CONTEXT'}}
 ,{'name':'BREAKOUT_WITH_ELEVATED_VOLUME_PROXY_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'BREAKOUT_WITH_ELEVATED_VOLUME_PROXY_CONTEXT'}}
 ,{'name':'NEAR_SUPPORT_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'NEAR_SUPPORT_CONTEXT'}}
 ,{'name':'WEAKENING_STRUCTURE_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'WEAKENING_STRUCTURE_CONTEXT'}}
 ,{'name':'BREAKDOWN_SETUP_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'BREAKDOWN_CONTEXT'}}
 ,{'name':'RELATIVE_STRENGTH_CONTEXT','research_session':session,'predicate':{'type':'setup','setup_id':'RELATIVE_STRENGTH_CONTEXT'}}
 ]
def build(product:Mapping[str,Any],eligibility:Mapping[str,Any],context:Mapping[str,Any],dossiers:Mapping[str,Any],event_context:Mapping[str,Any]|None=None,price_context:Mapping[str,Any]|None=None,setup_context:Mapping[str,Any]|None=None)->dict:
 er={x['ticker']:x for x in eligibility['records']};cr={x['ticker']:x for x in context['records']};ev={x['ticker']:x for x in (event_context or {'records':[]})['records']};price={x['ticker']:x for x in (price_context or {'records':[]})['records']};setups={x['ticker']:x for x in (setup_context or {'records':[]})['records']}; rows=[]
 for r in product['stock_research']:
  t=r['ticker']; authority=cr[t].get('relative_context_authority'); event=ev.get(t,{}); facts=event.get('event_facts',[]); structure=price.get(t,{}); setup=setups.get(t,{});rows.append({'ticker':t,'dossier_identity':dossiers[t]['dossier_identity'],'values':{'momentum_20d':r['ai_ready_brief']['facts']['momentum_20d'],'volatility_20d':r['ai_ready_brief']['facts']['volatility_20d'],'trend_state':r['research_summary']['trend_state'],'fundamental_authority':r['research_summary']['fundamental_authority'],'price_structure_state':structure.get('structure_status'),'range_state':structure.get('range_state'),'active_setup_ids':setup.get('active_setup_ids',[])},'lenses':er[t]['lenses'],'relative_available':cr[t]['context_status']=='AVAILABLE' and authority=='QUALIFIED_CLASSIFICATION','relative_provider_descriptive_available':cr[t]['context_status']=='AVAILABLE' and authority=='PROVIDER_DESCRIPTIVE_CLASSIFICATION','relative_any_available':cr[t]['context_status']=='AVAILABLE','relative_context_authority':authority,'event_available':bool(facts),'upcoming_event':any(x['temporal_state']=='ANNOUNCED_FUTURE' for x in facts),'negative_event':any(x.get('research_direction')=='NEGATIVE' for x in event.get('catalyst_interpretations',[])),'event_context_identity':event.get('event_context_identity'),'warnings':r['warnings']})
 sources={'daily_product':product['artifact_identity'],'eligibility':eligibility['artifact_identity'],'relative_context':context['artifact_identity'],'event_context':event_context.get('artifact_identity') if event_context else None,'price_structure_context':price_context.get('artifact_identity') if price_context else None,'setup_context':setup_context.get('artifact_identity') if setup_context else None}; presets=[query(rows,s,sources) for s in _specs(product['daily_market_research']['session'])];a={'schema_version':'1.0.0','contract_version':'evidence_aware_research_screener/v1','research_session':product['daily_market_research']['session'],'cohort_scope':'EMPIRICAL_ACTIVE_SHADOW_ONLY','source_artifact_identities':sources,'records':rows,'presets':presets,'verdict':'EVIDENCE_AWARE_RESEARCH_SCREENER_V1_READY'};a['artifact_sha256']=_hash(a);a['artifact_identity']='evidence_aware_research_screener:'+a['artifact_sha256'];return a
